...
static ngx_fd_t ngx_openat_file_owner(ngx_fd_t at_fd, const u_char *name,
                                      ngx_int_t mode, ngx_int_t create,
                     ...
                     ngx_open_file_info_t *of, ngx_pool_t *pool)
{
    ...
    if (file)
    {
        ...
        if (file->use_event
            || (file->event == NULL && (of->uniq == 0 || of->uniq == file->uniq)
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_CORE, pool->log, 0,
                       "retest open file: %s, fd:%d, c:%d, e:%d", file->name,
...
found:
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_CORE, pool->log, 0,
                   "cached open file: %s, fd:%d, c:%d, e:%d, u:%d", file->name,
...
static ngx_fd_t
ngx_openat_file_owner(ngx_fd_t at_fd, const u_char *name, ngx_int_t mode,
                      ngx_int_t create, ngx_int_t access, ngx_log_t *log)
{
    ngx_fd_t        fd;
    ngx_err_t       err;
...
static ngx_fd_t
ngx_open_file_wrapper(ngx_str_t *name, ngx_open_file_info_t *of, ngx_int_t mode,
                      ngx_int_t create, ngx_int_t access, ngx_log_t *log)
{
    ngx_fd_t fd;
    ...
    {
        at_fd = ngx_open_file("/", NGX_FILE_SEARCH | NGX_FILE_NONBLOCK,
                              NGX_FILE_OPEN, 0);
    ...
    {
        fd =
            ngx_openat_file(at_fd, p, mode | NGX_FILE_NOFOLLOW, create, access);
    }
                       ...
                       ngx_log_t *log)
{
    ...
    if (!of->log)
    {
        ...
        fd =
            ngx_open_file_wrapper(name, of, NGX_FILE_RDONLY | NGX_FILE_NONBLOCK,
                                  NGX_FILE_OPEN, 0, log);
    }
    else
    {
...
ngx_open_file_add_event(ngx_open_file_cache_t  *cache,
                        ngx_cached_open_file_t *file, ngx_open_file_info_t *of,
                        ngx_log_t *log)
{
    ngx_open_file_cache_event_t *fev;

    if (!(ngx_event_flags & NGX_USE_VNODE_EVENT) || !of->events || file->event
        || of->fd == NGX_INVALID_FILE || file->uses < of->min_uses)
    {
        return;
    }
...
ngx_close_cached_file(ngx_open_file_cache_t  *cache,
                      ngx_cached_open_file_t *file, ngx_uint_t min_uses,
                      ngx_log_t *log)
{
                                        ...
                                        ngx_rbtree_node_t *sentinel)
{
    ...
    for (;;)
    {
        ...
        { /* node->key == temp->key */
            ...
            p = (ngx_strcmp(file->name, file_temp->name) < 0) ? &temp->left
                                                              : &temp->right;
        }
