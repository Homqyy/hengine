...
#endif
{
    ...
    /* pid#tid */
    p = ngx_slprintf(p, last, "%P#" NGX_TID_T_FMT ": ", ngx_log_pid,
                     ngx_log_tid);
...
ngx_log_abort(ngx_err_t err, const char *fmt, ...)
{
    ...
    ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, err, "%*s", p - errstr,
                  errstr);
}
...
#if (NGX_WIN32)
    buf = ngx_slprintf(
        buf, last, ((unsigned)err < 0x80000000) ? " (%d: " : " (%Xd: ", err);
#else
...
ngx_log_init(u_char *prefix)
{
    ...
    ngx_log_file.fd =
        ngx_open_file(name, NGX_FILE_APPEND, NGX_FILE_CREATE_OR_OPEN,
...
ngx_log_set_log(ngx_conf_t *cf, ngx_log_t **head)
{
    ...
    else if (ngx_strncmp(value[1].data, "memory:", 7) == 0)
    {
        ...
        buf->pos =
            ngx_slprintf(buf->start, buf->end, "MEMLOG %uz %V:%ui%N", size,
...
static void
ngx_log_memory_writer(ngx_log_t *log, ngx_uint_t level, u_char *buf, size_t len)
{
    u_char               *p;
