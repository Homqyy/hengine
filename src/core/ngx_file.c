...
ngx_write_chain_to_temp_file(ngx_temp_file_t *tf, ngx_chain_t *chain)
{
    ...
    if (tf->file.fd == NGX_INVALID_FILE)
    {
        ...
        if (tf->log_level)
        {
            ngx_log_error(tf->log_level, tf->file.log, 0, "%s %V", tf->warn,
                          &tf->file.name);
        }
    }
                     ...
                     ngx_uint_t persistent, ngx_uint_t clean, ngx_uint_t access)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, file->log, 0, "hashed path: %s",
                       file->name.data);

        file->fd = ngx_open_tempfile(file->name.data, persistent, access);

        ngx_log_debug1(NGX_LOG_DEBUG_CORE, file->log, 0, "temp fd:%d",
                       file->fd);
...
ngx_create_path(ngx_file_t *file, ngx_path_t *path)
{
    ...
    for (i = 0; i < NGX_MAX_PATH_LEVEL; i++)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, file->log, 0, "temp file: \"%s\"",
                       file->name.data);
    ...
    for (/* void */; *p; p++)
    {
        ...
        *p = '\0';
        ...
        if (ngx_create_dir(dir, access) == NGX_FILE_ERROR)
        {
            ...
            switch (err)
            {
            case NGX_EEXIST: err = 0;
            case NGX_EACCES: break;
...
ngx_ext_rename_file(ngx_str_t *src, ngx_str_t *to, ngx_ext_rename_file_t *ext)
{
    ...
    if (err == NGX_EXDEV)
    {
        ...
        if (ngx_copy_file(src->data, name, &cf) == NGX_OK)
        {
            ...
            ngx_log_error(NGX_LOG_CRIT, ext->log, ngx_errno,
                          ngx_rename_file_n " \"%s\" to \"%s\" failed", name,
                          to->data);
...
failed:
    ...
    {
        ngx_log_error(NGX_LOG_CRIT, ext->log, err,
                      ngx_rename_file_n " \"%s\" to \"%s\" failed", src->data,
                      to->data);
    }
    ...
    while (size > 0)
    {
        ...
        if ((size_t)n != len)
        {
            ngx_log_error(NGX_LOG_ALERT, cf->log, 0,
                          ngx_read_fd_n " has read only %z of %O from %s", n,
                          size, from);
            goto failed;
        }
        ...
        {
            ngx_log_error(NGX_LOG_ALERT, cf->log, 0,
                          ngx_write_fd_n " has written only %z of %O to %s", n,
                          size, to);
            goto failed;
        }
...
ngx_walk_tree(ngx_tree_ctx_t *ctx, ngx_str_t *tree)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_CORE, ctx->log, 0, "tree name %uz:\"%s\"",
                       len, name);
        ...
        *p++ = '/';
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, ctx->log, 0, "tree path \"%s\"",
                       file.data);
        ...
        {
            ngx_log_debug1(NGX_LOG_DEBUG_CORE, ctx->log, 0, "tree file \"%s\"",
                           file.data);
