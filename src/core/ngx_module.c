...
ngx_cycle_modules(ngx_cycle_t *cycle)
{
    ...
    cycle->modules =
        ngx_pcalloc(cycle->pool, (ngx_max_module + 1) * sizeof(ngx_module_t *));
               ...
               char **order)
{
    ...
    if (module->version != nginx_version)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "module \"%V\" version %ui instead of %ui", file,
                           module->version, (ngx_uint_t)nginx_version);
        return NGX_ERROR;
    }
    ...
    for (m = 0; cf->cycle->modules[m]; m++)
    {
        ...
        {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "module \"%s\" is already loaded", module->name);
            return NGX_ERROR;
        }
    }
            ...
            for (m = 0; m < before; m++)
            {
                if (ngx_strcmp(cf->cycle->modules[m]->name, order[i]) == 0)
                {
                    ngx_log_debug3(NGX_LOG_DEBUG_CORE, cf->log, 0,
                                   "module: %s before %s:%i", module->name,
                                   order[i], m);
