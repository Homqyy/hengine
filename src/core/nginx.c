...
static ngx_command_t ngx_core_commands[] = {
    ...
    {ngx_string("master_env"), NGX_MAIN_CONF | NGX_DIRECT_CONF | NGX_CONF_TAKE1,
     ngx_master_set_env, 0, 0, NULL},
...
ngx_show_version_info(void)
{
    ...
    if (ngx_show_help)
    {
        ngx_write_stderr(
            ...
            "[-p prefix] [-g directives]" NGX_LINEFEED NGX_LINEFEED
            ...
            "exit" NGX_LINEFEED "  -q            : suppress non-error messages "
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        while (*p)
        {
            ...
            case 'T': ngx_test_config = 1; ngx_dump_config = 1;
#if (NGX_SSL && NGX_SSL_ASYNC)
                ngx_no_ssl_init = 1;
#endif
                break;
...

    {
        ...
        if (lock_file.len)
        {
            ...
            if (ccf->lock_file.len != lock_file.len
                || ngx_strncmp(ccf->lock_file.data, lock_file.data,
                               lock_file.len)
                       != 0)
            {
                ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
        ...
        else
        {
            ...
            cycle->lock_file.data = ngx_pnalloc(
                cycle->pool, ccf->lock_file.len + sizeof(".accept"));
            if (cycle->lock_file.data == NULL)
            {
                return NGX_CONF_ERROR;
            }
...
ngx_set_cpu_affinity(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (/* void */; n < cf->args->nelts; n++)
    {
        ...
        for (p = value[n].data + value[n].len - 1; p >= value[n].data; p--)
        {
            ...
            ngx_conf_log_error(
                NGX_LOG_EMERG, cf, 0,
                "invalid character \"%c\" in \"worker_cpu_affinity\"", ch);
            return NGX_CONF_ERROR;
        }
    }
...
ngx_load_module(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           ngx_dlopen_n " \"%s\" failed (%s)", file.data,
                           ngx_dlerror());
        return NGX_CONF_ERROR;
    }
