...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    for (;;)
    {
        ...
        if (need_space)
        {
            ...
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unexpected \"%c\"",
                                   ch);
                return NGX_ERROR;
            }
        }
        ...
        else
        {
            ...
            if (found)
            {
                ...
                for (dst = word->data, src = start, len = 0; src < b->pos - 1;
                     len++)
                {
                    if (*src == '\\')
                    {
                   ...
                   const char *fmt, ...)
{
    ...
    if (cf->conf_file->file.fd == NGX_INVALID_FILE)
    {
        ngx_log_error(level, cf->log, 0, "%*s in command line", p - errstr,
                      errstr);
        return;
    }
