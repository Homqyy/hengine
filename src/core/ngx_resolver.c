                                  ...
                                  ngx_str_t **names, ngx_uint_t *num)
{
    ...
    for (/* void */; p < end; p++)
    {
        if (*p == CR || *p == LF || p == (end - 1) || *(p + 1) == '#')
        {
            ...
            if (ngx_strncmp(line, "nameserver", sizeof("nameserver") - 1) == 0)
            {
                ...
                while (*line_end == ' ' || *line_end == '\t' || *line_end == CR
                       || *line_end == LF)
                {
                    line_end--;
                }
...
ngx_int_t
ngx_resolver_read_resolv_file(ngx_conf_t *cf, ngx_str_t *filename,
...
ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names, ngx_uint_t n)
{
    ...
    if (names == NULL)
    {
        if (ngx_resolver_read_resolv_file(cf, &default_file, &names, &n)
            != NGX_OK)
        {
            return NULL;
        }
...
ngx_resolve_name(ngx_resolver_ctx_t *ctx)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\"",
                   &ctx->name);
...
ngx_resolve_addr(ngx_resolver_ctx_t *ctx)
{
    ...
    else
    {
        ...
        default: /* AF_INET */ rn->node.key = addr;
        }
...
ngx_resolver_resend(ngx_resolver_t *r, ngx_rbtree_t *tree, ngx_queue_t *queue)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0,
                       "resolver resend \"%*s\" %p", (size_t)rn->nlen, rn->name,
                       rn->waiting);
...
dns_error_name:
    ...
    ngx_log_error(r->log_level, r->log, 0,
                  ...
                  ngx_resolver_strerror(code), ident, (size_t)rn->nlen,
                  rn->name);
    return;
                       ...
                       ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    {
        ngx_log_error(r->log_level, r->log, 0,
                      "wrong ident %ui response for %V, expect %ui", ident,
                      &name, qident);
        ngx_resolver_free(r, name.data);
        goto failed;
    }
    ...
    found:
        ...
        case NGX_RESOLVE_CNAME: cname = &buf[i]; break;

        case NGX_RESOLVE_DNAME: break;
                         ...
                         ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0,
                   "resolver nsrvs:%ui cname:%p ttl:%uD", nsrvs, cname,
                   rn->ttl);
...
valid:
    ...
    found:
        ...
        switch (type)
        {
        case NGX_RESOLVE_PTR: goto ptr;

        case NGX_RESOLVE_CNAME: break;
                               ...
                               ngx_str_t *name)
{
    ...
    *p = (u_char)len;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\" AAAA %i",
                   name, ident & 0xffff);
                              ...
                              ngx_str_t *name)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\" SRV %i",
                   name, ident & 0xffff);
