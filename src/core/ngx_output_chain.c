...
static ngx_inline ngx_int_t ngx_output_chain_as_is(ngx_output_chain_ctx_t *ctx,
                                                   ngx_buf_t              *buf);
        ...
        while (ctx->in)
        {
            ...
            if (bsize == 0 && !ngx_buf_special(ctx->in->buf))
            {
                ngx_log_error(NGX_LOG_ALERT, ctx->pool->log, 0,
                              ...
                              ctx->in->buf->file, ctx->in->buf->file_pos,
...
ngx_output_chain_get_buf(ngx_output_chain_ctx_t *ctx, off_t bsize)
{
    ...
    if (in->last_in_chain)
    {
        ...
        else if (!ctx->directio && ctx->bufs.num == 1
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
    ...
    for (size = 0; in; in = in->next)
    {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_CORE, c->log, 0,
                       "chain writer buf fl:%d s:%uO", in->buf->flush,
                       ngx_buf_size(in->buf));
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "chain writer in: %p",
                   ctx->out);
    ...
    for (cl = ctx->out; cl; cl = cl->next)
    {
        if (ngx_buf_size(cl->buf) == 0 && !ngx_buf_special(cl->buf))
        {
            ngx_log_error(NGX_LOG_ALERT, ctx->pool->log, 0,
                          "zero size buf in chain writer "
                          "t:%d r:%d f:%d %p %p-%p %p %O-%O",
                          cl->buf->temporary, cl->buf->recycled,
                          cl->buf->in_file, cl->buf->start, cl->buf->pos,
                          cl->buf->last, cl->buf->file, cl->buf->file_pos,
                          cl->buf->file_last);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0, "chain writer out: %p",
                   chain);
