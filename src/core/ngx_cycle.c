...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_CORE, log, 0, "log: %p %d \"%s\"",
...
old_shm_zone_done:
    ...
    for (i = 0; i < old_cycle->listening.nelts; i++)
    {
        ...
        if (ls[i].sockaddr->sa_family == AF_UNIX)
        {
            ...
            ngx_log_error(NGX_LOG_WARN, cycle->log, 0, "deleting socket %s",
                          name);
...
ngx_init_zone_pool(ngx_cycle_t *cycle, ngx_shm_zone_t *zn)
{
    ...
    file =
        ngx_pnalloc(cycle->pool, cycle->lock_file.len + zn->shm.name.len + 1);
    if (file == NULL)
    {
        return NGX_ERROR;
    }
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        if (user != (ngx_uid_t)NGX_CONF_UNSET_UINT)
        {
            ...
            if (fi.st_uid != user)
            {
                ...
                {
                    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                                  "chown(\"%s\", %d) failed", file[i].name.data,
                                  user);
...
ngx_shutdown_timer_handler(ngx_event_t *ev)
{
    ...
    {
        if (c[i].fd == (ngx_socket_t)-1 || c[i].read == NULL
            || c[i].read->accept || c[i].read->channel || c[i].read->resolver)
        {
            continue;
        }
