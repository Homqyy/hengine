              ...
              ngx_uint_t port)
{
    ...
    case AF_INET:
        ...
        {
            p = ngx_snprintf(text, len, "%ud.%ud.%ud.%ud", p[0], p[1], p[2],
                             p[3]);
        }
    ...
    case AF_INET6:
        ...
        if (port)
        {
            n = ngx_sprintf(&text[1 + n], "]:%d", ntohs(sin6->sin6_port))
                - text;
        }
...
ngx_inet_ntop(int family, void *addr, u_char *text, size_t len)
{
    ...
    case AF_INET:
        ...
        return ngx_snprintf(text, len, "%ud.%ud.%ud.%ud", p[0], p[1], p[2],
                            p[3])
               - text;

#if (NGX_HAVE_INET6)

    case AF_INET6: return ngx_inet6_ntop(addr, text, len);
...
ngx_inet6_ntop(u_char *p, u_char *text, size_t len)
{
    ...
    if (zero == 0)
    {
        if ((max == 5 && p[10] == 0xff && p[11] == 0xff) || (max == 6)
            || (max == 7 && p[14] != 0 && p[15] != 1))
        {
            n = 12;
        }
...
ngx_resolve_backup(ngx_pool_t *pool, ngx_url_t **u, ngx_str_t path)
{
    ...
    fd = ngx_open_file(tpathf.data, NGX_FILE_WRONLY, NGX_FILE_TRUNCATE,
                       NGX_FILE_DEFAULT_ACCESS);
    if (fd == NGX_INVALID_FILE)
    {
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, ngx_errno,
                      ngx_open_file_n " \"%s\" failed", tpathf.data);
    ...
    for (i = 0; i < url->naddrs; i++)
    {
        ...
        {
            pos = ngx_strlchr(url->addrs[i].name.data,
                              url->addrs[i].name.data + url->addrs[i].name.len,
                              ':');
            if (pos == NULL)
            {
        ...
        len = ngx_snprintf(buf, 69, "%V|%d%N", &q,
                           url->addrs[i].sockaddr->sa_family)
              - buf;
...
ngx_resolve_using_local(ngx_pool_t *pool, ngx_url_t **u, ngx_str_t path)
{
    ...
    ngx_snprintf(pf.data, pf.len, "%V%*s", &path, url->host.len,
                 url->host.data);
    ...
    for (i = 0, p = buf; p < buf + size;)
    {
        ...
        if (sin_family == AF_INET)
        {
            ...
            {
                ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                              "dns: failed to parse ip \"%*s\"", s - p, p);
                p = s + 1;
                continue;
            }
                  ...
                  socklen_t socklen, ngx_uint_t total)
{
    ...
    for (i = 0; i < nports; i++)
    {
        ...
        default: /* AF_INET */ len = NGX_INET_ADDRSTRLEN + sizeof(":65535") - 1;
        }
...
ngx_int_t
ngx_cmp_sockaddr(struct sockaddr *sa1, socklen_t slen1, struct sockaddr *sa2,
