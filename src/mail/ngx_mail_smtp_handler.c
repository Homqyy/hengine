...
ngx_mail_smtp_resolve_addr_handler(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state)
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                      "%V could not be resolved (%i: %s)", &c->addr_text,
...
ngx_mail_smtp_invalid_pipelining(ngx_event_t *rev)
{
    ...
    else
    {
        ...
        if (ngx_mail_smtp_discard_command(
                s, c, "client was rejected before greeting: \"%V\"")
            != NGX_OK)
        {
            return;
        }
...
ngx_mail_smtp_auth_state(ngx_event_t *rev)
{
    ...
    if (rc == NGX_OK)
    {
        ...
        case ngx_smtp_start:
            ...
            {
            case NGX_SMTP_HELO:
            case NGX_SMTP_EHLO: rc = ngx_mail_smtp_helo(s, c); break;

            case NGX_SMTP_AUTH: rc = ngx_mail_smtp_auth(s, c); break;
            ...
            case NGX_SMTP_MAIL: rc = ngx_mail_smtp_mail(s, c); break;

            case NGX_SMTP_RCPT: rc = ngx_mail_smtp_rcpt(s, c); break;

            case NGX_SMTP_RSET: rc = ngx_mail_smtp_rset(s, c); break;

            case NGX_SMTP_NOOP: break;
        ...
        case ngx_smtp_auth_plain: rc = ngx_mail_auth_plain(s, c, 0); break;

        case ngx_smtp_auth_cram_md5: rc = ngx_mail_auth_cram_md5(s, c); break;
