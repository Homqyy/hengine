...
ngx_mail_imap_auth_state(ngx_event_t *rev)
{
    ...
    if (rc == NGX_OK)
    {
        ...
        case ngx_imap_start:
            ...
            switch (s->command)
            {
            case NGX_IMAP_LOGIN: rc = ngx_mail_imap_login(s, c); break;
            ...
            case NGX_IMAP_NOOP: break;

            case NGX_IMAP_STARTTLS: rc = ngx_mail_imap_starttls(s, c); break;
        ...
        case ngx_imap_auth_plain: rc = ngx_mail_auth_plain(s, c, 0); break;

        case ngx_imap_auth_cram_md5: rc = ngx_mail_auth_cram_md5(s, c); break;
...
#else
    ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "imap login:\"%V\"",
                   &s->login);
#endif
