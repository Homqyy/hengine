...
static ngx_str_t ngx_mail_smtp_auth_methods_names[] = {
    ngx_string("PLAIN"),    ngx_string("LOGIN"),    ngx_null_string, /* APOP */
...
static ngx_command_t ngx_mail_smtp_commands[] = {
    ...
    {ngx_string("smtp_client_buffer"),
     ...
     offsetof(ngx_mail_smtp_srv_conf_t, client_buffer_size), NULL},

    {ngx_string("smtp_greeting_delay"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_smtp_srv_conf_t, greeting_delay), NULL},

    {ngx_string("smtp_capabilities"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_1MORE,
     ngx_mail_capabilities, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_smtp_srv_conf_t, capabilities), NULL},
...
ngx_mail_smtp_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_size_value(conf->client_buffer_size,
                              prev->client_buffer_size, (size_t)ngx_pagesize);
    ...
    *p   = LF;
    ...
    for (m = NGX_MAIL_AUTH_PLAIN_ENABLED, i = 0;
         m <= NGX_MAIL_AUTH_EXTERNAL_ENABLED; m <<= 1, i++)
    {
    ...
    *p = '-';
    ...
    if (last < auth)
    {
        p = conf->starttls_only_capability.data + (last - conf->capability.data)
            + 3;
        *p = '-';
    }
