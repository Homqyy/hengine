        ...
        if (rc == NGX_OK)
        {
                       ...
                       == 0)
            {
                ...
                case NGX_MAIL_IMAP_PROTOCOL:
                    size =
                        s->tag.len + sizeof("NO ") - 1 + len + sizeof(CRLF) - 1;
                    break;
                ...
                default: /* NGX_MAIL_SMTP_PROTOCOL */ break;
                }
                       ...
                       == 0)
            {
                ...
                s->passwd.data =
                    ngx_pnalloc(s->connection->pool, s->passwd.len);
                       ...
                       == 0)
            {
                ...
                ctx->errcode.data =
                    ngx_pnalloc(s->connection->pool, ctx->errcode.len);
    ...
    for (p = ctx->response->pos; p < ctx->response->last; p++)
    {
        ...
        case sw_start:
            ...
            case CR:
                ...
                break;
            case LF: ctx->header_end = p; goto header_done;
            default:
                state                  = sw_name;
        ...
        case sw_value:
            ...
            case CR:
                ...
                state           = sw_almost_done;
                break;
            case LF: ctx->header_end = p; goto done;
            }
            break;
                                  ...
                                  ngx_mail_auth_http_conf_t *ahcf)
{
    ...
    len = sizeof("GET ") - 1 + ahcf->uri.len + sizeof(" HTTP/1.0" CRLF) - 1
          ...
          - 1 + sizeof("Client-IP: ") - 1 + s->connection->addr_text.len
          + sizeof(CRLF) - 1 + sizeof("Client-Host: ") - 1 + s->host.len
          + sizeof(CRLF) - 1 + sizeof("Auth-SMTP-Helo: ") - 1 + s->smtp_helo.len
          + sizeof(CRLF) - 1 + sizeof("Auth-SMTP-From: ") - 1 + s->smtp_from.len
          + sizeof(CRLF) - 1 + sizeof("Auth-SMTP-To: ") - 1 + s->smtp_to.len
          + sizeof(CRLF) - 1
#if (NGX_MAIL_SSL)
          + sizeof("Auth-SSL: on" CRLF) - 1 + sizeof("Auth-SSL-Verify: ") - 1
          ...
          + serial.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Fingerprint: ") - 1
          + fingerprint.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Cert: ") - 1
          + cert.len + sizeof(CRLF) - 1
#endif
    ...
    b->last =
        ngx_cpymem(b->last, " HTTP/1.0" CRLF, sizeof(" HTTP/1.0" CRLF) - 1);
    ...
    *b->last++ = LF;
    ...
    b->last =
        ngx_cpymem(b->last, ngx_mail_auth_http_method[s->auth_method].data,
    ...
    *b->last++ = LF;
    ...
    {
        b->last =
            ngx_cpymem(b->last, "Client-Host: ", sizeof("Client-Host: ") - 1);
    ...
    if (s->auth_method == NGX_MAIL_AUTH_NONE)
    {
        ...
        b->last = ngx_cpymem(
            b->last, "Auth-SMTP-Helo: ", sizeof("Auth-SMTP-Helo: ") - 1);
        b->last    = ngx_copy(b->last, s->smtp_helo.data, s->smtp_helo.len);
        *b->last++ = CR;
        *b->last++ = LF;

        b->last = ngx_cpymem(
            b->last, "Auth-SMTP-From: ", sizeof("Auth-SMTP-From: ") - 1);
    ...
    if (c->ssl)
    {
        ...
        {
            b->last = ngx_cpymem(
                b->last, "Auth-SSL-Cert: ", sizeof("Auth-SSL-Cert: ") - 1);
...
ngx_mail_auth_http(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_parse_url(cf->pool, &u) != NGX_OK)
    {
        if (u.err)
        {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in auth_http \"%V\"",
                               u.err, &u.url);
        }
