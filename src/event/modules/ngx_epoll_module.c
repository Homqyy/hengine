...
ngx_epoll_notify_init(ngx_log_t *log)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, log, 0, "notify eventfd: %d",
                   notify_fd);
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                   "epoll add event: fd:%d op:%d ev:%08XD", c->fd, op,
                   ee.events);
...
ngx_epoll_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                   "epoll del event: fd:%d op:%d ev:%08XD", c->fd, op,
                   ee.events);
...
ngx_epoll_add_async_connection(ngx_connection_t *c)
{
    ...
    ee.events = EPOLLIN | EPOLLOUT | EPOLLET | EPOLLRDHUP;
    ee.data.ptr =
        (void *)((uintptr_t)c | (c->async->async << 1) | c->async->instance);

    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "epoll add async connection: fd:%d ev:%08XD", c->async_fd,
                   ee.events);
    if (epoll_ctl(ep, EPOLL_CTL_ADD, c->async_fd, &ee) == -1)
    {
        ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno,
                      "async add conn epoll_ctl(EPOLL_CTL_ADD, %d) failed",
                      c->async_fd);
        return NGX_ERROR;
    }
...
ngx_epoll_del_async_connection(ngx_connection_t *c, ngx_uint_t flags)
{
    ...
    {
        ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno,
                      "async del conn epoll_ctl(%d, %d) failed", op,
                      c->async_fd);
        c->async_fd = -1;
        return NGX_ERROR;
    }
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "epoll timer: %M",
                   timer);
    ...
    for (i = 0; i < events; i++)
    {
        ...
        if (revents & (EPOLLERR | EPOLLHUP))
        {
            ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                           "epoll_wait() error on fd:%d ev:%04XD", c->fd,
                           revents);
...
ngx_epoll_eventfd_handler(ngx_event_t *ev)
{
    ...
    while (ready)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "io_getevents: %d",
                       events);
        ...
        if (events > 0)
        {
            ...
            for (i = 0; i < events; i++)
            {
                ngx_log_debug4(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                               "io_event: %XL %XL %L %L", event[i].data,
                               event[i].obj, event[i].res, event[i].res2);
