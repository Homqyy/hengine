...
static ngx_command_t ngx_iocp_commands[] = {
    ...
    {ngx_string("iocp_threads"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_iocp_conf_t, threads), NULL},

    {ngx_string("post_acceptex"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_iocp_conf_t, post_acceptex), NULL},

    {ngx_string("acceptex_read"), NGX_EVENT_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, 0, offsetof(ngx_iocp_conf_t, acceptex_read), NULL},
...
ngx_module_t ngx_iocp_module = {NGX_MODULE_V1,
...
ngx_os_io_t ngx_iocp_io = {ngx_overlapped_wsarecv,
                           NULL,
...
ngx_iocp_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    {
        iocp =
            CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, cf->threads);
    }
...
static ngx_thread_value_t __stdcall ngx_iocp_timer(void *data)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ngx_cycle->log, 0, "THREAD %p %p",
                   &msec, data);
...
ngx_iocp_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    case NGX_IOCP_CONNECT: ev->ready = 1;
    }

    ev->available = bytes;

    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "iocp event handler: %p",
                   ev->handler);
