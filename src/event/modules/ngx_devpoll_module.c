...
ngx_devpoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (max_changes < dpcf->changes)
    {
        ...
        change_list =
            ngx_alloc(sizeof(struct pollfd) * dpcf->changes, cycle->log);
        if (change_list == NULL)
        {
            return NGX_ERROR;
        }
        ...
        change_index =
            ngx_alloc(sizeof(ngx_event_t *) * dpcf->changes, cycle->log);
    ...
    if (nevents < dpcf->events)
    {
        ...
        event_list =
            ngx_alloc(sizeof(struct pollfd) * dpcf->events, cycle->log);
        if (event_list == NULL)
        {
            return NGX_ERROR;
        }
                           ...
                           ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++)
    {
        ...
        if (c == NULL || c->fd == -1)
        {
            ...
            case -1:
                ngx_log_error(
                    NGX_LOG_ALERT, cycle->log, ngx_errno,
                    "ioctl(DP_ISPOLLED) failed for socket %d, event %04Xd", fd,
                    revents);
                break;
