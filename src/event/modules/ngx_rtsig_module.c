...
static ngx_int_t ngx_rtsig_del_connection(ngx_connection_t *c,
                                          ngx_uint_t        flags);
static ngx_int_t ngx_rtsig_process_events(ngx_cycle_t *cycle, ngx_msec_t timer,
                                          ngx_uint_t flags);
...
static char *ngx_check_ngx_overflow_threshold_bounds(ngx_conf_t *cf, void *post,
                                                     void *data);
...
static ngx_command_t ngx_rtsig_commands[] = {
    ...
    {ngx_string("rtsig_signo"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_rtsig_conf_t, signo), NULL},

    {ngx_string("rtsig_overflow_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_rtsig_conf_t, overflow_events),
     NULL},

    {ngx_string("rtsig_overflow_test"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_rtsig_conf_t, overflow_test), NULL},

    {ngx_string("rtsig_overflow_threshold"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_rtsig_conf_t, overflow_threshold),
...
ngx_module_t ngx_rtsig_module = {NGX_MODULE_V1,
...
ngx_rtsig_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    overflow_list =
        ngx_alloc(sizeof(struct pollfd) * rtscf->overflow_events, cycle->log);
    ...
    ngx_event_flags =
        NGX_USE_RTSIG_EVENT | NGX_USE_GREEDY_EVENT | NGX_USE_FD_EVENT;
...
ngx_rtsig_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "rtsig timer: %M",
                   timer);
    ...
    if (signo == -1)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, err, "rtsig signo:%d",
                       signo);
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "rtsig signo:%d fd:%d band:%04Xd", signo, si.si_fd,
                   si.si_band);
                           ...
                           ngx_uint_t flags)
{
    ...
    for (;;)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (rev->active && !rev->closed && rev->handler
            ...
            if (wev->active && !wev->closed && wev->handler
                && (overflow_list[i].revents
            ...
            if (ngx_linux_rtsig_max)
            {
                ...
                if (rtsig_max / (int)rtscf->overflow_threshold < rtsig_nr)
                {
                    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                                   "rtsig queue state: %d/%d", rtsig_nr,
                                   rtsig_max);
