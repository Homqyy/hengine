...
ngx_event_acceptex(ngx_event_t *rev)
{
    ...
    if (setsockopt(c->fd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *)&ls->fd,
                   sizeof(ngx_socket_t))
        == -1)
    {
    ...
    ngx_getacceptexsockaddrs(c->buffer->pos, ls->post_accept_buffer_size,
    ...
    if (ls->addr_ntop)
    {
        ...
        c->addr_text.len =
            ngx_sock_ntop(c->sockaddr, c->socklen, c->addr_text.data,
...
ngx_event_post_acceptex(ngx_listening_t *ls, ngx_uint_t n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, &ls->log, 0, ngx_socket_n " s:%d",
                       s);
        ...
        *log   = ls->log;
        ...
        if (ngx_acceptex(ls->fd, s, c->buffer->pos, ls->post_accept_buffer_size,
                         ls->socklen + 16, ls->socklen + 16, &rcvd,
