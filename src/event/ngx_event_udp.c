...
#if (NGX_DEBUG)
        {
            ...
            if (log->log_level & NGX_LOG_DEBUG_EVENT)
            {
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_EVENT, log, 0,
                               "*%uA recvmsg: %V fd:%d n:%z", c->number, &addr,
                               c->fd, n);
            }
        }
#endif
...
void
ngx_udp_rbtree_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node,
                            ngx_rbtree_node_t *sentinel)
{
    ...
    for (;;)
    {
        ...
        { /* node->key == temp->key */
            ...
            rc = ngx_cmp_sockaddr(c->sockaddr, c->socklen, ct->sockaddr,
                                  ct->socklen, 1);
