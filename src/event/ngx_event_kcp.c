...
ngx_event_kcp_process_connections(ngx_cycle_t *cycle)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                       "kcp timer del: %d:%M", kcp->number, kcp->timer.key);
...
ngx_event_kcp_update_timer(ngx_log_t *log, ngx_kcp_t *kcp)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, log, 0, "kcp timer update: %d:%M",
                   kcp->number, kcp->timer.key);
...
ngx_event_kcp_add_timer(ngx_log_t *log, ngx_kcp_t *kcp)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, log, 0, "kcp timer add: %d:%M",
                   kcp->number, kcp->timer.key);
}
...
ngx_event_kcp_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                   "kcp handler: %d, close: %d, error: %d, waiting_read: %d, "
                   "waiting_write: %d",
                   kcp->number, kcp->close, kcp->error, kcp->waiting_read,
                   kcp->waiting_write);
