...
ngx_http_proxy_create_key(ngx_http_request_t *r)
{
    ...
    *key = ctx->vars.key_start;
    ...
    {
        escape = 2
                 * ngx_escape_uri(NULL, r->uri.data + loc_len,
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    {
        loc_len =
            (r->valid_location && ctx->vars.uri.len) ? plcf->location.len : 0;
    ...
    *b->last++ = ' ';
    ...
    else
    {
        ...
        {
            b->last =
                ngx_copy(b->last, r->uri.data + loc_len, r->uri.len - loc_len);
        }
                                       ...
                                       size_t len, ngx_http_proxy_rewrite_t *pr)
{
    ...
    if (pattern.len > len
        || ngx_rstrncmp(h->value.data + prefix, pattern.data, pattern.len) != 0)
    {
        return NGX_DECLINED;
    }
...
static ngx_int_t
ngx_http_proxy_rewrite_domain_handler(ngx_http_request_t *r, ngx_table_elt_t *h,
                                      size_t prefix, size_t len,
                                      ngx_http_proxy_rewrite_t *pr)
{
    u_char   *p;
