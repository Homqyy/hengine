...
static char     *ngx_http_gunzip_merge_conf(ngx_conf_t *cf, void *parent,
                                            void *child);
...
static ngx_command_t ngx_http_gunzip_filter_commands[] = {
    ...
    {ngx_string("gunzip"),
     ...
     offsetof(ngx_http_gunzip_conf_t, enable), NULL},
                                ...
                                ngx_http_gunzip_ctx_t *ctx)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "gunzip in: %p",
                   ctx->in);
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "gunzip in_buf:%p ni:%p ai:%ud", ctx->in_buf,
                               ...
                               ngx_http_gunzip_ctx_t *ctx)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "gunzip in_buf:%p pos:%p", ctx->in_buf, ctx->in_buf->pos);
...
ngx_http_gunzip_filter_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ctx->request->connection->log, 0,
                   "gunzip alloc: n:%ud s:%ud", items, size);
