...
static ngx_command_t ngx_http_sub_filter_commands[] = {
    ...
    {ngx_string("sub_filter_once"),
     ...
     offsetof(ngx_http_sub_loc_conf_t, once), NULL},

    {ngx_string("sub_filter_last_modified"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_sub_loc_conf_t, last_modified), NULL},
...
ngx_http_sub_header_filter(ngx_http_request_t *r)
{
    ...
    if (slcf->pairs == NULL || r->headers_out.content_length_n == 0
...
ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if ((in == NULL && ctx->buf == NULL && ctx->in == NULL
         && ctx->busy == NULL))
    {
        return ngx_http_next_body_filter(r, in);
    }
    ...
    while (ctx->in || ctx->buf)
    {
        ...
        while (ctx->pos < ctx->buf->last)
        {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "parse: %i, looked: \"%V\" %p-%p", rc, &ctx->looked,
            ...
            if (ctx->sub == NULL)
            {
                ctx->sub = ngx_pcalloc(r->pool,
                                       sizeof(ngx_str_t) * ctx->matches->nelts);
                if (ctx->sub == NULL)
                {
                    return NGX_ERROR;
                }
...
ngx_http_sub_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        slcf->pairs =
            ngx_array_create(cf->pool, 1, sizeof(ngx_http_sub_pair_t));
        if (slcf->pairs == NULL)
        {
            return NGX_CONF_ERROR;
        }
