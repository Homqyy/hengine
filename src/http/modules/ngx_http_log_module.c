...
static ngx_command_t ngx_http_log_commands[] = {
    ...
    {ngx_string("access_log"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_HTTP_LIF_CONF | NGX_HTTP_LMT_CONF | NGX_CONF_1MORE,
...
ngx_http_log_handler(ngx_http_request_t *r)
{
    ...
    for (l = 0; l < lcf->logs->nelts; l++)
    {
        ...
        if (log[l].syslog_peer)
        {
            ...
            len += sizeof("<255>Jan 01 00:00:00 ") - 1 + ngx_cycle->hostname.len
                   + 1 + log[l].syslog_peer->tag.len + 2;
    ...
    alloc_line:
        ...
        if (log[l].syslog_peer)
        {
            ...
            {
                ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                              "send() to syslog has written only %z of %uz", n,
                              size);
            }
                          ...
                          u_char **name, u_char *buf, size_t len)
{
    ...
    *name                 = log.data;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http log \"%s\"",
                   log.data);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http log #%d",
                   of.fd);
                  ...
                  ngx_log_t *log)
{
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, log, 0,
                   "deflate in: ni:%p no:%p ai:%ud ao:%ud", zstream.next_in,
                   zstream.next_out, zstream.avail_in, zstream.avail_out);

    rc = deflate(&zstream, Z_FINISH);

    if (rc != Z_STREAM_END)
    {
        ngx_log_error(NGX_LOG_ALERT, log, 0, "deflate(Z_FINISH) failed: %d",
                      rc);
        goto done;
    }
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, log, 0,
                   ...
                   zstream.next_in, zstream.next_out, zstream.avail_in,
                   zstream.avail_out, rc);
...
ngx_http_log_gzip_alloc(void *opaque, u_int items, u_int size)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pool->log, 0, "gzip alloc: n:%ud s:%ud",
                   items, size);
...
ngx_http_log_flush(ngx_open_file_t *file, ngx_log_t *log)
{
    ...
    if (n == -1)
    {
        ngx_log_error(NGX_LOG_ALERT, log, ngx_errno,
                      ngx_write_fd_n " to \"%s\" failed", file->name.data);
    }
...
ngx_http_log_set_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    else if (ngx_strncmp(value[1].data, "pipe:", 5) == 0)
    {
        ...
        ngx_open_pipe_t *pipe_conf =
            ngx_conf_open_pipe(cf->cycle, &value[1], "w");
        if (pipe_conf == NULL)
        {
            return NGX_CONF_ERROR;
        }
...
process_formats:
    ...
    if (log->format == NULL)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unknown log format \"%V\"",
                           &name);
        return NGX_CONF_ERROR;
    }
    ...
    if (size)
    {
        ...
        if (log->file->data)
        {
            ...
            if (buffer->last - buffer->start != size || buffer->flush != flush
                || buffer->gzip != gzip)
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
