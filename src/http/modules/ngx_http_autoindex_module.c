...
static char     *ngx_http_autoindex_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                                   void *child);
...
static ngx_command_t ngx_http_autoindex_commands[] = {
    ...
    {ngx_string("autoindex"),
     ...
     offsetof(ngx_http_autoindex_loc_conf_t, enable), NULL},
    ...
    {ngx_string("autoindex_localtime"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_autoindex_loc_conf_t, localtime), NULL},

    {ngx_string("autoindex_exact_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_autoindex_loc_conf_t, exact_size), NULL},
...
ngx_http_autoindex_handler(ngx_http_request_t *r)
{
    ...
    for (;;)
    {
        ...
        if (!dir.valid_info)
        {
            ...
            {
                allocated =
                    path.len + 1 + len + 1 + NGX_HTTP_AUTOINDEX_PREALLOCATE;
...
ngx_http_autoindex_html(ngx_http_request_t *r, ngx_array_t *entries)
{
    ...
    len = sizeof(title) - 1 + r->uri.len + escape_html + sizeof(header) - 1
          + r->uri.len + escape_html + sizeof("</h1>") - 1
          + sizeof("<hr><pre><a href=\"../\">../</a>" CRLF) - 1
          + sizeof("</pre><hr>") - 1 + sizeof(tail) - 1;
    ...
    for (i = 0; i < entries->nelts; i++)
    {
        entry[i].escape =
            2
            * ngx_escape_uri(NULL, entry[i].name.data, entry[i].name.len,
                             NGX_ESCAPE_URI_COMPONENT);

        entry[i].escape_html =
            ngx_escape_html(NULL, entry[i].name.data, entry[i].name.len);

        if (utf8)
        {
            entry[i].utf_len =
                ngx_utf8_length(entry[i].name.data, entry[i].name.len);
        }
        else
        {
        ...
        entry_len = sizeof("<a href=\"") - 1 + entry[i].name.len
                    + entry[i].escape + 1 /* 1 is for "/" */
                    + sizeof("\">") - 1 + entry[i].name.len - entry[i].utf_len
                    + entry[i].escape_html + NGX_HTTP_AUTOINDEX_NAME_LEN
                    + sizeof("&gt;") - 2 + sizeof("</a>") - 1
                    + sizeof(" 28-Sep-1970 12:00 ") - 1 + 20 /* the file size */
                    + 2;
    ...
    for (i = 0; i < entries->nelts; i++)
    {
        ...
        {
            b->last =
                ngx_cpymem(b->last, entry[i].name.data, entry[i].name.len);
        }
        ...
        *b->last++ = '>';
        ...
        if (entry[i].name.len != len)
        {
            ...
            last    = b->last;
            b->last = ngx_utf8_cpystrn(b->last, entry[i].name.data, char_len,
                                       entry[i].name.len + 1);
                        ...
                        ngx_str_t *callback)
{
    ...
    {
        entry[i].escape =
            ngx_escape_json(NULL, entry[i].name.data, entry[i].name.len);

        entry_len = sizeof("{  }," CRLF) - 1 + sizeof("\"name\":\"\"") - 1
                    + entry[i].name.len + entry[i].escape
                    + sizeof(", \"type\":\"directory\"") - 1
                    + sizeof(", \"mtime\":\"Wed, 31 Dec 1986 10:00:00 GMT\"")
                    - 1;
...
ngx_http_autoindex_xml(ngx_http_request_t *r, ngx_array_t *entries)
{
    ...
    for (i = 0; i < entries->nelts; i++)
    {
        *b->last++ = '<';
        ...
        {
            b->last =
                ngx_cpymem(b->last, "\" size=\"", sizeof("\" size=\"") - 1);
            b->last = ngx_sprintf(b->last, "%O", entry[i].size);
        }
