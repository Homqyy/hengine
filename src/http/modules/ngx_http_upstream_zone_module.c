...
static ngx_command_t ngx_http_upstream_zone_commands[] = {
    ...
    {ngx_string("zone"), NGX_HTTP_UPS_CONF | NGX_CONF_TAKE12,
     ngx_http_upstream_zone, 0, 0, NULL},
...
ngx_http_upstream_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (!value[1].len)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid zone name \"%V\"",
                           &value[1]);
    ...
    uscf->shm_zone =
        ngx_shared_memory_add(cf, &value[1], size, &ngx_http_upstream_module);
