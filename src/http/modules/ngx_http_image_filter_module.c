...
static ngx_command_t ngx_http_image_filter_commands[] = {
    ...
    {ngx_string("image_filter"), NGX_HTTP_LOC_CONF | NGX_CONF_TAKE123,
     ngx_http_image_filter, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
    ...
    {ngx_string("image_filter_transparency"),
     ...
     offsetof(ngx_http_image_filter_conf_t, transparency), NULL},

    {ngx_string("image_filter_interlace"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_image_filter_conf_t, interlace), NULL},

    {ngx_string("image_filter_buffer"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_image_filter_conf_t, buffer_size), NULL},

#if (T_NGX_HTTP_IMAGE_FILTER)
    {ngx_string("image_filter_crop_offset"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE2,
     ngx_http_image_filter_offset, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
#endif
...
static ngx_str_t ngx_http_image_types[] = {
    ngx_string("image/jpeg"), ngx_string("image/gif"), ngx_string("image/png"),
...
ngx_http_image_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    case NGX_HTTP_IMAGE_START:
        ...
        if (ctx->type == NGX_HTTP_IMAGE_NONE)
        {
            ...
            return ngx_http_filter_finalize_request(
                r, &ngx_http_image_filter_module,
    ...
    case NGX_HTTP_IMAGE_PASS: return ngx_http_next_body_filter(r, in);
...
ngx_http_image_process(ngx_http_request_t *r)
{
    ...
    ctx->max_height =
        ngx_http_image_filter_get_value(r, conf->hcv, conf->height);
    if (ctx->max_height == 0)
    {
        return NULL;
    }

    if (rc == NGX_OK && ctx->width <= ctx->max_width
        && ctx->height <= ctx->max_height && ctx->angle == 0 && !ctx->force)
    {
...
{
    int sx, sy, dx, dy, ox, oy, ax, ay, size, colors, palette, transparent,
        sharpen, red, green, blue, t;
#if (T_NGX_HTTP_IMAGE_FILTER)
    ...
    if (!ctx->force && ctx->angle == 0 && (ngx_uint_t)sx <= ctx->max_width
...
transparent:
    ...
    if (ctx->angle)
    {
        ...
        case 270:
            ...
            gdImageCopyRotated(dst, src, ox, oy, 0, 0, dx + ax, dy + ay,
                               ctx->angle);
            gdImageDestroy(src);
    ...
    if (conf->filter == NGX_HTTP_IMAGE_CROP)
    {
        ...
        if (ox || oy)
        {
...
#if (T_NGX_HTTP_IMAGE_FILTER)
            offset_x =
                ngx_http_image_filter_get_value(r, conf->oxcv, conf->offset_x);
            offset_y =
                ngx_http_image_filter_get_value(r, conf->oycv, conf->offset_y);
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "image crop: %d x %d @ %d x %d", dx, dy, ox, oy);
...
ngx_http_image_source(ngx_http_request_t *r, ngx_http_image_filter_ctx_t *ctx)
{
    ...
    default: failed = "unknown image type"; break;
    }
    ...
    if (n == NGX_ERROR)
    {
                 ...
                 && ngx_strncmp(value->data, "top", sizeof("top") - 1) == 0)
        {
            return NGX_HTTP_IMAGE_OFFSET_TOP;
                 ...
                 && ngx_strncmp(value->data, "bottom", sizeof("bottom") - 1)
                        == 0)
        {
...
static char *
ngx_http_image_filter_sharpen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_image_filter_conf_t *imcf = conf;
...
#if (T_NGX_HTTP_IMAGE_FILTER)
static char *
ngx_http_image_filter_offset(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_image_filter_conf_t *imcf = conf;
