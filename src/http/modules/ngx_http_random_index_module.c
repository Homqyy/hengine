...
static char *ngx_http_random_index_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                                  void *child);


static ngx_command_t ngx_http_random_index_commands[] = {

    {ngx_string("random_index"), NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
...
ngx_http_random_index_handler(ngx_http_request_t *r)
{
    ...
    for (;;)
    {
        ...
        if (dir.type == 0 || ngx_de_is_link(&dir))
        {
            ...
            {
                allocated =
                    path.len + 1 + len + 1 + NGX_HTTP_RANDOM_INDEX_PREALLOCATE;
