...
static ngx_command_t ngx_http_geo_commands[] = {
    ...
    {ngx_string("geo"), NGX_HTTP_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_TAKE12,
     ngx_http_geo_block, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},
                           ...
                           uintptr_t data)
{
    ...
    {
        vv = (ngx_http_variable_value_t *)ngx_radix32tree_find(
    ...
    case AF_INET6:
        ...
        {
            vv = (ngx_http_variable_value_t *)ngx_radix128tree_find(
                ctx->u.trees.tree6, p);
        }
    ...
    *v = *vv;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http geo: %v",
                   v);
                  ...
                  ngx_addr_t *addr)
{
    ...
    {
        (void)ngx_http_get_forwarded_addr(r, addr, xfwd, NULL, ctx->proxies,
                                          ctx->proxy_recursive);
    }
                   ...
                   ngx_str_t *value)
{
    ...
    {
        ctx->high.low =
            ngx_pcalloc(ctx->pool, 0x10000 * sizeof(ngx_http_geo_range_t *));
        if (ctx->high.low == NULL)
        {
            return NGX_CONF_ERROR;
        }
                      ...
                      ngx_cidr_t *cidr, ngx_str_t *value, ngx_str_t *net)
{
    ...
    case AF_INET6:
        ...
        old = (ngx_http_variable_value_t *)ngx_radix128tree_find(
    ...
    default: /* AF_INET */
        ...
        old = (ngx_http_variable_value_t *)ngx_radix32tree_find(
                   ...
                   ngx_str_t *value)
{
    ...
    gvvn = (ngx_http_geo_variable_value_node_t *)ngx_str_rbtree_lookup(
    ...
    gvvn =
        ngx_palloc(ctx->temp_pool, sizeof(ngx_http_geo_variable_value_node_t));
    if (gvvn == NULL)
    {
        return NULL;
    }
                                 ...
                                 ngx_str_t *name)
{
    ...
    for (i = 0; i < 0x10000; i++)
    {
        ...
        {
            ranges[i] =
                (ngx_http_geo_range_t *)((u_char *)ranges[i] + (size_t)base);
        }
    }
    ...
    while ((u_char *)range < base + size)
    {
        while (range->value)
        {
            ngx_crc32_update(&crc32, (u_char *)range,
                             sizeof(ngx_http_geo_range_t));
            range->value = (ngx_http_variable_value_t *)((u_char *)range->value
                                                         + (size_t)base);
            range++;
        }
...
ngx_http_geo_create_binary_base(ngx_http_geo_conf_ctx_t *ctx)
{
    ...
    header = fm.addr;
    header->crc32 =
        ngx_crc32_long((u_char *)fm.addr + sizeof(ngx_http_geo_header_t),
