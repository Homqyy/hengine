...
ngx_http_xslt_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for (cl = in; cl; cl = cl->next)
    {
        ...
        if (cl->buf->last_buf || cl->buf->last_in_chain)
        {
            ...
            if (wellFormed)
            {
                return ngx_http_xslt_send(
                    r, ctx, ngx_http_xslt_apply_stylesheet(r, ctx));
            }
                               ...
                               ngx_http_xslt_filter_ctx_t *ctx)
{
    ...
    for (i = 0; i < conf->sheets.nelts; i++)
    {
        ...
        res =
            xsltApplyStylesheetUser(sheet[i].stylesheet, doc, ctx->params.elts,
...
ngx_http_xslt_param(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        xlcf->params =
            ngx_array_create(cf->pool, 2, sizeof(ngx_http_xslt_param_t));
        if (xlcf->params == NULL)
        {
            return NGX_CONF_ERROR;
        }
