...
static char     *ngx_http_access_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                                void *child);
...
ngx_http_access_rule(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cidr.family == AF_INET || all)
    {
        ...
        {
            alcf->rules =
                ngx_array_create(cf->pool, 4, sizeof(ngx_http_access_rule_t));
            if (alcf->rules == NULL)
            {
                return NGX_CONF_ERROR;
            }
    ...
    if (cidr.family == AF_INET6 || all)
    {
        ...
        {
            alcf->rules6 =
                ngx_array_create(cf->pool, 4, sizeof(ngx_http_access_rule6_t));
            if (alcf->rules6 == NULL)
            {
                return NGX_CONF_ERROR;
            }
    ...
    if (cidr.family == AF_UNIX || all)
    {
        ...
        {
            alcf->rules_un = ngx_array_create(
                cf->pool, 1, sizeof(ngx_http_access_rule_un_t));
            if (alcf->rules_un == NULL)
            {
                return NGX_CONF_ERROR;
            }
