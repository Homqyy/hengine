...
static ngx_http_modern_browser_mask_t ngx_http_modern_browser_masks[] = {
    ...
    {"msie", sizeof("Mozilla/4.0 (compatible; ") - 1, sizeof("MSIE ") - 1,
     "MSIE "},
    ...
    {"konqueror", sizeof("Mozilla/5.0 (compatible; ") - 1,
     sizeof("Konqueror/") - 1, "Konqueror/"},
...
static ngx_http_variable_t ngx_http_browser_vars[] = {
    ...
    {ngx_string("msie"), NULL, ngx_http_msie_variable, 0,
...
ngx_http_browser(ngx_http_request_t *r, ngx_http_browser_conf_t *cf)
{
    ...
    if (cf->modern_browsers)
    {
        ...
        for (i = 0; i < cf->modern_browsers->nelts; i++)
        {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "version: \"%ui\" \"%ui\"", modern[i].version,
                           version);
...
ngx_http_modern_browser(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        bcf->modern_browsers =
            ngx_array_create(cf->pool, 5, sizeof(ngx_http_modern_browser_t));
        if (bcf->modern_browsers == NULL)
        {
            return NGX_CONF_ERROR;
        }
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unknown browser name \"%V\"",
                       &value[1]);
...
ngx_http_modern_browser_value(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    bcf->modern_browser_value =
        ngx_palloc(cf->pool, sizeof(ngx_http_variable_value_t));
