...
ngx_http_fastcgi_create_request(ngx_http_request_t *r)
{
    ...
    if (flcf->upstream.pass_request_headers)
    {
        ...
        for (i = 0; /* void */; i++)
        {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "fastcgi param: \"%*s: %*s\"", key_len,
                           b->last - (key_len + val_len), val_len,
                           b->last - val_len);
        next:
        ...
        while (body)
        {
            ...
            do
            {
                ...
                b = ngx_create_temp_buf(
                    r->pool, sizeof(ngx_http_fastcgi_header_t) + padding);
                if (b == NULL)
                {
                    return NGX_ERROR;
                }
    ...
    while (in)
    {
        ...
        do
        {
            ...
            if (b->start == NULL)
            {
                ...
                b->start =
                    ngx_palloc(r->pool, sizeof(ngx_http_fastcgi_header_t) + 7);
                if (b->start == NULL)
                {
                    return NGX_ERROR;
                }
...
ngx_http_fastcgi_process_header(ngx_http_request_t *r)
{
    ...
    for (;;)
    {
        ...
        if (f->type == NGX_HTTP_FASTCGI_STDERR)
        {
            if (f->length)
            {
                ...
                if (flcf->catch_stderr)
                {
                    ...
                    {
                        if (ngx_strnstr(msg, (char *)pattern[i].data, p - msg)
                            != NULL)
                        {
                            return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                        }
                ...
                if (u->buffer.pos == u->buffer.last)
                {
                    if (!f->fastcgi_stdout)
                    {
                        ...
                        {
                            u->buffer.pos =
                                u->buffer.start + r->cache->header_start;
                        }
                        else
                        {
        ...
        for (;;)
        {
            ...
            if (rc == NGX_OK)
            {
                ...
                else
                {
                    ...
                    h->key.data =
                        ngx_pnalloc(r->pool, h->key.len + 1 + h->value.len + 1
                                                 + h->key.len);
                    if (h->key.data == NULL)
                    {
                        h->hash = 0;
        ...
        {
            f->split_parts = ngx_array_create(
                r->pool, 1, sizeof(ngx_http_fastcgi_split_part_t));
            if (f->split_parts == NULL)
            {
                return NGX_ERROR;
            }
        ...
        if (f->type == NGX_HTTP_FASTCGI_STDERR)
        {
            if (f->length)
            {
                ...
                ngx_log_error(NGX_LOG_ERR, p->log, 0,
                              "FastCGI sent in stderr: \"%*s\"", m + 1 - msg,
                              msg);
            }
            else
            {
                                ...
                                ngx_http_fastcgi_ctx_t *f)
{
    ...
    for (p = f->pos; p < f->last; p++)
    {
        ...
        case ngx_http_fastcgi_st_type:
            ...
            case NGX_HTTP_FASTCGI_END_REQUEST: f->type = (ngx_uint_t)ch; break;
            default:
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
...
ngx_http_fastcgi_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->upstream.cache_lock, prev->upstream.cache_lock,
                         0);
    ...
    if (ngx_http_upstream_hide_headers_hash(
            cf, &conf->upstream, &prev->upstream, ngx_http_fastcgi_hide_headers,
            &hash)
        != NGX_OK)
    {
        return NGX_CONF_ERROR;
