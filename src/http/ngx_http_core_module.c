...
static ngx_command_t ngx_http_core_commands[] = {
    ...
    {ngx_string("client_body_buffers"),
     ...
     offsetof(ngx_http_core_loc_conf_t, client_body_buffers), NULL},

    {ngx_string("client_body_postpone_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_core_loc_conf_t, client_body_postpone_size), NULL},
#endif
    ...
    {ngx_string("retry_cached_connection"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_core_loc_conf_t, retry_cached_connection), NULL},
#endif
    ...
    {ngx_string("request_time_cache"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_core_loc_conf_t, request_time_cache), NULL},
#endif
    ...
    {ngx_string("server_tag"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_http_set_server_tag, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
#endif
    ...
    {ngx_string("resolver_file"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_http_core_resolver_file, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
#endif
    ...
    {ngx_string("server_admin"),
     ...
     offsetof(ngx_http_core_srv_conf_t, server_admin), NULL},

    {ngx_string("server_info"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_core_loc_conf_t, server_info), NULL},
#endif
                                ...
                                ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http cl:%O max:%O", r->headers_in.content_length_n,
    ...
    if (cscf->named_locations)
    {
        for (clcfp = cscf->named_locations; *clcfp; clcfp++)
        {
            ...
            ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "using location: %V \"%V?%V\"", &r->uri, &r->uri,
                           &r->args);
            r->loc_conf = (*clcfp)->loc_conf;
            return NGX_OK;
        }
...
ngx_http_set_content_type(ngx_http_request_t *r)
{
    ...
    if (r->exten.len)
    {
        ...
        type =
            ngx_hash_find(&clcf->types_hash, hash, r->exten.data, r->exten.len);
...
ngx_int_t
ngx_http_send_response(ngx_http_request_t *r, ngx_uint_t status, ngx_str_t *ct,
...
ngx_http_named_location(ngx_http_request_t *r, ngx_str_t *name)
{
    ...
    if (cscf->named_locations)
    {
        for (clcfp = cscf->named_locations; *clcfp; clcfp++)
        {
            ...
            ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "using location: %V \"%V?%V\"", name, &r->uri,
                           &r->args);
                            ...
                            ngx_array_t *proxies, int recursive)
{
    ...
    if (headers == NULL)
    {
        return ngx_http_get_forwarded_addr_internal(
            r, addr, value->data, value->len, proxies, recursive);
    }
...
ngx_http_core_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_str_value(conf->server_tag_header, prev->server_tag_header,
                             "");
#endif
    ...
    ngx_conf_merge_bufs_value(conf->client_body_buffers,
                              prev->client_body_buffers, 16, ngx_pagesize);

    ngx_conf_merge_size_value(conf->client_body_postpone_size,
                              prev->client_body_postpone_size, 64 * 1024);
                      ...
                      * conf->client_body_buffers.size)))
    {
        ...
        conf->client_body_buffers.num =
            1 + (conf->client_max_body_size / conf->client_body_buffers.size);
    }
        ...
        > (conf->client_body_buffers.num * conf->client_body_buffers.size))
    {
        ...
        conf->client_body_buffers.num = 1
                                        + (conf->client_body_postpone_size
                                           / conf->client_body_buffers.size);
    }
...
#if (T_NGX_RET_CACHE)
    ngx_conf_merge_value(conf->request_time_cache, prev->request_time_cache, 1);
#endif
    ...
    ngx_conf_merge_ptr_value(conf->open_file_cache, prev->open_file_cache,
                             NULL);
...
ngx_http_gzip_disable(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        clcf->gzip_disable =
            ngx_array_create(cf->pool, 2, sizeof(ngx_regex_elt_t));
...
ngx_http_disable_symlinks(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "duplicate parameters \"%V %V\"", &value[1],
                           &value[2]);
        return NGX_CONF_ERROR;
    }
...
ngx_http_set_server_tag(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    else
    {
        ...
        ccf->server_tag_header.len =
            value[1].len + sizeof("Server: ") - 1 + sizeof(CRLF) - 1;
        ccf->server_tag_header.data =
            ngx_palloc(cf->pool, ccf->server_tag_header.len);
