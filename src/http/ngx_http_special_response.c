...
static u_char ngx_http_server_info_head[] =
    ...
    "Please report this message and include the following information to "
    "us.<br/>" CRLF "Thank you very much!</p>" CRLF "<table>" CRLF "<tr>" CRLF
    "<td>URL:</td>" CRLF "<td>";


static u_char ngx_http_server_info_server[] =
    "</td>" CRLF "</tr>" CRLF "<tr>" CRLF "<td>Server:</td>" CRLF "<td>";


static u_char ngx_http_server_info_admin[] =
    "</td>" CRLF "</tr>" CRLF "<tr>" CRLF "<td>Admin:</td>" CRLF "<td>";


static u_char ngx_http_server_info_date[] =
    "</td>" CRLF "</tr>" CRLF "<tr>" CRLF "<td>Date:</td>" CRLF "<td>";
...
static u_char ngx_http_error_banner[] = "<hr/>Powered by " TENGINE;
...
static u_char ngx_http_error_powered_by[] = "<hr/>Powered by ";
#endif
...
ngx_http_special_response_handler(ngx_http_request_t *r, ngx_int_t error)
{
    ...
    if (r->keepalive)
    {
        ...
        case NGX_HTTP_NOT_IMPLEMENTED: r->keepalive = 0;
        }
    }
    ...
    if (r->lingering_close)
    {
        ...
        case NGX_HTTP_TO_HTTPS:
        case NGX_HTTPS_CERT_ERROR:
        case NGX_HTTPS_NO_CERT: r->lingering_close = 0;
        }
    }
                                 ...
                                 ngx_int_t error)
{
    ...
    {
    case NGX_OK:
    case NGX_DONE: return NGX_ERROR;
                               ...
                               ngx_http_core_loc_conf_t *clcf, ngx_uint_t err)
{
    ...
    if (ngx_http_error_pages[err].len)
    {
        ...
        r->headers_out.content_length_n =
            sizeof(ngx_http_error_doctype) - 1 + ngx_http_error_pages[err].len
            + (ib ? (ib->last - ib->pos) : 0) + sizeof(ngx_http_error_tail) - 1;

        if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_ON)
        {
            r->headers_out.content_length_n +=
                clcf->server_tokens ? sizeof(ngx_http_error_full_banner) - 1
    ...
    else if (clcf->server_tag_type == NGX_HTTP_SERVER_TAG_CUSTOMIZED)
    {
        ...
        b->last =
            ngx_http_error_powered_by + sizeof(ngx_http_error_powered_by) - 1;
...
ngx_http_set_server_info(ngx_http_request_t *r)
{
    ...
    size = sizeof(ngx_http_server_info_head) - 1 + scheme.len + host->len
           + ehost + port.len + r->unparsed_uri.len + euri
           + sizeof(ngx_http_server_info_server) - 1 + ngx_cycle->hostname.len
           + sizeof(ngx_http_server_info_date) - 1 + ngx_cached_err_log_time.len
    ...
    {
        b->last =
            ngx_cpymem(b->last, r->unparsed_uri.data, r->unparsed_uri.len);
    }
    else
    {
    ...
    b->last =
        ngx_cpymem(b->last, ngx_cycle->hostname.data, ngx_cycle->hostname.len);
