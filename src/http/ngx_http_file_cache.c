...
ngx_http_file_cache_lock(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http file cache lock u:%d wt:%M", c->updating,
                   c->wait_time);
...
ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c)
{
    ...
    if (c->valid_sec < now)
    {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "http file cache expired: %i %T %T", rc, c->valid_sec,
                       now);
...
ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug6(
            ...
            fcn->count, fcn->exists, fcn->key[0], fcn->key[1], fcn->key[2],
            fcn->key[3]);
...
ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++)
    {
        ...
        if (ngx_strncmp(value[i].data, "manager_threshold=", 18) == 0)
        {
            ...
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                                   "invalid manager_threshold value \"%V\"",
                                   &value[i]);
                return NGX_CONF_ERROR;
            }
