...
static char     *ngx_http_copy_filter_merge_conf(ngx_conf_t *cf, void *parent,
                                                 void *child);
...
ngx_http_copy_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http copy filter: \"%V?%V\"",
                   &r->uri, &r->args);
    ...
    if (ctx == NULL)
    {
        ...
        ctx->need_in_memory =
            r->main_filter_need_in_memory || r->filter_need_in_memory;
        ...
        ctx->output_filter =
            (ngx_output_chain_filter_pt)ngx_http_next_body_filter;
        ctx->filter_ctx = r;
...
ngx_http_copy_aio_event_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http aio: \"%V?%V\"",
                   &r->uri, &r->args);
...
ngx_http_copy_thread_event_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http thread: \"%V?%V\"",
                   &r->uri, &r->args);
