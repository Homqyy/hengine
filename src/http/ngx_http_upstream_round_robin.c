    ...
    if (us->servers)
    {
        ...
        for (i = 0; i < us->servers->nelts; i++)
        {
            ...
            for (j = 0; j < server[i].naddrs; j++)
            {
                ...
                if (!server[i].down)
                {
                    peer[n].check_index = ngx_http_upstream_check_add_peer(
                        cf, us, &server[i].addrs[j]);
                }
                else
                {
                                          ...
                                          ngx_http_upstream_resolved_t *ur)
{
    ...
    peer =
        ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_rr_peer_t) * ur->naddrs);
    if (peer == NULL)
    {
        return NGX_ERROR;
    }
...
ngx_http_upstream_get_peer(ngx_http_upstream_rr_peer_data_t *rrp)
{
    ...
    flag = 1;
    for (i   = rrp->peers->init_number; i != rrp->peers->init_number || flag;
