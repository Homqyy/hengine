...
#define NGX_HTTP_V2_STATIC_TABLE_ENTRIES \
    (sizeof(ngx_http_v2_static_table) / sizeof(ngx_http_v2_header_t))
                               ...
                               ngx_uint_t name_only)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 get indexed %s: %ui", name_only ? "name" : "header",
                   index);
                       ...
                       ngx_http_v2_header_t     *header)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 table add: \"%V: %V\"", &header->name,
                   &header->value);
    ...
    if (h2c->hpack.entries == NULL)
    {
        ...
        h2c->hpack.entries =
            ngx_palloc(h2c->connection->pool,
                       sizeof(ngx_http_v2_header_t *) * h2c->hpack.allocated);
        if (h2c->hpack.entries == NULL)
        {
            return NGX_ERROR;
        }
    ...
    {
        h2c->hpack.pos =
            ngx_cpymem(h2c->hpack.pos, header->name.data, header->name.len);
    }
    else
    {
        ngx_memcpy(h2c->hpack.pos, header->name.data, avail);
        h2c->hpack.pos =
            ngx_cpymem(h2c->hpack.storage, header->name.data + avail,
    ...
    {
        h2c->hpack.pos =
            ngx_cpymem(h2c->hpack.pos, header->value.data, header->value.len);
    }
    else
    {
        ngx_memcpy(h2c->hpack.pos, header->value.data, avail);
        h2c->hpack.pos =
            ngx_cpymem(h2c->hpack.storage, header->value.data + avail,
...
ngx_http_v2_table_account(ngx_http_v2_connection_t *h2c, size_t size)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 table account: %uz free:%uz", size, h2c->hpack.free);
...
ngx_http_v2_table_size(ngx_http_v2_connection_t *h2c, size_t size)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                   "http2 new hpack table size: %uz was:%uz", size,
                   h2c->hpack.size);
