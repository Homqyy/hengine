...
ngx_http_v2_init(ngx_event_t *rev)
{
    ...
    {
        h2mcf->recv_buffer =
            ngx_palloc(ngx_cycle->pool, h2mcf->recv_buffer_size);
        if (h2mcf->recv_buffer == NULL)
        {
    ...
    h2c->streams_index = ngx_pcalloc(
        c->pool, ngx_http_v2_index_size(h2scf) * sizeof(ngx_http_v2_node_t *));
...
ngx_http_v2_state_data(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    {
        if (ngx_http_v2_send_window_update(
                h2c, 0, NGX_HTTP_V2_MAX_WINDOW - h2c->recv_window)
            == NGX_ERROR)
        {
            return ngx_http_v2_connection_error(h2c,
                               ...
                               u_char *end)
{
    ...
    if (value < 0)
    {
        if (value == NGX_AGAIN)
        {
            return ngx_http_v2_state_headers_save(
                h2c, pos, end, ngx_http_v2_state_header_block);
        }
                             ...
                             u_char *end)
{
    ...
    if (ngx_http_v2_huff_decode(
            &h2c->state.field_state, pos, size, &h2c->state.field_end,
            h2c->state.field_rest == 0, h2c->connection->log)
        != NGX_OK)
    {
        ngx_log_error(NGX_LOG_INFO, h2c->connection->log, 0,
                                  ...
                                  u_char *end)
{
    ...
    if (!(h2c->state.flags & NGX_HTTP_V2_END_HEADERS_FLAG))
    {
        return ngx_http_v2_handle_continuation(
            h2c, pos, end, ngx_http_v2_state_header_complete);
    }
        ...
        case NGX_HTTP_V2_INIT_WINDOW_SIZE_SETTING:
            ...
            if (value > NGX_HTTP_V2_MAX_WINDOW)
            {
                ...
                return ngx_http_v2_connection_error(
                    h2c, NGX_HTTP_V2_FLOW_CTRL_ERROR);
            }
...
ngx_http_v2_state_ping(ngx_http_v2_connection_t *h2c, u_char *pos, u_char *end)
{
    ...
    frame =
        ngx_http_v2_get_frame(h2c, NGX_HTTP_V2_PING_SIZE,
                              NGX_HTTP_V2_PING_FRAME, NGX_HTTP_V2_ACK_FLAG, 0);
    if (frame == NULL)
    {
        return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
    }
...
static u_char *
ngx_http_v2_connection_error(ngx_http_v2_connection_t *h2c, ngx_uint_t err)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
...
ngx_http_v2_create_stream(ngx_http_v2_connection_t *h2c, ngx_uint_t push)
{
    ...
    r->header_in =
        ngx_create_temp_buf(r->pool, cscf->client_header_buffer_size);
    if (r->header_in == NULL)
    {
...
ngx_http_v2_set_dependency(ngx_http_v2_connection_t *h2c,
                           ngx_http_v2_node_t *node, ngx_uint_t depend,
