...
ngx_http_v2_header_filter(ngx_http_request_t *r)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        *pos++ = 0;
        ...
        pos = ngx_http_v2_write_name(pos, header[i].key.data, header[i].key.len,
                                     tmp);
                          ...
                          ngx_str_t *binary)
{
    ...
    *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_PATH_INDEX);
    ...
    for (i = 0; i < NGX_HTTP_V2_PUSH_HEADERS; i++)
    {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                       "http2 push header: \"%V: %V\"", &ph[i].name,
                       &(*h)->value);
...
ngx_http_v2_send_chain(ngx_connection_t *fc, ngx_chain_t *in, off_t limit)
{
    ...
    for (;;)
    {
        ...
        while ((off_t)rest >= size)
        {
            if (offset)
            {
                cl = ngx_http_v2_filter_get_shadow(stream, in->buf, offset,
                                                   size);
                if (cl == NULL)
                {
                    return NGX_CHAIN_ERROR;
        ...
        {
            frame =
                ngx_http_v2_filter_get_data_frame(stream, frame_size, out, cl);
            if (frame == NULL)
            {
                return NGX_CHAIN_ERROR;
            }
                                  ...
                                  ngx_chain_t *first, ngx_chain_t *last)
{
    ...
    {
        frame =
            ngx_palloc(stream->request->pool, sizeof(ngx_http_v2_out_frame_t));
        if (frame == NULL)
        {
            return NULL;
        }
