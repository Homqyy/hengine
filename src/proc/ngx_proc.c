...
static ngx_command_t ngx_procs_commands[] = {
    ...
    {ngx_string("processes"), NGX_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_NOARGS,
     ngx_procs_block, 0, 0, NULL},

    ngx_null_command};


static ngx_core_module_t ngx_procs_module_ctx = {ngx_string("procs"), NULL,
                                                 NULL};


ngx_module_t ngx_procs_module = {NGX_MODULE_V1,
...
static ngx_command_t ngx_proc_core_commands[] = {
    ...
    {ngx_string("process"),
     NGX_PROC_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_TAKE1, ngx_proc_process, 0,
     0, NULL},

    {ngx_string("count"), NGX_PROC_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot,
     NGX_PROC_CONF_OFFSET, offsetof(ngx_proc_conf_t, count), NULL},

    {ngx_string("priority"), NGX_PROC_CONF | NGX_CONF_TAKE1,
     ngx_procs_set_priority, NGX_PROC_CONF_OFFSET, 0, NULL},

    {ngx_string("delay_start"), NGX_PROC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_PROC_CONF_OFFSET,
     offsetof(ngx_proc_conf_t, delay_start), NULL},

    {ngx_string("respawn"), NGX_PROC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_PROC_CONF_OFFSET,
     offsetof(ngx_proc_conf_t, respawn), NULL},

    ngx_null_command};


static ngx_proc_module_t ngx_proc_core_module_ctx = {ngx_string("proc_core"),
...
ngx_procs_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_proc_conf_ctx_t **)conf = ctx;
    ...
    ctx->main_conf =
        ngx_pcalloc(cf->pool, ngx_procs_max_module * sizeof(void *));
    ...
    ctx->proc_conf =
        ngx_pcalloc(cf->pool, sizeof(void *) * ngx_procs_max_module);
                       ...
                       ngx_int_t priority)
{
    ...
    {
        ngx_log_error(NGX_LOG_ERR, cycle->log, 0, "process %V init event error",
                      &module->name);
        exit(2);
    }
...
ngx_procs_channel_handler(ngx_event_t *ev)
{
    ...
    for (;;)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, ev->log, 0, "process channel: %i",
                       n);
        ...
        switch (ch.command)
        {
        case NGX_CMD_QUIT: ngx_quit = 1; break;

        case NGX_CMD_TERMINATE: ngx_terminate = 1; break;

        case NGX_CMD_REOPEN: ngx_reopen = 1; break;

        case NGX_CMD_OPEN_CHANNEL:

            ngx_log_debug3(NGX_LOG_DEBUG_CORE, ev->log, 0,
                           "process got channel s:%i pid:%P fd:%d", ch.slot,
                           ch.pid, ch.fd);
        ...
        for (i = 0; i < cycle->connection_n; i++)
        {
                ...
                && !c[i].read->channel && !c[i].read->resolver)
            {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0,
                              "open socket #%d left in connection %ui", c[i].fd,
                              i);
                ngx_debug_quit = 1;
            }
        }
...
ngx_proc_process(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (flag == 0)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "no %V process module",
                           &value[1]);
        return NGX_CONF_ERROR;
    }
