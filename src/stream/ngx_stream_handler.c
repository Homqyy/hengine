...
ngx_stream_init_connection(ngx_connection_t *c)
{
    ...
    s->main_conf = ((ngx_stream_core_srv_conf_t *)addr_conf->default_server)
                       ->ctx->main_conf;
    s->srv_conf = ((ngx_stream_core_srv_conf_t *)addr_conf->default_server)
                      ->ctx->srv_conf;
#endif
...
ngx_stream_proxy_protocol_handler(ngx_event_t *rev)
{
    ...
    if (n == -1)
    {
        if (err == NGX_EAGAIN)
        {
            ...
            {
                cscf =
                    ngx_stream_get_module_srv_conf(s, ngx_stream_core_module);
