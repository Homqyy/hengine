...
static char     *ngx_stream_access_merge_srv_conf(ngx_conf_t *cf, void *parent,
                                                  void *child);
...
ngx_stream_access_rule(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cidr.family == AF_INET || all)
    {
        ...
        {
            ascf->rules =
                ngx_array_create(cf->pool, 4, sizeof(ngx_stream_access_rule_t));
            if (ascf->rules == NULL)
            {
                return NGX_CONF_ERROR;
            }
    ...
    if (cidr.family == AF_UNIX || all)
    {
        ...
        {
            ascf->rules_un = ngx_array_create(
                cf->pool, 1, sizeof(ngx_stream_access_rule_un_t));
            if (ascf->rules_un == NULL)
            {
                return NGX_CONF_ERROR;
            }
