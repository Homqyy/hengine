...
static ngx_int_t ngx_stream_upstream_addr_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_upstream_response_time_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_upstream_bytes_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
...
static ngx_stream_variable_t ngx_stream_upstream_vars[] = {
    ...
    {ngx_string("upstream_addr"), NULL, ngx_stream_upstream_addr_variable, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("upstream_bytes_sent"), NULL,
     ngx_stream_upstream_bytes_variable, 0, NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("upstream_connect_time"), NULL,
     ngx_stream_upstream_response_time_variable, 2, NGX_STREAM_VAR_NOCACHEABLE,
     0},

    {ngx_string("upstream_first_byte_time"), NULL,
     ngx_stream_upstream_response_time_variable, 1, NGX_STREAM_VAR_NOCACHEABLE,
     0},

    {ngx_string("upstream_session_time"), NULL,
     ngx_stream_upstream_response_time_variable, 0, NGX_STREAM_VAR_NOCACHEABLE,
     0},

    {ngx_string("upstream_bytes_received"), NULL,
     ngx_stream_upstream_bytes_variable, 1, NGX_STREAM_VAR_NOCACHEABLE, 0},
...
ngx_stream_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    uscf = ngx_stream_upstream_add(
        cf, &u,
        NGX_STREAM_UPSTREAM_CREATE | NGX_STREAM_UPSTREAM_WEIGHT
    ...
    uscf->servers =
        ngx_array_create(cf->pool, 4, sizeof(ngx_stream_upstream_server_t));
...
ngx_stream_upstream_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (u.no_port)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "no port in upstream \"%V\"",
                           &u.url);
        return NGX_CONF_ERROR;
    }
...
ngx_stream_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    {
        uscf->servers =
            ngx_array_create(cf->pool, 1, sizeof(ngx_stream_upstream_server_t));
        if (uscf->servers == NULL)
        {
            return NULL;
        }
