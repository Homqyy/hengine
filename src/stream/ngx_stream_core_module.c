...
static ngx_command_t ngx_stream_core_commands[] = {
    ...
    {ngx_string("variables_hash_max_size"),
     NGX_STREAM_MAIN_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot,
    ...
    {ngx_string("variables_hash_bucket_size"),
     NGX_STREAM_MAIN_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot,
     NGX_STREAM_MAIN_CONF_OFFSET,
     offsetof(ngx_stream_core_main_conf_t, variables_hash_bucket_size), NULL},
    ...
    {ngx_string("resolver_timeout"),
     ...
     offsetof(ngx_stream_core_srv_conf_t, resolver_timeout), NULL},

    {ngx_string("proxy_protocol_timeout"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_core_srv_conf_t, proxy_protocol_timeout), NULL},

    {ngx_string("tcp_nodelay"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_core_srv_conf_t, tcp_nodelay), NULL},

    {ngx_string("preread_buffer_size"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_core_srv_conf_t, preread_buffer_size), NULL},

    {ngx_string("preread_timeout"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_core_srv_conf_t, preread_timeout), NULL},

#if (NGX_STREAM_SNI)
    {ngx_string("server_name"), NGX_STREAM_SRV_CONF | NGX_CONF_1MORE,
     ngx_stream_core_server_name, NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},

    {ngx_string("server_names_hash_max_size"),
     NGX_STREAM_MAIN_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot,
     NGX_STREAM_MAIN_CONF_OFFSET,
     offsetof(ngx_stream_core_main_conf_t, server_names_hash_max_size), NULL},
                              ...
                              ngx_stream_phase_handler_t *ph)
{
    ...
    if (c->type == SOCK_STREAM && cscf->tcp_nodelay
...
ngx_stream_core_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->handler == NULL)
    {
        ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                      "no handler for server in %s:%ui", conf->file_name,
                      conf->line);
        return NGX_CONF_ERROR;
    }
    ...
    ngx_conf_merge_msec_value(conf->preread_timeout, prev->preread_timeout,
                              30000);
...
ngx_stream_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ctx->srv_conf =
        ngx_pcalloc(cf->pool, sizeof(void *) * ngx_stream_max_module);
...
ngx_stream_core_listen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 2; i < cf->args->nelts; i++)
    {
        ...
        if (ngx_strcmp(value[i].data, "ssl") == 0)
        {
            ...
            sslcf =
                ngx_stream_conf_get_module_srv_conf(cf, ngx_stream_ssl_module);
