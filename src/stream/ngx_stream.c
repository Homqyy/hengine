...
static ngx_command_t ngx_stream_commands[] = {
    ...
    {ngx_string("stream"), NGX_MAIN_CONF | NGX_CONF_BLOCK | NGX_CONF_NOARGS,
     ngx_stream_block, 0, 0, NULL},

    ngx_null_command};


static ngx_core_module_t ngx_stream_module_ctx = {ngx_string("stream"), NULL,
                                                  NULL};


ngx_module_t ngx_stream_module = {NGX_MODULE_V1,
...
ngx_stream_init_phases(ngx_conf_t *cf, ngx_stream_core_main_conf_t *cmcf)
{
    ...
    if (ngx_array_init(&cmcf->phases[NGX_STREAM_SSL_PHASE].handlers, cf->pool,
                       1, sizeof(ngx_stream_handler_pt))
        != NGX_OK)
    {
        return NGX_ERROR;
    }
    ...
    if (ngx_array_init(&cmcf->phases[NGX_STREAM_LOG_PHASE].handlers, cf->pool,
                       1, sizeof(ngx_stream_handler_pt))
        != NGX_OK)
    {
        return NGX_ERROR;
                      ...
                      ngx_stream_conf_addr_t *addr)
{
    ...
    else
    {
        ...
        for (i = 0; i < addr->servers.nelts; i++)
        {
            if (server[i] == cscf)
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "a duplicate listen");
                return NGX_ERROR;
            }
        }
    }
                               ...
                               ngx_stream_core_main_conf_t *cmcf)
{
    ...
    for (i = 0; i < NGX_STREAM_LOG_PHASE; i++)
    {
        ...
        default: checker = ngx_stream_core_generic_phase;
        }
...
found:
    ...
    for (i = 0; i < port->addrs.nelts; i++)
    {
        if (ngx_cmp_sockaddr(listen->sockaddr, listen->socklen,
                             addr[i].opt.sockaddr, addr[i].opt.socklen, 0)
            != NGX_OK)
        {
            continue;
        }
                     ...
                     ngx_stream_conf_addr_t *addr)
{
    ...
    stport->addrs =
        ngx_pcalloc(cf->pool, stport->naddrs * sizeof(ngx_stream_in_addr_t));
                      ...
                      ngx_stream_conf_addr_t *addr)
{
    ...
    stport->addrs =
        ngx_pcalloc(cf->pool, stport->naddrs * sizeof(ngx_stream_in6_addr_t));
    if (stport->addrs == NULL)
    {
        return NGX_ERROR;
    }
