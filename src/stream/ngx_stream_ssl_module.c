...
static ngx_command_t ngx_stream_ssl_commands[] = {
...
#endif
     NGX_STREAM_SRV_CONF_OFFSET, offsetof(ngx_stream_ssl_conf_t, protocols),
    ...
    {ngx_string("ssl_sni_force"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_ssl_conf_t, sni_force), NULL},
#endif
...
not_match:
    ...
    if (sscf->sni_force)
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                      "SSL sni not match, sni:%s, reject",
...
static ngx_int_t
ngx_stream_ssl_variable(ngx_stream_session_t *s, ngx_stream_variable_value_t *v,
                        uintptr_t data)
{
    ngx_ssl_variable_handler_pt handler = (ngx_ssl_variable_handler_pt)data;
...
ngx_stream_ssl_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->verify)
    {
        ...
        if (ngx_ssl_trusted_certificate(
                cf, &conf->ssl, &conf->trusted_certificate, conf->verify_depth)
            != NGX_OK)
        {
            return NGX_CONF_ERROR;
        }
...
static ngx_int_t
ngx_stream_ssl_compile_certificates(ngx_conf_t *cf, ngx_stream_ssl_conf_t *conf)
{
    ngx_str_t                         *cert, *key;
    ngx_uint_t                         i, nelts;
