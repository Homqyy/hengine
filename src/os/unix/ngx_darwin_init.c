...
static ngx_os_io_t ngx_darwin_io = {ngx_unix_recv,
                                    ngx_readv_chain,
                                    ngx_udp_unix_recv,
                                    ngx_unix_send,
                                    ngx_udp_unix_send,
...
sysctl_t sysctls[] = {
    {"hw.ncpu", &ngx_darwin_hw_ncpu, sizeof(ngx_darwin_hw_ncpu), 0},

    {"net.inet.tcp.sendspace", &ngx_darwin_net_inet_tcp_sendspace,
     sizeof(ngx_darwin_net_inet_tcp_sendspace), 0},

    {"kern.ipc.somaxconn", &ngx_darwin_kern_ipc_somaxconn,
...
ngx_os_specific_init(ngx_log_t *log)
{
    ...
    if (sysctlbyname("kern.osrelease", ngx_darwin_kern_osrelease, &size, NULL,
                     0)
        == -1)
    {
        err = ngx_errno;
