                  ...
                  ngx_pool_t *pool)
{
    ...
    if (!ev->ready)
    {
        ngx_log_error(NGX_LOG_ALERT, file->log, 0, "second aio post for \"%V\"",
                      &file->name);
        return NGX_AGAIN;
    }

    ngx_log_debug4(NGX_LOG_DEBUG_CORE, file->log, 0,
                   "aio complete:%d @%O:%uz %V", ev->complete, offset, size,
                   &file->name);
    ...
    if (n == -1)
    {
        ...
        ngx_log_error(NGX_LOG_CRIT, file->log, n, "aio_read(\"%V\") failed",
                      &file->name);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, file->log, 0, "aio_read: fd:%d %d",
                   file->fd, n);
...
ngx_file_aio_result(ngx_file_t *file, ngx_event_aio_t *aio, ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, file->log, 0, "aio_error: fd:%d %d",
                   file->fd, n);
    ...
    if (n == -1)
    {
        ...
        ngx_log_error(NGX_LOG_ALERT, file->log, err, "aio_error(\"%V\") failed",
                      &file->name);
        return NGX_ERROR;
    }
    ...
    if (n == -1)
    {
        ...
        ngx_log_error(NGX_LOG_CRIT, file->log, err, "aio_return(\"%V\") failed",
                      &file->name);
        return NGX_ERROR;
    }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, file->log, 0, "aio_return: fd:%d %d",
                   file->fd, n);
...
ngx_file_aio_event_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, ev->log, 0, "aio event handler fd:%d %V",
