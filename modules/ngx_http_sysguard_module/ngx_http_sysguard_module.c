...
static char     *ngx_http_sysguard_mem(ngx_conf_t *cf, ngx_command_t *cmd,
                                       void *conf);
static char     *ngx_http_sysguard_rt(ngx_conf_t *cf, ngx_command_t *cmd,
                                      void *conf);
...
static ngx_command_t ngx_http_sysguard_commands[] = {
    ...
    {ngx_string("sysguard"),
     ...
     offsetof(ngx_http_sysguard_conf_t, enable), NULL},

    {ngx_string("sysguard_mode"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_enum_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_sysguard_conf_t, mode), &ngx_http_sysguard_modes},

    {ngx_string("sysguard_load"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE12,
     ngx_http_sysguard_load, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("sysguard_cpu"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE123,
     ngx_http_sysguard_cpuusage, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("sysguard_mem"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE12,
     ngx_http_sysguard_mem, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("sysguard_rt"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE123,
     ngx_http_sysguard_rt, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("sysguard_interval"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_sec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_sysguard_conf_t, interval), NULL},
...
ngx_http_sysguard_update_cpuinfo(ngx_http_request_t *r)
{
    ...
    if (ngx_http_sysguard_cached_cpuinfo_exptime < ngx_time())
    {
        ...
        ngx_http_sysguard_cached_pre_cputime =
            ngx_http_sysguard_cached_cur_cputime;
        ngx_http_sysguard_cached_cur_cputime = cpuinfo;
        ngx_http_sysguard_cached_cpuinfo_exptime =
            ngx_time() + glcf->cpu_interval;
    }
...
ngx_http_sysguard_update_mem(ngx_http_request_t *r, time_t exptime)
{
    ...
    ngx_http_sysguard_cached_swapstat =
        m.totalswap == 0 ? 0 : (m.totalswap - m.freeswap) * 100 / m.totalswap;
...
{
    ngx_uint_t                   rt = 0, rt_sec = 0, rt_requests = 0;
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "sysguard update rt: i: %i, c:%i h: %i", i, ring->current,
                   head);
    ...
    for (; (i != head) && (processed < glcf->rt_period); i--, processed++)
    {
        ...
        if (node->stamp == 0 || (cur_node->stamp - node->stamp) != processed)
        {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "rt sec: %ui, rt msec:%i, rc requests: %ui", rt_sec, rt_msec,
                   rt_requests);
...
ngx_http_sysguard_update_rt_node(ngx_http_request_t *r)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "sysguard update rt node: new current: %i", ring->current);
...
{
    ngx_http_sysguard_conf_t *glcf;
    ngx_int_t load_log = 0, swap_log = 0, free_log = 0, rt_log = 0, cpu_log = 0;
    ...
    if (glcf->load != NGX_CONF_UNSET)
    {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       ...
                       glcf->load * 1.0 / 1000, &r->uri, &glcf->load_action);
    ...
    if (glcf->cpuusage != NGX_CONF_UNSET)
    {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       ...
                       ngx_http_sysguard_cached_cpuusage, glcf->cpuusage,
                       &r->uri, &glcf->cpuusage_action);
    ...
    if (glcf->swap != NGX_CONF_UNSET)
    {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       ...
                       ngx_http_sysguard_cached_swapstat, glcf->swap, &r->uri,
                       &glcf->swap_action);
    ...
    if (glcf->free != NGX_CONF_UNSET_SIZE)
    {
        ...
        if (ngx_http_sysguard_cached_free != NGX_CONF_UNSET_SIZE)
        {
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           ...
                           ngx_http_sysguard_cached_free, glcf->free, &r->uri,
                           &glcf->free_action);
...
ngx_http_sysguard_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->rt != NGX_CONF_UNSET)
    {
        /* init glcf->ring */
        conf->rt_ring =
            ngx_pcalloc(cf->pool, sizeof(ngx_http_sysguard_rt_ring_t));
        if (conf->rt_ring == NULL)
        {
            return NGX_CONF_ERROR;
        }

        conf->rt_ring->slots = ngx_pcalloc(
            cf->pool, sizeof(ngx_http_sysguard_rt_node_t) * conf->rt_period);
