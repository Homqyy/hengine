                                ...
                                ngx_http_tfs_block_cache_value_t *value)
{
    ...
    if (ctx->curr_lookup_cache == NGX_HTTP_TFS_LOCAL_BLOCK_CACHE)
    {
        ...
        {
            rc = ngx_http_tfs_local_block_cache_lookup(ctx->local_ctx, pool,
                                                       log, key, value);
    ...
    if (ctx->curr_lookup_cache == NGX_HTTP_TFS_REMOTE_BLOCK_CACHE)
    {
        ...
        {
            rc = ngx_http_tfs_remote_block_cache_lookup(&ctx->remote_ctx, pool,
                                                        log, key);
        }
    }
                                ...
                                ngx_http_tfs_block_cache_value_t *value)
{
    if (ctx->use_cache & NGX_HTTP_TFS_REMOTE_BLOCK_CACHE)
    {
        ngx_http_tfs_remote_block_cache_insert(&ctx->remote_ctx, pool, log, key,
                                               value);
    }
                                ...
                                uint8_t                         hit_status)
{
    ...
    {
        ngx_http_tfs_remote_block_cache_remove(&ctx->remote_ctx, pool, log,
                                               key);
    }
}
...
ngx_http_tfs_block_cache_batch_lookup(ngx_http_tfs_block_cache_ctx_t *ctx,
                                      ngx_pool_t *pool, ngx_log_t *log,
                                      ngx_array_t *keys, ngx_array_t *kvs)
{
    uint32_t                        i;
    ngx_int_t                       rc;
    ...
    if (ctx->curr_lookup_cache == NGX_HTTP_TFS_LOCAL_BLOCK_CACHE)
    {
        ...
        if (ctx->use_cache & NGX_HTTP_TFS_LOCAL_BLOCK_CACHE)
        {
            rc = ngx_http_tfs_local_block_cache_batch_lookup(
                ctx->local_ctx, pool, log, keys, kvs);
    ...
    if (ctx->curr_lookup_cache == NGX_HTTP_TFS_REMOTE_BLOCK_CACHE)
    {
        ...
        if (ctx->use_cache & NGX_HTTP_TFS_REMOTE_BLOCK_CACHE)
        {
            ...
            for (i = 0; i < keys->nelts; i++, segment_data++)
            {
                ...
                {
                    key = (ngx_http_tfs_block_cache_key_t *)ngx_array_push(
                        &local_miss_keys);
            ...
            rc = ngx_http_tfs_remote_block_cache_batch_lookup(
                &ctx->remote_ctx, pool, log, &local_miss_keys);
        }
    }
