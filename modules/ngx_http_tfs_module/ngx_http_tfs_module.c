...
static char *ngx_http_tfs_merge_srv_conf(ngx_conf_t *cf, void *parent,
                                         void *child);

static void *ngx_http_tfs_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_tfs_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                         void *child);
...
static char *ngx_http_tfs_upstream(ngx_conf_t *cf, ngx_command_t *cmd,
                                   void *conf);
static char *ngx_http_tfs_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

static char *ngx_http_tfs_log(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_conf_post_t ngx_http_tfs_lowat_post = {ngx_http_tfs_lowat_check};
...
static ngx_command_t ngx_http_tfs_commands[] = {
    ...
    {ngx_string("tfs_upstream"),
     ...
     ngx_http_tfs_upstream, 0, 0, NULL},

    {ngx_string("tfs_pass"),
     NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_TAKE12, ngx_http_tfs_pass,
     NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("tfs_keepalive"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE2,
     ngx_http_tfs_keepalive, 0, 0, NULL},

    {ngx_string("tfs_log"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE12, ngx_http_tfs_log,
     NGX_HTTP_SRV_CONF_OFFSET, 0, NULL},

    {ngx_string("tfs_connect_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, tfs_connect_timeout), NULL},

    {ngx_string("tfs_send_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, tfs_send_timeout), NULL},

    {ngx_string("tfs_read_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, tfs_read_timeout), NULL},

    {ngx_string("tair_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, tair_timeout), NULL},

    {ngx_string("tfs_send_lowat"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, 0, offsetof(ngx_http_tfs_main_conf_t, send_lowat),
     &ngx_http_tfs_lowat_post},

    {ngx_string("tfs_buffer_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, 0, offsetof(ngx_http_tfs_main_conf_t, buffer_size),
     NULL},

    {ngx_string("tfs_body_buffer_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, body_buffer_size), NULL},

    {ngx_string("tfs_block_cache_zone"), NGX_HTTP_MAIN_CONF | NGX_CONF_1MORE,
     ngx_http_tfs_block_cache_zone, 0, 0, NULL},

    {ngx_string("tfs_enable_remote_block_cache"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, 0,
     offsetof(ngx_http_tfs_main_conf_t, enable_remote_block_cache), NULL},
...
ngx_http_tfs_upstream_parse(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (ngx_strcmp(value[0].data, "server") == 0)
    {
        ...
        if (ngx_parse_url(cf->pool, &u) != NGX_OK)
        {
            if (u.err)
            {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in tfs \"%V\"",
                                   u.err, &u.url);
            }
...
ngx_http_tfs_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    tlcf->upstream =
        ngx_http_tfs_upstream_add(cf, &u, NGX_HTTP_TFS_UPSTREAM_FIND);
    ...
    if (tlcf->upstream->enable_rcs)
    {
        ...
        tlcf->upstream->rcs_shm_zone = ngx_shared_memory_add(
            cf, &tlcf->upstream->rcs_zone_name, 0, &ngx_http_tfs_module);
    ...
    if (tmcf->local_block_cache_ctx != NULL)
    {
        ...
        tmcf->block_cache_shm_zone =
            ngx_shared_memory_add(cf, &s, 0, &ngx_http_tfs_module);
...
ngx_http_tfs_rcs_zone(ngx_conf_t *cf, ngx_http_tfs_upstream_t *tu)
{
    ...
    shm_zone = ngx_shared_memory_add(cf, &name, size, &ngx_http_tfs_module);
    if (shm_zone == NULL)
    {
        return NGX_CONF_ERROR;
    }
...
ngx_http_tfs_block_cache_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "\"%V\" must have \"size\" parameter", &cmd->name);
        return NGX_CONF_ERROR;
    }
...
ngx_http_tfs_read_body_handler(ngx_http_request_t *r)
{
    ...
    if (rc != NGX_OK)
    {
        ...
        case NGX_HTTP_SPECIAL_RESPONSE ... NGX_HTTP_INTERNAL_SERVER_ERROR:
            ...
            break;
        default:
            ngx_log_error(NGX_LOG_ERR, t->log, 0, "ngx_http_tfs_init failed");
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        }
    }
}
...
ngx_http_tfs_module_init(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < tmcf->upstreams.nelts; i++)
    {
        if (!tup[i]->enable_rcs || !tup[i]->lock_file.len || !tup[i]->used)
        {
            return NGX_OK;
        }
