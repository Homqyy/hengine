...
ngx_http_tfs_rcs_lookup(ngx_http_tfs_rc_ctx_t *ctx, ngx_str_t appkey)
{
    ngx_int_t                rc;
    ngx_uint_t               hash;
...
last_free:
    node = (ngx_rbtree_node_t *)((u_char *)rc_info_node
...
ngx_http_tfs_dump_rc_info(ngx_http_tfs_rcs_info_t *rc_info, ngx_log_t *log)
{
    ...
    ngx_log_debug1(
        ...
        "=========dump rc info for appkey: %V =========", &rc_info->appkey);
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, log, 0,
                   "appid: %uL, logical_cluster_count: %uD", rc_info->app_id,
                   rc_info->logical_cluster_count);
    ...
    for (i = 0; i < rc_info->logical_cluster_count; i++)
    {
        ...
        for (j = 0; j < logical_clusters[i].rw_cluster_count; j++)
        {
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, log, 0,
                           "cluster_stat: %uD, access_type: %uD, cluster_id: "
                           "%V, ns_vip: %V",
    ...
    for (j = 0; j < rc_info->unlink_cluster_group_count; j++)
    {
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, log, 0,
                       "cluster_id: %ud, info_count: %uD, group_count: %D",
        ...
        {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, log, 0,
                           "group_seq: %D, ns_vip: %V", group_info[k].group_seq,
