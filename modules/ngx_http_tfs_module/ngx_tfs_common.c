...
ngx_chain_t *
ngx_http_tfs_chain_get_free_buf(ngx_pool_t *p, ngx_chain_t **free, size_t size)
{
    ngx_chain_t *cl;
                             ...
                             size_t size, ngx_log_t *log)
{
    ...
    if (ngx_buf_in_memory(b))
    {
        ...
        t_crc->data_crc =
            ngx_http_tfs_crc(t_crc->data_crc, (const char *)(b->pos), size);
        b->last = b->pos + size;
        return NGX_OK;
    }
    ...
    if (n != (ssize_t)size)
    {
        ngx_log_error(NGX_LOG_ALERT, log, 0,
                      ngx_read_file_n " read only %z of %O from \"%s\"", n,
                      size, b->file->name.data);
        goto crc_error;
    }

    t_crc->crc = ngx_http_tfs_crc(t_crc->crc, (const char *)dst, size);
    t_crc->data_crc =
        ngx_http_tfs_crc(t_crc->data_crc, (const char *)dst, size);
    free(dst);
                           ...
                           ngx_http_tfs_inet_t *addr)
{
    ...
    ngx_sprintf(p->peer_addr_text, "%s:%d", inet_ntoa(in->sin_addr),
                ntohs(in->sin_port));
...
ngx_int_t
ngx_http_tfs_sum_md5(ngx_chain_t *data, u_char *md5_final, ssize_t *data_len,
                     ngx_log_t *log)
{
    u_char   *buf;
    ssize_t   n, buf_size;
    ...
    while (data)
    {
        ...
        else
        {
            ...
            n = ngx_read_file(data->buf->file, buf, buf_size,
                              data->buf->file_pos);
            if (n == NGX_ERROR)
            {
                free(buf);
...
ngx_http_tfs_status_message(ngx_buf_t *b, ngx_str_t *action, ngx_log_t *log)
{
    ...
    if (code != NGX_HTTP_TFS_STATUS_MESSAGE_OK)
    {
        ...
        ngx_log_error(NGX_LOG_ERR, log, 0,
                      "%V failed error code (%d) err_msg(%V)", action, code,
                      &err);
...
ngx_http_tfs_get_parent_dir(ngx_str_t *file_path, ngx_int_t *dir_level)
{
    ...
    {
        if (file_path->data[i] == '/' && (file_path->data[i + 1]) != '/')
        {
            last_slash_pos = i;
            if (dir_level != NULL)
            {
...
ngx_http_tfs_set_output_file_name(ngx_http_tfs_t *t)
{
    ...
    ngx_memcpy(t->file_name.data,
               ngx_http_tfs_raw_fsname_get_name(
                   &t->r_ctx.fsname, t->is_large_file, t->r_ctx.simple_name),
...
void *
ngx_http_tfs_prealloc(ngx_pool_t *pool, void *p, size_t old_size,
                      size_t new_size)
{
    void *new;
                               ...
                               ngx_log_t                   *log)
{
    ngx_log_debug7(NGX_LOG_DEBUG_HTTP, log, 0,
                   ...
                   segment->segment_info.file_id, segment->segment_info.offset,
                   segment->segment_info.size, segment->segment_info.crc,
                   segment->oper_offset, segment->oper_size);
}
...
ngx_http_tfs_alloc_st(ngx_http_tfs_t *t)
{
    ...
    /* peers */
    st->tfs_peer_servers =
        ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_peer_connection_t)
                                 * NGX_HTTP_TFS_SERVER_COUNT);
...
ngx_http_tfs_get_content_type(u_char *data, ngx_str_t *type)
{
    ...
    if ((memcmp(data, "CWS", 3) == 0) || (memcmp(data, "FWS", 3) == 0))
    {
        ngx_str_set(type, "application/x-shockwave-flash");
        return NGX_OK;
    }

    if ((memcmp(data, "BM", 2) == 0) || (memcmp(data, "BA", 2) == 0)
        || (memcmp(data, "CI", 2) == 0) || (memcmp(data, "CP", 2) == 0)
        || (memcmp(data, "IC", 2) == 0) || (memcmp(data, "PI", 2) == 0))
    {
...
ngx_int_t
ngx_chain_add_copy_with_buf(ngx_pool_t *pool, ngx_chain_t **chain,
                            ngx_chain_t *in)
{
    ngx_buf_t   *b;
    ngx_chain_t *cl, **ll;
