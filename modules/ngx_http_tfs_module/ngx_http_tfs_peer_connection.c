...
ngx_http_tfs_peer_init(ngx_http_tfs_t *t)
{
    ...
    if (t->loc_conf->upstream->enable_rcs)
    {
        ...
        addr                      = inet_ntoa(
                                 ((struct sockaddr_in *)(rc_server->peer.sockaddr))->sin_addr);
        port =
            ntohs(((struct sockaddr_in *)(rc_server->peer.sockaddr))->sin_port);
    ...
    else
    {
        ...
        addr                       = inet_ntoa(
                                  ((struct sockaddr_in *)(name_server->peer.sockaddr))->sin_addr);
        port = ntohs(
            ((struct sockaddr_in *)(name_server->peer.sockaddr))->sin_port);
...
ngx_http_tfs_select_peer_v1(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        ...
        case NGX_HTTP_TFS_STATE_REMOVE_DONE: return t->tfs_peer;
