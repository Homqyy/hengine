...
ngx_http_tfs_remote_block_cache_lookup(
    ngx_http_tfs_remote_block_cache_ctx_t *ctx, ngx_pool_t *pool,
    ngx_log_t *log, ngx_http_tfs_block_cache_key_t *key)
{
    ...
    rc = ngx_http_tfs_tair_get_helper(
        ctx->tair_instance, pool, log, &tair_key,
        ngx_http_tfs_remote_block_cache_get_handler, (void *)ctx);
    ...
    if (rc == NGX_HTTP_ETAIR_SUCCESS)
    {
                ...
                > NGX_HTTP_TFS_REMOTE_BLOCK_CACHE_VALUE_BASE_SIZE))
        {
            ...
            if (ds_count > 0)
            {
                ...
                segment_data->block_info.ds_addrs = ngx_pcalloc(
                    t->pool, sizeof(ngx_http_tfs_inet_t) * ds_count);
                ...
                ngx_http_tfs_peer_set_addr(
                    t->pool, &t->tfs_peer_servers[NGX_HTTP_TFS_DATA_SERVER],
                    addr);
            }
            else
            {
    ...
    ngx_http_tfs_block_cache_value_t *value)
{
    ...
    *(uint32_t *)tair_value.data = value->ds_count;
    ...
    rc = ngx_http_tfs_tair_put_helper(
        ctx->tair_instance, tmp_pool, log, &tair_key, &tair_value, 0 /*expire*/,
        0 /* do not care version */,
        ngx_http_tfs_remote_block_cache_dummy_handler, (void *)tmp_pool);
    ...
    ngx_log_t *log, ngx_http_tfs_block_cache_key_t *key)
{
    ...
    (void)ngx_http_tfs_tair_delete_helper(
        ctx->tair_instance, tmp_pool, log, &tair_keys,
        ngx_http_tfs_remote_block_cache_dummy_handler, (void *)tmp_pool);
}
...
ngx_http_tfs_remote_block_cache_batch_lookup(
    ngx_http_tfs_remote_block_cache_ctx_t *ctx, ngx_pool_t *pool,
    ngx_log_t *log, ngx_array_t *keys)
{
    ngx_int_t                       rc;
    ngx_uint_t                      i;
    ...
    tair_kvs =
        ngx_array_create(pool, keys->nelts, sizeof(ngx_http_tair_key_value_t));
    if (tair_kvs == NULL)
    {
        return NGX_ERROR;
    }
    ...
    rc = ngx_http_tfs_tair_mget_helper(
        ctx->tair_instance, pool, log, tair_kvs,
        ngx_http_tfs_remote_block_cache_mget_handler, (void *)ctx);
    return rc;
}
        ...
        for (i = 0; i < kvs->nelts; i++, kv++)
        {
                    ...
                    > NGX_HTTP_TFS_REMOTE_BLOCK_CACHE_VALUE_BASE_SIZE))
            {
                ...
                if (ds_count > 0)
                {
                    ...
                    {
                        if (segment_data[j].segment_info.block_id
                                == key.block_id
                    ...
                    segment_data[j].block_info.ds_addrs = ngx_pcalloc(
                        t->pool, ds_count * sizeof(ngx_http_tfs_inet_t));
...
ngx_http_tfs_get_remote_block_cache_instance(
    ngx_http_tfs_remote_block_cache_ctx_t *ctx, ngx_str_t *server_addr)
{
    ...
    if (server_addr->len == 0 || server_addr->data == NULL)
    {
        return NGX_ERROR;
    }
    ...
    rc = ngx_http_tfs_parse_tair_server_addr_info(
        &server_addr_info, server_addr->data, server_addr_len, t->pool, 0);
    if (rc == NGX_ERROR)
    {
        return NGX_ERROR;
    }
    ...
    group_name =
        &server_addr_info.server[NGX_HTTP_TFS_TAIR_CONFIG_SERVER_COUNT];
    instance->server = ngx_http_etair_create_server(group_name, &config_server,
