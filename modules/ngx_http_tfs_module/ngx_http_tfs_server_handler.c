        ...
        case NGX_HTTP_TFS_ACTION_MOVE_FILE:
            if (t->dir_lens == NULL)
            {
                parent_dir_len = ngx_http_tfs_get_parent_dir(&t->last_file_path,
                                                             &dir_levels);
                t->dir_lens =
                    ngx_pcalloc(t->pool, sizeof(ngx_int_t) * dir_levels);
                if (t->dir_lens == NULL)
                {
                    return NGX_ERROR;
                }
            ...
            {
                parent_dir_len =
                    ngx_http_tfs_get_parent_dir(&t->last_file_path, NULL);
            }
            t->last_dir_level++;
    ...
    if (rc == NGX_OK || rc == NGX_DECLINED)
    {
        ...
        case NGX_HTTP_TFS_ACTION_LS_FILE:
                ...
                && t->meta_info.file_count > 0)
            {
                ...
                cl = ngx_http_tfs_json_custom_file_info(
                    t->json_output, &t->meta_info, t->r_ctx.file_type);
                if (cl == NULL)
                {
                    return NGX_ERROR;
                }
        ...
        case NGX_HTTP_TFS_ACTION_READ_FILE:
                ...
                || (t->r_ctx.chk_file_hole && !t->file.still_have))
            {
                if (t->r_ctx.chk_file_hole)
                {
                    ...
                    {
                        t->json_output =
                            ngx_http_tfs_json_init(t->log, t->pool);
                        if (t->json_output == NULL)
                        {
                            return NGX_ERROR;
                        }

                        for (cl = t->out_bufs, ll = &t->out_bufs; cl;
                             cl = cl->next)
                        {
                            ll = &cl->next;
                        }
        ...
        if (t->name_server_addr_text.len == 0)
        {
                ...
                && t->state == NGX_HTTP_TFS_STATE_WRITE_CLUSTER_ID_NS)
            {
                logical_cluster =
                    &t->rc_info_node
                         ->logical_clusters[t->logical_cluster_index];
                physical_cluster =
...
ngx_http_tfs_process_ms_ls_dir(ngx_http_tfs_t *t)
{
        ...
        || rc == NGX_HTTP_TFS_EXIT_TABLE_VERSION_ERROR)
    {
        ...
        ngx_http_tfs_peer_set_addr(
            t->pool, &tps[NGX_HTTP_TFS_ROOT_SERVER],
            (ngx_http_tfs_inet_t *)&t->loc_conf->meta_root_server);
    ...
    if (rc == NGX_OK)
    {
        ...
        for (meta_info = &t->meta_info; meta_info->next;
...
ngx_http_tfs_process_ns(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_WRITE_FILE:
        ...
        case NGX_HTTP_TFS_STATE_WRITE_GET_GROUP_SEQ:
            rc_info = t->rc_info_node;
            ngx_http_tfs_rcs_set_group_info_by_addr(
                rc_info, t->group_count, t->group_seq, t->name_server_addr);
            rc = ngx_http_tfs_select_name_server(
                t, rc_info, &t->name_server_addr, &t->name_server_addr_text);
            if (rc == NGX_ERROR)
            {
                return NGX_HTTP_TFS_EXIT_SERVER_OBJECT_NOT_FOUND;
            }
    ...
    addr = ngx_http_tfs_select_data_server(
            ...
            if (t->r_ctx.chk_exist == NGX_HTTP_TFS_NO)
            {
                ...
                cl = ngx_http_tfs_json_raw_file_stat(
                    ...
                    ngx_http_tfs_raw_fsname_get_name(&t->r_ctx.fsname,
                                                     t->is_large_file, 0),
                    t->r_ctx.fsname.file.block_id, &t->file_stat);
                if (cl == NULL)
                {
                    return NGX_ERROR;
                }
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        ...
        case NGX_HTTP_TFS_STATE_REMOVE_STAT_FILE:
            if (rc == NGX_OK)
            {
                ...
                {
                    t->state = NGX_HTTP_TFS_STATE_REMOVE_READ_META_SEGMENT;
                    segment_data->oper_size = ngx_min(
                        t->file_stat.size, NGX_HTTP_TFS_MAX_READ_FILE_SIZE);
                    return NGX_OK;
                }
        ...
        case NGX_HTTP_TFS_STATE_REMOVE_DELETE_DATA:
            ...
            if (t->file.segment_index >= t->file.segment_count)
            {
                ...
                if (t->r_ctx.version == 2)
                {
                    ...
                    body_buffer = &t->tfs_peer_servers[NGX_HTTP_TFS_META_SERVER]
                                       .body_buffer;
    ...
    if (addr == NULL)
    {
        ...
        case NGX_HTTP_TFS_ACTION_WRITE_FILE:
            ...
            else
            {
                ...
                rc = ngx_chain_add_copy_with_buf(t->pool, &segment_data->data,
                                                 segment_data->orig_data);
                if (rc == NGX_ERROR)
                {
                    return NGX_ERROR;
                }
...
ngx_http_tfs_process_ds_input_filter(ngx_http_tfs_t *t)
{
    ...
    if (resp->data_len < 0)
    {
        ...
        ngx_log_error(NGX_LOG_ERR, t->log, 0,
                      ...
                      segment_data->segment_info.file_id, tp->peer_addr_text,
                      resp->data_len);
        ngx_http_tfs_clear_buf(b);
        return NGX_HTTP_TFS_AGAIN;
    }
    ...
    if (resp->data_len >= NGX_HTTP_TFS_IMAGE_TYPE_SIZE)
    {
            ...
            && t->headers_in.content_type == NULL)
        {
            ...
            t->headers_in.content_type =
                ngx_pcalloc(t->pool, sizeof(ngx_table_elt_t));
            if (t->headers_in.content_type == NULL)
            {
                return NGX_ERROR;
            }

            rc = ngx_http_tfs_get_content_type(
                b->pos, &t->headers_in.content_type->value);
            if (rc != NGX_OK)
            {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, t->log, 0,
                   "read len is %O, data len is %D", t->length, resp->data_len);
...
ngx_http_tfs_process_ms_input_filter(ngx_http_tfs_t *t)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, t->log, 0, "ls dir len is %O",
                   t->length);
