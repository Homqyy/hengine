...
ngx_int_t
ngx_http_tfs_dedup_get(ngx_http_tfs_dedup_ctx_t *ctx, ngx_pool_t *pool,
                       ngx_log_t *log)
{
    u_char              *p;
    ssize_t              data_len;
    ...
    *(uint32_t *)p = htonl(data_len);
    ...
    rc = ngx_http_tfs_tair_get_helper(ctx->tair_instance, pool, log, &tair_key,
                               ...
                               void *data)
{
    ...
    if (rc == NGX_HTTP_ETAIR_SUCCESS)
    {
            ...
            && (kv->value->len > NGX_HTTP_TFS_DUPLICATE_VALUE_BASE_SIZE))
        {
            ...
            ctx->dup_file_name.data =
                ngx_pcalloc(t->pool, ctx->dup_file_name.len);
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, t->log, 0,
                       ...
                       &ctx->dup_file_name, ctx->file_ref_count,
                       ctx->dup_version);
    }
    else
    {
...
ngx_int_t
ngx_http_tfs_dedup_set(ngx_http_tfs_dedup_ctx_t *ctx, ngx_pool_t *pool,
                       ngx_log_t *log)
{
    u_char              *p;
    ssize_t              data_len;
    ...
    {
        rc =
            ngx_http_tfs_sum_md5(ctx->file_data, ctx->tair_key, &data_len, log);
        if (rc == NGX_ERROR)
        {
            return NGX_ERROR;
        }
    ...
    rc = ngx_http_tfs_tair_put_helper(
        ctx->tair_instance, pool, log, &tair_key, &tair_value, 0 /*expire*/,
        ctx->dup_version, ngx_http_tfs_dedup_set_handler, ctx);
...
ngx_int_t
ngx_http_tfs_dedup_remove(ngx_http_tfs_dedup_ctx_t *ctx, ngx_pool_t *pool,
                          ngx_log_t *log)
{
    u_char               *p;
    ssize_t               data_len;
    ...
    rc = ngx_http_tfs_tair_delete_helper(
        ctx->tair_instance, pool, log, &tair_keys,
                ...
                if (rc == NGX_OK)
                {
                    ...
                    if (t->r_ctx.unlink_type == NGX_HTTP_TFS_UNLINK_DELETE)
                    {
                        if (--ctx->file_ref_count <= 0)
                        {
                            ...
                            rc =
                                ngx_http_tfs_dedup_remove(ctx, t->pool, t->log);
...
ngx_int_t
ngx_http_tfs_get_dedup_instance(
    ngx_http_tfs_dedup_ctx_t             *ctx,
    ...
    uint32_t                              server_addr_hash)
{
    ...
    server = ngx_http_etair_create_server(server, &config_server,
