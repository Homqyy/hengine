...
ngx_http_tfs_select_meta_server(ngx_http_tfs_t *t)
{
    ...
    hash =
        ngx_http_tfs_murmur_hash((u_char *)&h, sizeof(ngx_http_tfs_meta_hh_t));
...
ngx_http_tfs_meta_server_create_message(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_CREATE_FILE:
        ngx_log_error(NGX_LOG_DEBUG, t->log, 0,
                      ...
                      t->last_dir_level, t->last_file_path.len,
                      &t->last_file_path);
...
ngx_http_tfs_meta_server_parse_message(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_LS_DIR:
    case NGX_HTTP_TFS_ACTION_LS_FILE: return ngx_http_tfs_parse_ls_message(t);
...
ngx_http_tfs_create_write_meta_message(ngx_http_tfs_t *t)
{
    ...
    frag_size =
        sizeof(ngx_http_tfs_meta_frag_info_t)
        + sizeof(ngx_http_tfs_meta_frag_meta_info_t) * need_write_frag_count;

    size = sizeof(ngx_http_tfs_ms_base_msg_header_t) + r_ctx->file_path_s.len
           + 1 +
           /* version */
           sizeof(uint64_t) + frag_size;
...
ngx_http_tfs_parse_write_meta_message(ngx_http_tfs_t *t)
{
    ...
    default:
        ngx_log_error(NGX_LOG_WARN, t->log, 0, " file type is %d ", type);
        return NGX_ERROR;
    }
...
ngx_http_tfs_parse_read_meta_message(ngx_http_tfs_t *t)
{
    ...
    if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_READ_FILE)
    {
        ...
        if (!t->file.still_have)
        {
            ...
            if (end_offset > ((uint64_t)last_segment->segment_info.offset
    ...
    ngx_log_error(NGX_LOG_DEBUG, t->log, 0,
                  "still_have is %d, frag count is %d", t->file.still_have,
                  count);
...
ngx_http_tfs_parse_ls_message(ngx_http_tfs_t *t)
{
    ...
    else
    {
        ...
        if (meta_info->file_index == meta_info->file_count)
        {
            ...
            meta_info =
                ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_custom_meta_info_t));
            if (meta_info == NULL)
            {
                return NGX_ERROR;
            }
            meta_info->files = ngx_pcalloc(
                t->pool, sizeof(ngx_http_tfs_custom_file_t) * count);
            if (meta_info->files == NULL)
            {
                return NGX_ERROR;
            }
    ...
    for (i = meta_info->file_index;
         i < meta_info->file_count && p < (b->last - sizeof(uint32_t)); i++)
    {
        ...
        {
            file[i].file_name.data =
                ngx_pcalloc(t->pool, file[i].file_name.len - 1);
