...
static ngx_chain_t *ngx_http_tfs_create_block_info_message(
...
static ngx_int_t ngx_http_tfs_parse_block_info_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data);
static ngx_int_t ngx_http_tfs_parse_batch_block_info_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data);
                                ...
                                ngx_http_tfs_inet_t *addr, ngx_str_t *addr_text)
{
                 ...
                 t->logical_cluster_index++)
            {
                     ...
                     t->rw_cluster_index++)
                {
                        ...
                        == NGX_HTTP_TFS_ACCESS_READ_AND_WRITE)
                    {
                        ...
                        {
                            cluster_id_text =
                                &physical_cluster->cluster_id_text;
                            cluster_id = ngx_http_tfs_get_cluster_id(
                                cluster_id_text->data);
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        for (i = 0; i < rc_info->unlink_cluster_group_count; i++)
        {
            ...
            /* if there are two clusters or more but group count is 1, sth wrong
             * in TFS cluster configure */
            ...
            for (j = 0; j < info_count; j++)
            {
                if (group_info[j].group_seq < 0)
                {
                    ...
                    else if (t->r_ctx.action.code
                             == NGX_HTTP_TFS_ACTION_WRITE_FILE)
                    {
                ...
                if (ngx_http_tfs_group_seq_match(
                        curr_block_id, cluster_group_info->group_count,
...
ngx_http_tfs_name_server_create_message(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_STAT_FILE:
        t->file.open_mode =
            NGX_HTTP_TFS_OPEN_MODE_STAT | NGX_HTTP_TFS_OPEN_MODE_READ;
...
ngx_http_tfs_name_server_parse_message(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_WRITE_FILE:
        ...
        {
        case NGX_HTTP_TFS_STATE_WRITE_GET_GROUP_COUNT:
            return ngx_http_tfs_parse_ctl_message(
                t, NGX_HTTP_TFS_CMD_GET_GROUP_COUNT);
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        ...
        {
        case NGX_HTTP_TFS_STATE_REMOVE_GET_GROUP_COUNT:
            rc = ngx_http_tfs_parse_ctl_message(
                t, NGX_HTTP_TFS_CMD_GET_GROUP_COUNT);
            return rc;
...
static ngx_chain_t *
ngx_http_tfs_create_block_info_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data)
{
    size_t                                size;
    ngx_buf_t                            *b;
                                      ...
                                      ngx_http_tfs_segment_data_t *segment_data)
{
    ...
    block_info->ds_addrs =
        ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_inet_t) * ds_count);
        ...
        || t->file.open_mode & NGX_HTTP_TFS_OPEN_MODE_STAT)
    {
        ...
        ngx_http_tfs_block_cache_insert(&t->block_cache_ctx, t->pool, t->log,
                                        &key, &value);
    }
...
ngx_int_t
ngx_http_tfs_parse_batch_block_info_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data)
{
    u_char                    *p;
    uint16_t                   type;
    uint32_t                   block_count, complete_count, ds_count, block_id;
    ngx_str_t                  err_msg;
    ngx_uint_t                 i, j, k;
    ngx_http_tfs_header_t     *header;
    ngx_http_tfs_block_info_t *block_info;
    ...
    for (i = 0; i < resp->block_count; i++)
    {
        ...
        if (t->file.open_mode & NGX_HTTP_TFS_OPEN_MODE_READ)
        {
            ...
            value.ds_count = block_info->ds_count;
            value.ds_addrs = (uint64_t *)block_info->ds_addrs;
            ngx_http_tfs_block_cache_insert(&t->block_cache_ctx, t->pool,
                                            t->log, &key, &value);
        }
