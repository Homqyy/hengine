...
static ngx_chain_t *ngx_http_tfs_create_createfile_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data);
static ngx_chain_t                     *
ngx_http_tfs_create_write_message(ngx_http_tfs_t              *t,
                                                      ngx_http_tfs_segment_data_t *segment_data);
static ngx_chain_t *ngx_http_tfs_create_closefile_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data);
static ngx_chain_t *
ngx_http_tfs_create_read_message(ngx_http_tfs_t              *t,
                                 ngx_http_tfs_segment_data_t *segment_data,
                                 uint8_t read_ver, uint8_t read_flag);
static ngx_chain_t *
...
static ngx_int_t ngx_http_tfs_parse_createfile_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data);
...
static int32_t   ngx_http_tfs_find_segment(uint32_t                     seg_count,
                                           ...
                                           int64_t                      offset);
static ngx_int_t ngx_http_tfs_copy_body_buffer(ngx_http_tfs_t *t, ssize_t bytes,
                                               u_char *body);
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        if (t->is_stat_dup_file)
        {
            if (block_info->ds_count > 0)
            {
                ...
                {
                    segment_data->ds_index =
                        ngx_random() % block_info->ds_count;
                }
            }
        }
        else
...
ngx_http_tfs_data_server_create_message(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_STAT_FILE:
        ...
        if (t->is_large_file)
        {
            ...
            return ngx_http_tfs_create_read_message(
                t, segment_data, NGX_HTTP_TFS_READ_V2,
    ...
    case NGX_HTTP_TFS_ACTION_READ_FILE:
        ...
        if (t->is_large_file && t->is_process_meta_seg)
        {
            if (t->meta_segment_data == NULL)
            {
                ...
                cl = ngx_http_tfs_chain_get_free_buf(
                    t->pool, &t->free_bufs, NGX_HTTP_TFS_MAX_FRAGMENT_SIZE);
                if (cl == NULL)
                {
                    return NULL;
                }
        ...
        if (t->r_ctx.version == 1 && t->file.file_offset == 0 && !t->parent)
        {
        ...
        return ngx_http_tfs_create_read_message(t, segment_data, t->read_ver,
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        ...
        case NGX_HTTP_TFS_STATE_REMOVE_READ_META_SEGMENT:
            ...
            return ngx_http_tfs_create_read_message(
                t, segment_data, t->read_ver, NGX_HTTP_TFS_READ_STAT_FORCE);
...
static ngx_chain_t *
ngx_http_tfs_create_createfile_message(
    ngx_http_tfs_t *t, ngx_http_tfs_segment_data_t *segment_data)
{
    size_t                        size;
    ngx_buf_t                    *b;
    ...
    req->base_header.crc = ngx_http_tfs_crc(
        NGX_HTTP_TFS_PACKET_FLAG, (const char *)(&req->base_header + 1),
        req->base_header.len);
                                  ...
                                  ngx_http_tfs_segment_data_t *segment_data)
{
    ...
    *((uint64_t *)p) = block_info->lease_id;
    ...
    req->header.base_header.len =
        size - sizeof(ngx_http_tfs_header_t) + req->length;
                                      ...
                                      ngx_http_tfs_segment_data_t *segment_data)
{
    ...
    *((uint32_t *)p) = NGX_HTTP_TFS_FILE_DEFAULT_OPTION;
    ...
    req->header.base_header.crc = ngx_http_tfs_crc(
        NGX_HTTP_TFS_PACKET_FLAG, (const char *)(&req->header.base_header + 1),
        (size - sizeof(ngx_http_tfs_header_t)));
    ...
    if (t->read_ver == NGX_HTTP_TFS_READ_V2)
    {
        ...
        if (t->length < 0 || (size_t)t->length <= NGX_HTTP_TFS_READ_V2_TAIL_LEN)
        {
            ...
            if (t->length == 0)
            {
                ...
                ngx_http_tfs_wrap_raw_file_info(&readv2_rsp_tail->file_info,
                                                &t->file_stat);
                t->file.left_length =
                    ngx_min(t->file.left_length, (uint64_t)t->file_stat.size);
            }
            return NGX_OK;
        }
        ...
        if (left_len < NGX_HTTP_TFS_READ_V2_TAIL_LEN)
        {
            ...
            {
                readv2_rsp_tail =
                    (ngx_http_tfs_ds_readv2_response_tail_t *)(b->pos + size);
                /* should not happened */
                if (readv2_rsp_tail->file_info_len
                    != NGX_HTTP_TFS_RAW_FILE_INFO_SIZE)
                {
            ...
            {
                t->readv2_rsp_tail_buf =
                    ngx_create_temp_buf(t->pool, NGX_HTTP_TFS_READ_V2_TAIL_LEN);
                if (t->readv2_rsp_tail_buf == NULL)
                {
                    return NGX_ERROR;
                }
                t->readv2_rsp_tail_buf->last = ngx_cpymem(
                    t->readv2_rsp_tail_buf->last, b->pos + size, tail_len);
            }
            else
            {
                return NGX_ERROR;
...
ngx_http_tfs_get_meta_segment(ngx_http_tfs_t *t)
{
    ...
    {
        t->file.segment_data =
            ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_segment_data_t));
        if (t->file.segment_data == NULL)
        {
            return NGX_ERROR;
        }
    ...
    ngx_log_error(NGX_LOG_INFO, t->log, 0,
                  ...
                  "seq_id: %uD, suffix: %uD",
                  segment_info->block_id, segment_info->file_id,
                  t->r_ctx.fsname.file.seq_id, t->r_ctx.fsname.file.suffix);
...
ngx_http_tfs_set_meta_segment_data(ngx_http_tfs_t *t)
{
    ...
    seg_info =
        (ngx_http_tfs_segment_info_t *)(b->pos
    ...
    rc = ngx_chain_add_copy_with_buf(t->pool, &segment_data->orig_data,
                                     segment_data->data);
    if (rc == NGX_ERROR)
    {
        return NGX_ERROR;
...
ngx_http_tfs_parse_meta_segment(ngx_http_tfs_t *t, ngx_chain_t *data)
{
    ...
    b = ngx_create_temp_buf(
        t->pool, sizeof(ngx_http_tfs_segment_head_t)
                     + segment_count * sizeof(ngx_http_tfs_segment_info_t));
    if (b == NULL)
    {
        return NGX_ERROR;
...
ngx_http_tfs_get_segment_for_read(ngx_http_tfs_t *t)
{
    ...
    max_seg_count =
        (b->last - (u_char *)seg_info) / sizeof(ngx_http_tfs_segment_info_t);
    ...
    /* alloc segment_data */
    t->file.segment_data =
        ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_segment_data_t) * seg_count);
    if (t->file.segment_data == NULL)
    {
        return NGX_ERROR;
    }
    ...
    last_segment->oper_size = ngx_min(
        (end_offset
         - (last_segment->segment_info.offset + last_segment->oper_offset)),
    ...
    {
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, t->log, 0,
                       "segment index: %d, oper_offset: %uD, oper_size: %uD", i,
                       t->file.segment_data[i].oper_offset,
                       t->file.segment_data[i].oper_size);
    }
#endif
...
ngx_http_tfs_get_segment_for_write(ngx_http_tfs_t *t)
{
    ...
    for (i = 0; i < seg_count; i++)
    {
        ...
        while (left_size > 0)
        {
            ...
            {
                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, t->log, 0, "zero body buf");
                body = body->next;
            }
            if (body == NULL)
            {
            ...
            else
            {
                ...
                ngx_log_debug3(NGX_LOG_DEBUG_HTTP, t->log, 0,
                               "pos: %O, last: %O, size: %z", b->file_pos,
                               b->file_last, buf_size);
            }
        ...
        rc = ngx_chain_add_copy_with_buf(t->pool,
                                         &t->file.segment_data[i].orig_data,
                                         t->file.segment_data[i].data);
        if (rc == NGX_ERROR)
        {
            return NGX_ERROR;
        }
...
ngx_http_tfs_get_segment_for_delete(ngx_http_tfs_t *t)
{
    ...
    t->file.segment_data = ngx_http_tfs_prealloc(
        t->pool, t->file.segment_data, sizeof(ngx_http_tfs_segment_data_t),
...
ngx_int_t
ngx_http_tfs_check_file_hole(ngx_http_tfs_file_t *file, ngx_array_t *file_holes,
                             ngx_log_t *log)
{
    int64_t                        curr_length;
