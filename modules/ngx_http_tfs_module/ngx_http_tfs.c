...
ngx_http_tfs_init(ngx_http_tfs_t *t)
{
    ...
    if (rc != NGX_OK)
    {
        ngx_log_error(NGX_LOG_ERR, t->log, 0, "tfs peer init failed");
    ...
    {
        ngx_log_error(NGX_LOG_ERR, t->log, 0, "tfs alloc chains failed");
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
    ...
    if (t->r_ctx.action.code != NGX_HTTP_TFS_ACTION_KEEPALIVE)
    {
        if (!upstream->enable_rcs)
        {
            ...
            case NGX_HTTP_TFS_ACTION_WRITE_FILE:
                ...
                break;
            default: ngx_shmtx_unlock(&rc_ctx->shpool->mutex); return NGX_ERROR;
            }

            t->name_server_addr.ip =
                ((struct sockaddr_in *)(upstream->ups_addr->sockaddr))
                    ->sin_addr.s_addr;
            t->name_server_addr.port =
                ntohs(((struct sockaddr_in *)(upstream->ups_addr->sockaddr))
                          ->sin_port);
                ...
                || (t->r_ctx.action.code != NGX_HTTP_TFS_ACTION_WRITE_FILE))
            {
                ...
                if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_WRITE_FILE)
                {
                    ...
                    t->file.left_length =
                        t->file.segment_data[0].segment_info.size;
                    t->file.segment_data[0].oper_size = ngx_min(
                        t->file.left_length, NGX_HTTP_TFS_MAX_FRAGMENT_SIZE);
                }
                else
                {
                    ...
                    t->file.segment_data[0].oper_size   = ngx_min(
                          t->file.left_length, NGX_HTTP_TFS_MAX_READ_FILE_SIZE);
                }
            }
        }
        else
...
ngx_http_tfs_lookup_block_cache(ngx_http_tfs_t *t)
{
    ...
    case NGX_OK:
        ...
        segment_data->block_info.ds_addrs =
            (ngx_http_tfs_inet_t *)value.ds_addrs;
        ...
        else
        {
            ...
            t->state += 1;
            ngx_http_tfs_peer_set_addr(
                t->pool, &t->tfs_peer_servers[NGX_HTTP_TFS_DATA_SERVER], addr);
        }

        break;
    case NGX_ERROR:
        /* block cache should not affect, go for ns */
        ngx_log_error(NGX_LOG_ERR, t->log, 0, "lookup block cache failed.");
        break;
    }
    rc = NGX_OK;
                                ...
                                ngx_http_tfs_segment_data_t *segment_data)
{
    ...
    key.block_id = segment_data->segment_info.block_id;
    ngx_http_tfs_block_cache_remove(&t->block_cache_ctx, t->pool, t->log, &key,
                                    segment_data->cache_hit);
...
ngx_http_tfs_batch_lookup_block_cache(ngx_http_tfs_t *t)
{
    ...
    rc = ngx_http_tfs_block_cache_batch_lookup(&t->block_cache_ctx, t->pool,
                                               t->log, &keys, &kvs);
    ...
    if (rc != NGX_ERROR && kvs.nelts > 0)
    {
        ...
        for (i = 0; i < kvs.nelts; i++, kv++)
        {
            ...
            segment_data[j].block_info.ds_addrs =
                (ngx_http_tfs_inet_t *)kv->value->ds_addrs;
...
ngx_http_tfs_connect(ngx_http_tfs_t *t)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, p->log, 0, "connecting %V, addr: %s", p->name,
                  tp->peer_addr_text);
    ...
    {
        ngx_log_error(NGX_LOG_ERR, p->log, 0, "connect to (%V: %s) failed",
                      p->name, tp->peer_addr_text);
...
ngx_http_tfs_event_handler(ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http tfs request: \"%V?%V\"",
                   &r->uri, &r->args);
...
ngx_http_tfs_send(ngx_http_request_t *r, ngx_http_tfs_t *t)
{
    ...
    if (rc == NGX_ERROR)
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "ngx output chain failed");
        ngx_http_tfs_finalize_request(r, t, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "ngx handle write event failed");
        ngx_http_tfs_finalize_request(r, t, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
ngx_http_tfs_alloc_buf(ngx_http_tfs_t *t)
{
    ...
    {
        tp->body_buffer.start =
            ngx_palloc(r->pool, t->main_conf->body_buffer_size);
        if (tp->body_buffer.start == NULL)
        {
            return NGX_ERROR;
        }
        ...
        tp->body_buffer.end =
            tp->body_buffer.start + t->main_conf->body_buffer_size;
...
ngx_http_tfs_finalize_state(ngx_http_tfs_t *t, ngx_int_t rc)
{
    ...
    if (rc == NGX_HTTP_CLIENT_CLOSED_REQUEST || rc == NGX_HTTP_REQUEST_TIME_OUT)
    {
        ngx_log_error(NGX_LOG_ERR, t->log, 0,
        ...
        if (!t->parent)
        {
            if (t->srv_conf->log != NULL)
            {
                ...
                if (action == NGX_HTTP_TFS_ACTION_REMOVE_FILE)
                {
                    ...
                    else if (t->r_ctx.unlink_type
                             == NGX_HTTP_TFS_UNLINK_CONCEAL)
                    {
                ...
                ngx_log_error(
                    NGX_LOG_INFO, t->srv_conf->log, 0,
                    "%d, %uL, %V, %V, %uD, %uL, %uL, %uL, %V", action,
                    t->loc_conf->upstream->enable_rcs
                        ? t->rc_info_node->app_id
                        : NGX_HTTP_TFS_DEFAULT_APPID,
                    &t->file_name, &t->r_ctx.file_suffix,
                    t->r_ctx.fsname.file.block_id,
                    ngx_http_tfs_raw_fsname_get_file_id(t->r_ctx.fsname),
                    t->r_ctx.offset, t->stat_info.size,
            ...
            if (t->loc_conf->upstream->enable_rcs)
            {
                ...
                if (rc_info != NULL)
                {
                    ...
                    {
                    case NGX_HTTP_TFS_ACTION_WRITE_FILE:
                        ngx_http_tfs_rcs_stat_update(t, rc_info,
    ...
    ngx_log_error(NGX_LOG_INFO, t->log, 0,
                  ...
                  t->tfs_peer->peer.name, t->tfs_peer->peer_addr_text);
...
ngx_http_tfs_process_upstream_request(ngx_http_request_t *r, ngx_http_tfs_t *t)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   ...
                   tp->peer.name, tp->peer_addr_text);
    ...
    if (rc == NGX_ERROR)
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "tfs alloc buf failed");
        ngx_http_tfs_finalize_request(r, t, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
ngx_http_tfs_set_header_line(ngx_http_tfs_t *t)
{
    ...
    case NGX_HTTP_TFS_ACTION_READ_FILE:
        ...
        case NGX_HTTP_TFS_STATE_READ_DONE:
            ...
            else
            {
                ...
                {
                    r->headers_out.content_type_len =
                        t->headers_in.content_type->value.len;
                    r->headers_out.content_type =
                        t->headers_in.content_type->value;
    ...
    ngx_log_error(NGX_LOG_INFO, t->log, 0, "%V success", &ctx->action.msg);
    return;
...
ngx_http_tfs_process_non_buffered_downstream(ngx_http_request_t *r)
{
    ...
    if (wev->timedout)
    {
        ...
        if (t->r_ctx.version == 1
            && t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_WRITE_FILE
            && t->r_ctx.is_raw_update == NGX_HTTP_TFS_NO && !t->parent)
        {
                                          ...
                                          ngx_uint_t      do_write)
{
    ...
    if (t->r_ctx.version == 1)
    {
        ...
        upstream =
            t->tfs_peer_servers[NGX_HTTP_TFS_DATA_SERVER].peer.connection;
    }
        ...
        if (do_write)
        {
            ...
            if (t->busy_bufs == NULL)
            {
                        ...
                        && t->state == NGX_HTTP_TFS_STATE_WRITE_DONE))
                {
                    /* need log size */
                    ngx_log_error(NGX_LOG_INFO, t->log, 0,
                                  "%V, output %uL byte", &t->r_ctx.action.msg,
                                  t->output_size);
                    ngx_http_tfs_finalize_request(r, t, 0);
                    return;
                }
            ...
            if (n > 0)
            {
                ...
                if (rc == NGX_ERROR)
                {
                    ...
                    ngx_http_tfs_finalize_request(
                        t->data, t, NGX_HTTP_INTERNAL_SERVER_ERROR);
                    return;
                }
                              ...
                              ngx_int_t rc)
{
    ...
    if (t->parent && t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_READ_FILE
        && t->parent->sp_curr != t->sp_curr)
    {
        if (rc != NGX_DONE)
        {
...
ngx_http_tfs_read_handler(ngx_http_request_t *r, ngx_http_tfs_t *t)
{
    ...
    if (c->read->timedout)
    {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "read from (%V: %s) timeout",
                      tp->peer.name, tp->peer_addr_text);
        ngx_http_tfs_handle_connection_failure(t, tp);
        return;
    }
                                       ...
                                       ngx_http_tfs_peer_connection_t *tp)
{
    ...
    if (ngx_strcmp(p->name->data, ms_name.data) == 0)
    {
        ...
        ngx_http_tfs_peer_set_addr(
            t->pool, &t->tfs_peer_servers[NGX_HTTP_TFS_ROOT_SERVER],
    ...
    {
        ngx_http_tfs_remove_block_cache(
            t, &t->file.segment_data[t->file.segment_index]);
    }

    /*connect rcserver fail, select another rc server*/
    if (ngx_strncmp(p->name->data, rcs_name.data, p->name->len) == 0)
    {
        ngx_log_error(NGX_LOG_WARN, t->log, 0, "select a new rc server");
...
ngx_http_tfs_misc_ctx_init(ngx_http_tfs_t *t, ngx_http_tfs_rcs_info_t *rc_info)
{
    ...
    if (t->r_ctx.version == 1)
    {
        ...
        ngx_http_tfs_peer_set_addr(
            t->pool, &t->tfs_peer_servers[NGX_HTTP_TFS_NAME_SERVER],
            &t->name_server_addr);
            ...
            || (t->r_ctx.action.code != NGX_HTTP_TFS_ACTION_WRITE_FILE))
        {
            ...
            if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_WRITE_FILE)
            {
                ...
                rc = ngx_chain_add_copy_with_buf(
                    t->pool, &t->file.segment_data[0].orig_data,
                    t->file.segment_data[0].data);
                if (rc == NGX_ERROR)
                {
                    return NGX_ERROR;
                }
    ...
    { /* custom tfs file */
        ...
        if (t->loc_conf->meta_server_table.version != 0)
        {
            ...
            ngx_http_tfs_peer_set_addr(
                t->pool, &t->tfs_peer_servers[NGX_HTTP_TFS_META_SERVER], addr);
        }
    }
    ...
    case NGX_HTTP_TFS_ACTION_REMOVE_FILE:
        ...
        if (t->r_ctx.version == 2 || !t->is_stat_dup_file
            || t->state != NGX_HTTP_TFS_STATE_REMOVE_GET_BLK_INFO)
        {
            break;
        }
    ...
    if (t->r_ctx.version == 1
        && t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_WRITE_FILE
        && !t->is_large_file && rc_info->need_duplicate && !t->r_ctx.no_dedup)
    {
        ...
        rc              = ngx_http_tfs_get_dedup_instance(
                         &t->dedup_ctx, &logical_cluster->dup_server_info,
                         logical_cluster->dup_server_addr_hash);
        if (rc == NGX_ERROR)
        {
            ngx_log_error(NGX_LOG_ERR, t->log, 0, "get dedup instance failed.");
            /* no dedup */
            return NGX_OK;
        }
...
ngx_http_tfs_set_duplicate_info(ngx_http_tfs_t *t)
{
    ...
    rc = ngx_http_tfs_dedup_set(&t->dedup_ctx, t->pool, t->log);
    /* save tair failed */
    if (rc == NGX_ERROR)
    {
        if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_WRITE_FILE)
        {
            ...
            case NGX_HTTP_TFS_STATE_WRITE_STAT_DUP_FILE:
                ...
                rc                                            = NGX_OK;
                break;
            case NGX_HTTP_TFS_STATE_WRITE_DONE: rc = NGX_DONE;
            }
        }
...
ngx_http_tfs_batch_process_next(ngx_http_tfs_t *t)
{
    if (t->sp_ready)
    {
        ...
        {
            ngx_log_error(NGX_LOG_ERR, t->log, 0, "(other) sub process failed");
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, t->log, 0,
                       "segment[%uD] wake up, will output...", t->sp_curr);
...
static void
ngx_http_tfs_check_broken_connection(ngx_http_request_t *r, ngx_event_t *ev)
{
    int               n;
    char              buf[1];
    ngx_err_t         err;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                   "http tfs check client, write event:%d, \"%V\"", ev->write,
                   &r->uri);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, err, "http tfs recv(): %d", n);
