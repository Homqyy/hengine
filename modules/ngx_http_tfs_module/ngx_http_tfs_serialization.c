...
ngx_int_t
ngx_http_tfs_serialize_string(u_char **p, ngx_str_t *string)
{
...
ngx_int_t
ngx_http_tfs_deserialize_string(u_char **p, ngx_pool_t *pool, ngx_str_t *string)
{
...
ngx_int_t
ngx_http_tfs_deserialize_vstring(u_char **p, ngx_pool_t *pool, uint32_t *count,
                                 ngx_str_t **string)
{
    ...
    ngx_http_tfs_object_info_t *object_info)
{
    ...
    *((uint32_t *)*p) = object_info->tfs_file_count;
    ...
    for (i = 0; i < object_info->tfs_file_count; i++) {
        ngx_http_tfs_serialize_tfs_file_info(p, object_info->tfs_file_infos +
i);
    }
...
//                     rc = ngx_http_tfs_deserialize_tfs_file_info(p,
//                     object_info->tfs_file_infos + i); if (rc == NGX_ERROR) {
//                         return NGX_ERROR;
//                     }
//                 }
//             }
...
//             rc = ngx_http_tfs_deserialize_object_meta_info(p,
//             &object_info->meta_info); if (rc == NGX_ERROR) {
//                 return NGX_ERROR;
//             }
//             break;
...
//             rc = ngx_http_tfs_deserialize_customize_info(p, pool,
//             &object_info->customize_info); if (rc == NGX_ERROR) {
//                 return NGX_ERROR;
//             }
//             break;
//         case NGX_HTTP_TFS_END_TAG:
...
ngx_int_t
ngx_http_tfs_serialize_rcs_stat(u_char **p, ngx_http_tfs_rcs_info_t *rc_info,
                                ngx_int_t *count)
{
    ngx_int_t                i;
    ...
    *count   = 0;
    ...
    for (i = 0; i < NGX_HTTP_TFS_OPER_COUNT; ++i)
    {
        ...
        *((uint32_t *)*p) =
            (stat_rcs[i].oper_app_id << 16) | stat_rcs[i].oper_type;
        (*p) += sizeof(uint32_t);
