...
ngx_http_upstream_do_get_keepalive_peer(ngx_peer_connection_t *pc,
                                        ngx_queue_t *cache, ngx_queue_t *free,
                                        off_t offset);
                                       ...
                                       ngx_str_t *val)
{
    ...
    kcf->slice_key = ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
...
ngx_http_upstream_keepalive(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        for (j = 0; j < sizeof(keepalive_params)
                                 ...
                                 ngx_http_upstream_srv_conf_t *us)
{
    ...
    kcf =
        ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_keepalive_module);
    ...
    if (kcf->slice_key)
    {
        ...
        {
            node = (ngx_http_upstream_keepalive_node_t *)(index
                                                          + offsetof(
                                                              ngx_rbtree_node_t,
                                                              color));
...
ngx_http_upstream_keepalive_cleanup(void *data)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "keepalive cleanup");
...
/* DONE */
static ngx_int_t
ngx_http_upstream_keepalive_get_peer_in_slice(
    ngx_peer_connection_t *pc, ngx_http_upstream_keepalive_peer_data_t *kp)
{
    ngx_int_t                           rc;
    ngx_http_upstream_keepalive_node_t *ukn;
                                      ...
                                      ngx_uint_t state)
{
    ...
    if (kp->conf->index)
    {
        ...
        if (ukn == NULL)
        {
            ...
            node = (ngx_rbtree_node_t *)((u_char *)ukn
...
ngx_http_upstream_keepalive_create_conf(ngx_conf_t *cf)
{
    ...
    conf =
        ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_keepalive_srv_conf_t));
    if (conf == NULL)
    {
        return NULL;
    }
