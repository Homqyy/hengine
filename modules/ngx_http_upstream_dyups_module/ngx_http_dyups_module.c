...
static ngx_command_t ngx_http_dyups_commands[] = {
    ...
    {ngx_string("dyups_interface"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
     ngx_http_dyups_interface, 0, 0, NULL},

    {ngx_string("dyups_read_msg_timeout"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_dyups_main_conf_t, read_msg_timeout), NULL},
    ...
    {ngx_string("dyups_read_msg_log"),
     ...
     offsetof(ngx_http_dyups_main_conf_t, read_msg_log), NULL},

    {ngx_string("dyups_shm_zone_size"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_dyups_main_conf_t, shm_size), NULL},

    {ngx_string("dyups_upstream_conf"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_http_dyups_cmd_deprecated, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},

    {ngx_string("dyups_trylock"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_flag_slot, NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_dyups_main_conf_t, trylock), NULL},
...
ngx_http_dyups_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "[dyups] init shm:%V, size:%ui",
                  &dmcf->shm_name, dmcf->shm_size);
...
ngx_http_dyups_init(ngx_conf_t *cf)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++)
    {
        ...
        duscf->dynamic =
            (uscfp[i]->port == 0 && uscfp[i]->srv_conf && uscfp[i]->servers
    ...
    ngx_http_dyups_deleted_upstream.srv_conf =
        ((ngx_http_conf_ctx_t *)(cf->ctx))->srv_conf;
    ngx_http_dyups_deleted_upstream.servers =
...
ngx_http_dyups_init_process(ngx_cycle_t *cycle)
{
    ...
    dmcf =
        ngx_http_cycle_get_module_main_conf(ngx_cycle, ngx_http_dyups_module);
    ...
    if (sh->status == NULL)
    {
        sh->status = ngx_slab_alloc_locked(shpool, sizeof(ngx_dyups_status_t)
    ...
    if (sh->version != 0)
    {
        ...
        for (i = 1; i < ccf->worker_processes; i++)
        {
            ngx_log_error(NGX_LOG_WARN, cycle->log, 0,
                          "[dyups] process %P %ui %ui", sh->status[i].pid,
                          status->time, sh->status[i].time);
        ...
        ngx_log_error(NGX_LOG_WARN, cycle->log, 0,
                      "[dyups] new process is %P, old process is %P", ngx_pid,
                      pid);
...
ngx_dyups_purge_msg(ngx_pid_t opid, ngx_pid_t npid)
{
    ...
    for (q = ngx_queue_last(&sh->msg_queue);
         q != ngx_queue_sentinel(&sh->msg_queue); q = ngx_queue_prev(q))
    {
    ...
    for (i = 0; i < dumcf->dy_upstreams.nelts; i++)
    {
        ...
        for (j = 0; j < duscf->upstream->servers->nelts; j++)
        {
            buf->last = ngx_sprintf(buf->last,
                                    ...
                                    &us[j].addrs->name, us[j].weight,
#ifdef NGX_HTTP_UPSTREAM_MAX_CONNS
                                    us[j].max_conns,
#endif
                                    us[j].max_fails, us[j].fail_timeout,
                                    us[j].backup, us[j].down);
        }
        buf->last = ngx_sprintf(buf->last, "\n");
    }
                        ...
                        ngx_uint_t index)
{
    ...
    ctx->main_conf =
        ((ngx_http_conf_ctx_t *)ngx_cycle->conf_ctx[ngx_http_module.index])
            ->main_conf;
                         ...
                         ngx_http_upstream_srv_conf_t *us)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "[dyups] dynamic upstream init peer: %i", rc);
...
ngx_http_dyups_read_msg_locked(ngx_event_t *ev)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, 0, "[dyups] read msg %P",
                   ngx_pid);
    ...
    for (i = 0; i < ccf->worker_processes; i++)
    {
        ...
        if (status->pid == 0 || status->pid == ngx_pid)
        {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                           "[dyups] process %P update time %ui", status->pid,
                           status->time);
         ...
         q != ngx_queue_sentinel(&sh->msg_queue); q = ngx_queue_prev(q))
    {
        ...
        if (msg->count == ccf->worker_processes)
        {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                           "[dyups] destroy msg %V:%V", &msg->name,
                           &msg->content);
        ...
        {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                           "[dyups] msg %V count %ui found", &msg->name,
                           msg->count);
            continue;
        }
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ev->log, 0, "[dyups] read end");
...
ngx_http_dyups_send_msg(ngx_str_t *name, ngx_buf_t *body, ngx_uint_t flag)
{
    ...
    {
        msg->content.data =
            ngx_slab_alloc_locked(shpool, body->last - body->pos);
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "[dyups] send msg %V count %ui version: %ui", &msg->name,
                   msg->count, sh->version);
                   ...
                   ngx_uint_t flag)
{
    ...
    if (flag == NGX_DYUPS_DELETE)
    {
        ...
        ngx_log_error(NGX_LOG_INFO, ngx_cycle->log, 0,
                      "[dyups] sync del: %V rv: %V rc: %i", name, &rv, rc);
    ...
    else if (flag == NGX_DYUPS_ADD)
    {
        ...
        ngx_log_error(NGX_LOG_INFO, ngx_cycle->log, 0,
                      "[dyups] sync add: %V rv: %V rc: %i", name, &rv, rc);
...
#else
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "set session: %p:%d",
...
#else
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "save session: %p:%d",
                   ssl_session, ssl_session->references);
#endif
...
#else
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "old session: %p:%d",
