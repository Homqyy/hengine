...
typedef void (*ngx_http_upstream_check_status_format_pt)(
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag);
...
#define upstream_check_index_invalid(check_ctx, index)          \
    (check_ctx == NULL || index >= check_ctx->peers_shm->number \
...
static void ngx_http_upstream_check_status_parse_args(
    ngx_http_request_t *r, ngx_http_upstream_check_status_ctx_t *ctx);
...
static void ngx_http_upstream_check_status_html_format(
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag);
static void ngx_http_upstream_check_status_csv_format(
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag);
static void ngx_http_upstream_check_status_json_format(
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag);
static void ngx_http_upstream_check_status_prometheus_format(
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag);
...
static char *ngx_http_upstream_check(ngx_conf_t *cf, ngx_command_t *cmd,
                                     void *conf);
...
static char *ngx_http_upstream_check_status(ngx_conf_t *cf, ngx_command_t *cmd,
                                            void *conf);
...
static ngx_int_t ngx_http_upstream_check_init_shm_peer(
    ...
    ngx_pool_t *pool, ngx_str_t *peer_name);
...
static ngx_command_t ngx_http_upstream_check_commands[] = {
    ...
    {ngx_string("check"), NGX_HTTP_UPS_CONF | NGX_CONF_1MORE,
     ngx_http_upstream_check, 0, 0, NULL},

    {ngx_string("check_keepalive_requests"), NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
     ngx_http_upstream_check_keepalive_requests, 0, 0, NULL},

    {ngx_string("check_http_send"), NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
     ngx_http_upstream_check_http_send, 0, 0, NULL},

    {ngx_string("check_http_expect_alive"), NGX_HTTP_UPS_CONF | NGX_CONF_1MORE,
     ngx_http_upstream_check_http_expect_alive, 0, 0, NULL},

    {ngx_string("check_fastcgi_param"), NGX_HTTP_UPS_CONF | NGX_CONF_TAKE2,
     ngx_http_upstream_check_fastcgi_params, 0, 0, NULL},

    {ngx_string("check_shm_size"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_http_upstream_check_shm_size, 0, 0, NULL},

    {ngx_string("check_status"),
     NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1 | NGX_CONF_NOARGS,
     ngx_http_upstream_check_status, 0, 0, NULL},
...
static char sslv3_client_hello_pkt[] = {
    ...
    "\x00\x02"
    "\x00\x03"
    "\x00\x04" /* 39 most common ciphers :  */
    "\x00\x05"
    "\x00\x06"
    "\x00\x07"
    "\x00\x08" /* 0x01...0x1B, 0x2F...0x3A  */
    "\x00\x09"
    "\x00\x0A"
    "\x00\x0B"
    "\x00\x0C" /* This covers RSA/DH,       */
    "\x00\x0D"
    "\x00\x0E"
    "\x00\x0F"
    "\x00\x10" /* various bit lengths,      */
    "\x00\x11"
    "\x00\x12"
    "\x00\x13"
    "\x00\x14" /* SHA1/MD5, DES/3DES/AES... */
    "\x00\x15"
    "\x00\x16"
    "\x00\x17"
    "\x00\x18"
...
static char ngx_ajp_cping_packet[] = {0x12, 0x34,          0x00,
                                      0x01, NGX_AJP_CPING, 0x00};

static char ngx_ajp_cpong_packet[] = {0x41, 0x42, 0x00, 0x01, NGX_AJP_CPONG};


static ngx_check_conf_t ngx_check_types[] = {

    {NGX_HTTP_CHECK_TCP, ngx_string("tcp"), ngx_null_string, 0,
     ngx_http_upstream_check_peek_handler, ngx_http_upstream_check_peek_handler,
     NULL, NULL, NULL, 0, 0},

    {NGX_HTTP_CHECK_HTTP, ngx_string("http"),
     ngx_string("GET / HTTP/1.0\r\n\r\n"),
     NGX_CONF_BITMASK_SET | NGX_CHECK_HTTP_2XX | NGX_CHECK_HTTP_3XX,
     ngx_http_upstream_check_send_handler, ngx_http_upstream_check_recv_handler,
     ngx_http_upstream_check_http_init, ngx_http_upstream_check_http_parse,
     ngx_http_upstream_check_http_reinit, 1, 1},

    {NGX_HTTP_CHECK_HTTP, ngx_string("fastcgi"), ngx_null_string, 0,
     ngx_http_upstream_check_send_handler, ngx_http_upstream_check_recv_handler,
     ngx_http_upstream_check_http_init, ngx_http_upstream_check_fastcgi_parse,
     ngx_http_upstream_check_http_reinit, 1, 0},

    {NGX_HTTP_CHECK_SSL_HELLO, ngx_string("ssl_hello"),
     ngx_string(sslv3_client_hello_pkt), 0,
     ngx_http_upstream_check_send_handler, ngx_http_upstream_check_recv_handler,
     ngx_http_upstream_check_ssl_hello_init,
     ngx_http_upstream_check_ssl_hello_parse,
     ngx_http_upstream_check_ssl_hello_reinit, 1, 0},

    {NGX_HTTP_CHECK_MYSQL, ngx_string("mysql"), ngx_null_string, 0,
     ngx_http_upstream_check_send_handler, ngx_http_upstream_check_recv_handler,
     ngx_http_upstream_check_mysql_init, ngx_http_upstream_check_mysql_parse,
     ngx_http_upstream_check_mysql_reinit, 1, 0},

    {NGX_HTTP_CHECK_AJP, ngx_string("ajp"), ngx_string(ngx_ajp_cping_packet), 0,
     ngx_http_upstream_check_send_handler, ngx_http_upstream_check_recv_handler,
     ngx_http_upstream_check_ajp_init, ngx_http_upstream_check_ajp_parse,
     ngx_http_upstream_check_ajp_reinit, 1, 0},
...
static ngx_check_status_conf_t ngx_check_status_formats[] = {
    ...
    {ngx_string("csv"), ngx_string("text/plain"),
     ngx_http_upstream_check_status_csv_format},

    {ngx_string("json"), ngx_string("application/json"), /* RFC 4627 */
     ngx_http_upstream_check_status_json_format},

    {ngx_string("prometheus"), ngx_string("text/plain"),
...
static ngx_check_status_command_t ngx_check_status_commands[] = {
    ...
    {ngx_string("format"), ngx_http_upstream_check_status_command_format},

    {ngx_string("status"), ngx_http_upstream_check_status_command_status},
                                         ...
                                         ngx_addr_t *peer_addr)
{
    ...
    index =
        ngx_http_upstream_check_add_dynamic_peer_shm(pool, ucscf, peer_addr);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pool->log, 0,
                   "http upstream check add dynamic upstream: %p, n: %ui", p,
                   peers->peers.nelts);
        ...
        if (elts != peers->peers.elts)
        {
            ...
            for (i = 0; i < peers->peers.nelts - 1; i++)
            {
                ...
                ngx_log_error(NGX_LOG_INFO, pool->log, 0,
                              ...
                              np[i].upstream_name, np[i].check_ev.data, &np[i],
                ...
                ngx_http_upstream_check_add_timer(
                    &np[i], np[i].conf->check_type_conf, 0, pool->log);
            }
        }
    }
    ...
    if (ucscf->port)
    {
        ...
        if (ngx_http_upstream_check_addr_change_port(
                pool, peer->check_peer_addr, peer_addr, ucscf->port)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
    ...
    ngx_http_upstream_check_add_timer(peer, ucscf->check_type_conf, 0,
                                      pool->log);
                                 ...
                                 ngx_addr_t                   *peer_addr)
{
    ...
    {
        return ngx_http_upstream_check_add_dynamic_peer(cf->pool, us,
                                                        peer_addr);
    }
    ...
    if (ucscf->port)
    {
        ...
        if (ngx_http_upstream_check_addr_change_port(
                cf->pool, peer->check_peer_addr, peer_addr, ucscf->port)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
                                            ...
                                            ngx_addr_t *peer_addr)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        if (peer[i].upstream_name->len != name->len
            || ngx_strncmp(peer[i].upstream_name->data, name->data, name->len)
                   != 0)
        {
            continue;
        }
...
static ngx_uint_t
ngx_http_upstream_check_add_dynamic_peer_shm(
    ngx_pool_t *pool, ngx_http_upstream_check_srv_conf_t *ucscf,
    ngx_addr_t *peer_addr)
{
    ngx_int_t                            rc;
    ...
    peer_shm[index].sockaddr = ngx_slab_alloc_locked(shpool, peer_shm->socklen);
    ...
    rc = ngx_http_upstream_check_init_shm_peer(
        &peer_shm[index], NULL, ucscf->default_down, pool, &peer_addr->name);
    if (rc != NGX_OK)
    {
        goto fail;
    }
...
static ngx_uint_t
ngx_http_upstream_check_unique_peer(
    ngx_http_upstream_check_peers_t *peers, ngx_addr_t *peer_addr,
    ngx_http_upstream_check_srv_conf_t *peer_conf)
{
    ngx_uint_t                          i;
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        if (ngx_memcmp(peer[i].peer_addr->sockaddr, peer_addr->sockaddr,
                       peer_addr->socklen)
            != 0)
        {
            continue;
        ...
        if (ngx_strncmp(opeer_conf->send.data, peer_conf->send.data,
                        peer_conf->send.len)
            != 0)
        {
            continue;
...
ngx_http_upstream_check_upstream_down(ngx_str_t *upstream)
{
    ...
    for (i = 0; i < check_peers_ctx->peers.nelts; i++)
    {
        if (peers[i].upstream_name->len == upstream->len
            && ngx_strncmp(peers[i].upstream_name->data, upstream->data,
                           upstream->len)
                   == 0)
        {
            if (!peers[i].shm->down)
...
ngx_http_upstream_check_add_timers(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        ngx_http_upstream_check_add_timer(&peer[i], ucscf->check_type_conf, t,
                                          cycle->log);
    }
...
ngx_http_upstream_check_begin_handler(ngx_event_t *event)
{
    ...
    if (peer->shm->owner == ngx_pid || peer->check_timeout_ev.timer_set)
    {
        return;
    }
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, event->log, 0,
                   ...
                   peer->index, peer->shm->owner, ngx_pid, interval,
...
ngx_http_upstream_check_peek_one_byte(ngx_connection_t *c)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, err,
                   "http check upstream recv(): %i, fd: %d", n, c->fd);
...
ngx_http_upstream_check_send_handler(ngx_event_t *event)
{
    ...
    {
        peer->check_data =
            ngx_pcalloc(peer->pool, sizeof(ngx_http_upstream_check_ctx_t));
        if (peer->check_data == NULL)
        {
            goto check_send_fail;
        }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http check send total: %z",
...
ngx_http_upstream_check_recv_handler(ngx_event_t *event)
{
    ...
    default: ngx_http_upstream_check_status_update(peer, 1); break;
    }
...
ngx_http_upstream_check_http_parse(ngx_http_upstream_check_peer_t *peer)
{
    ...
    {
        rc = ngx_http_upstream_check_parse_status_line(ctx, &ctx->recv,
                                                       &ctx->status);
        if (rc == NGX_AGAIN)
        {
            return rc;
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "http_parse: code_n: %ui, conf: %ui", code_n,
                       ucscf->code.status_alive);
    ...
    ngx_http_upstream_check_ctx_t *ctx, ngx_buf_t *b, ngx_http_status_t *status)
{
    ...
    for (p = b->pos; p < b->last; p++)
    {
        ...
        case ngx_http_fastcgi_st_padding: break;
        }
    }
...
ngx_http_upstream_check_fastcgi_parse(ngx_http_upstream_check_peer_t *peer)
{
    ...
    for (;;)
    {
        ...
        {
            rc = ngx_http_upstream_check_fastcgi_process_record(ctx, &ctx->recv,
                                                                &ctx->status);
        ...
        for (;;)
        {
            rc = ngx_http_upstream_check_parse_fastcgi_status(ctx, &ctx->recv,
                                                              &ctx->status);
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "fastcgi http_parse: code_n: %ui, conf: %ui", code_n,
                       ucscf->code.status_alive);
                                             ...
                                             ngx_http_status_t *status)
{
    ...
    for (p = b->pos; p < b->last; p++)
    {
        ...
        case sw_start:
            ...
            switch (ch)
            {
            case CR: state = sw_header_almost_done; break;
            case LF: goto header_done;
            default:
                state = sw_name;
        ...
        case sw_name:
            ...
            if (ch == ':')
            {
                ...
                if (ngx_strncasecmp(name_s, (u_char *)"status", name_e - name_s)
                    == 0)
                {
        ...
        case sw_value:
            ...
            switch (ch)
            {
            case ' ': state = sw_space_after_value; break;
            case CR: state = sw_almost_done; break;
            case LF: goto done;
            case '\0': return NGX_ERROR;
            }
                                          ...
                                          ngx_http_status_t             *status)
{
    ...
    for (p = b->pos; p < b->last; p++)
    {
        ...
        case sw_space_after_status:
            switch (ch)
            {
            case ' ': state = sw_status_text; break;
...
ngx_http_upstream_check_clean_event(ngx_http_upstream_check_peer_t *peer)
{
    ...
    {
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,
                       "http check clean event: index:%i, fd: %d", peer->index,
                       c->fd);
        if (c->error == 0 && cf->need_keepalive
...
ngx_http_upstream_check_clear_peer(ngx_http_upstream_check_peer_t *peer)
{
    if (peer != peer->check_ev.data)
    {
        ngx_log_error(NGX_LOG_CRIT, ngx_cycle->log, 0,
                      "different peer: %p, data: %p, timer: %p", peer,
                      peer->check_ev.data, &peer->check_ev);
    }
...
static void
ngx_http_upstream_check_status_parse_args(
    ngx_http_request_t *r, ngx_http_upstream_check_status_ctx_t *ctx)
{
    ngx_str_t                   value;
    ngx_uint_t                  i;
    ...
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last = ngx_snprintf(
            ...
            "    <td>%ui</td>\n"
            "  </tr>\n",
            peer[i].shm->down ? " bgcolor=\"#FF0000\"" : "", i,
            peer[i].upstream_name, &peer[i].peer_addr->name,
            peer[i].shm->down ? "down" : "up", peer[i].shm->rise_count,
            peer[i].shm->fall_count, &peer[i].conf->check_type_conf->name,
            peer[i].conf->port);
    }
    ...
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last = ngx_snprintf(
            b->last, b->end - b->last, "%ui,%V,%V,%s,%ui,%ui,%V,%ui\n", i,
            peer[i].upstream_name, &peer[i].peer_addr->name,
            peer[i].shm->down ? "down" : "up", peer[i].shm->rise_count,
            peer[i].shm->fall_count, &peer[i].conf->check_type_conf->name,
            peer[i].conf->port);
    ...
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last = ngx_snprintf(
            ...
            "\"port\": %ui}"
            "%s\n",
            i, peer[i].upstream_name, &peer[i].peer_addr->name,
            peer[i].shm->down ? "down" : "up", peer[i].shm->rise_count,
            peer[i].shm->fall_count, &peer[i].conf->check_type_conf->name,
            peer[i].conf->port, (last == count) ? "" : ",");
    }
    ...
    ngx_buf_t *b, ngx_http_upstream_check_peers_t *peers, ngx_uint_t flag)
{
    ...
    b->last = ngx_snprintf(
        ...
        "# HELP nginx_upstream_count_up Nginx total number of servers that are "
        "UP\n"
        "# TYPE nginx_upstream_count_up gauge\n"
        "nginx_upstream_count_up %ui\n"
        "# HELP nginx_upstream_count_down Nginx total number of servers that "
        "are DOWN\n"
        ...
        count, upCount, downCount, ngx_http_upstream_check_shm_generation);
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last = ngx_snprintf(
            b->last, b->end - b->last,
            "nginx_upstream_server_rise{index=\"%ui\",upstream=\"%V\",name=\"%"
            "V\",status=\"%s\",type=\"%V\",port=\"%ui\"} %ui\n",
            i, peer[i].upstream_name, &peer[i].peer_addr->name,
            peer[i].shm->down ? "down" : "up",
            &peer[i].conf->check_type_conf->name, peer[i].conf->port,
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last = ngx_snprintf(
            b->last, b->end - b->last,
            "nginx_upstream_server_fall{index=\"%ui\",upstream=\"%V\",name=\"%"
            "V\",status=\"%s\",type=\"%V\",port=\"%ui\"} %ui\n",
            i, peer[i].upstream_name, &peer[i].peer_addr->name,
            peer[i].shm->down ? "down" : "up",
            &peer[i].conf->check_type_conf->name, peer[i].conf->port,
    ...
    b->last = ngx_snprintf(b->last, b->end - b->last,
                           "# HELP nginx_upstream_server_active Nginx active 1 "
                           "for UP / 0 for DOWN\n"
    ...
    for (i = 0; i < peers->peers.nelts; i++)
    {
        ...
        b->last =
            ngx_snprintf(b->last, b->end - b->last,
                         "nginx_upstream_server_active{index=\"%ui\",upstream="
                         "\"%V\",name=\"%V\",type=\"%V\",port=\"%ui\"} %ui\n",
                         i, peer[i].upstream_name, &peer[i].peer_addr->name,
                         &peer[i].conf->check_type_conf->name,
                         peer[i].conf->port, peer[i].shm->down ? 0 : 1);
    }
}
...
ngx_http_get_check_type_conf(ngx_str_t *str)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        if (ngx_strncmp(str->data, ngx_check_types[i].name.data, str->len) == 0)
        {
...
ngx_http_upstream_check(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ucscf =
        ngx_http_conf_get_module_srv_conf(cf, ngx_http_upstream_check_module);
    if (ucscf == NULL)
    {
                                          ...
                                          void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++)
    {
        ...
        {
            ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "invalid value \"%s\"",
                               value[i].data);
...
ngx_http_upstream_check_status(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cf->args->nelts == 2)
    {
        uclcf = ngx_http_conf_get_module_loc_conf(
            cf, ngx_http_upstream_check_module);
...
ngx_http_upstream_check_create_main_conf(ngx_conf_t *cf)
{
    ...
    ucmcf->peers =
        ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_check_peers_t));
    if (ucmcf->peers == NULL)
    {
        return NULL;
    }
...
ngx_http_upstream_check_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    b = ngx_http_upstream_check_create_fastcgi_request(
        cf->pool, fastcgi_default_params,
...
ngx_http_upstream_check_create_srv_conf(ngx_conf_t *cf)
{
    ...
    ucscf->fastcgi_params =
        ngx_array_create(cf->pool, 2 * 4, sizeof(ngx_str_t));
...
ngx_http_upstream_check_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    ngx_http_upstream_check_get_shm_name(
        shm_name, cf->pool, ngx_http_upstream_check_shm_generation);

    /* The default check shared memory size is 1M */
    shm_size = 1 * 1024 * 1024;

    shm_size =
        shm_size < ucmcf->check_shm_size ? ucmcf->check_shm_size : shm_size;
...
ngx_http_upstream_check_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (!same)
    {
        ...
        {
            ngx_http_upstream_check_get_shm_name(
                &oshm_name, pool, ngx_http_upstream_check_shm_generation - 1);

            /* The global variable ngx_cycle still points to the old one */
            oshm_zone =
                ngx_shared_memory_find((ngx_cycle_t *)ngx_cycle, &oshm_name,
                                       &ngx_http_upstream_check_module);
        ...
        size = sizeof(*peers_shm)
               + (number - 1 + MAX_DYNAMIC_PEER)
                     * sizeof(ngx_http_upstream_check_peer_shm_t);
    ...
    for (i = 0; i < number; i++)
    {
        ...
        if (opeers_shm)
        {
            ...
            if (opeer_shm)
            {
                ...
                rc = ngx_http_upstream_check_init_shm_peer(
                    peer_shm, opeer_shm, 0, pool, &peer[i].peer_addr->name);
                if (rc != NGX_OK)
                {
                    return NGX_ERROR;
                }
...
ngx_http_upstream_check_init_shm_peer(ngx_http_upstream_check_peer_shm_t *psh,
                                      ngx_http_upstream_check_peer_shm_t *opsh,
                                      ngx_uint_t init_down, ngx_pool_t *pool,
                                      ngx_str_t *name)
{
    u_char *file;
