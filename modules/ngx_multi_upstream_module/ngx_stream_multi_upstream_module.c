...
static ngx_int_t ngx_stream_multi_upstream_add_data(ngx_connection_t     *c,
...
static ngx_int_t ngx_stream_multi_upstream_init_connection(
    ngx_connection_t *c, ngx_peer_connection_t *pc, void *data);
...
static ngx_command_t ngx_stream_multi_upstream_commands[] = {
    {ngx_string("multi"), NGX_STREAM_UPS_CONF | NGX_CONF_TAKE1,
     ngx_stream_multi_upstream, NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},
                               ...
                               ngx_stream_upstream_srv_conf_t *us)
{
    ...
    kcf =
        ngx_stream_conf_upstream_srv_conf(us, ngx_stream_multi_upstream_module);
                                    ...
                                    ngx_stream_upstream_srv_conf_t *us)
{
    ...
    kp = ngx_palloc(s->connection->pool,
                    sizeof(ngx_stream_multi_upstream_peer_data_t));
    if (kp == NULL)
    {
        return NGX_ERROR;
    }
...
ngx_stream_multi_upstream_get_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (rc != NGX_OK)
    {
        ngx_log_error(NGX_LOG_ERR, kp->session->connection->log, 0,
                      "multi: balancer get: %i", rc);
        return rc;
    }
                                          ...
                                          ngx_peer_connection_t *pc, void *data)
{
    ...
    *fake_s->upstream = *kp->session->upstream;
    ...
    fake_s->upstream->state =
        ngx_pcalloc(c->pool, sizeof(ngx_stream_upstream_state_t));
                                    ...
                                    ngx_uint_t state)
{
    ...
    if (q)
    {
        ...
        ngx_log_error(NGX_LOG_INFO, pc->connection->log, 0,
                      "multi: free stream session %p, %p", session,
                      pc->connection);
    }
    else
    {
        ngx_log_error(NGX_LOG_WARN, pc->connection->log, 0,
                      "multi: free stream session not found %p, %p", session,
                      pc->connection);
    }
...
static void
ngx_stream_multi_upstream_notify_peer(ngx_peer_connection_t *pc, void *data,
                                      ngx_uint_t type)
{
    ngx_stream_multi_upstream_peer_data_t *kp = data;
...
ngx_stream_multi_upstream_create_conf(ngx_conf_t *cf)
{
    ...
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_stream_multi_upstream_srv_conf_t));
    if (conf == NULL)
    {
        return NULL;
    }
...
ngx_stream_multi_upstream_connection_close(ngx_connection_t *c)
{
    ...
    ngx_log_error(NGX_LOG_INFO, c->log, 0,
                  "multi: multi connection real close %p", c);
