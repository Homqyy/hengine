...
void
ngx_http_multi_upstream_send_response(ngx_http_request_t  *r,
...
// backend pc next, will close pc and do upstream_next for each front relate the
// pc
void
...
// backend pc finalize, will close pc and do finalize for each front relate  the
// pc
void
    ...
    if (u->buffer.start == NULL)
    {
            ...
            != NGX_OK)
        {
            ngx_http_multi_upstream_finalize_request(
                pc, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    ...
    for (;;)
    {
        if (b->last == b->end)
        {
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                           "multi: read buffer full %p, %p, %p, %p", b->start,
                           b->end, b->pos, b->last);
        }
        else
        {
        ...
        for (;;)
        {
            ...
            {
                ngx_http_multi_upstream_next(
                    pc, NGX_HTTP_UPSTREAM_FT_INVALID_HEADER);
                return;
            }
            ...
            {
                real_u->state->header_time =
                    ngx_current_msec - real_u->state->response_time;
                ...
                if (real_u->headers_in.status_n >= NGX_HTTP_SPECIAL_RESPONSE)
                {
                    ...
                    if (ngx_http_upstream_intercept_errors(real_r, real_u)
                        == NGX_OK)
                    {
                        continue;
                    }
            ...
            else if (rc == NGX_HTTP_UPSTREAM_GET_BODY_DATA)
            {
                if (!real_u->header_sent)
                {
                    ...
                    real_u->state->header_time =
                        ngx_current_msec - real_u->state->response_time;
                    if (real_u->headers_in.status_n
                        >= NGX_HTTP_SPECIAL_RESPONSE)
                    {
                        if (ngx_http_upstream_test_next(real_r, real_u)
                            == NGX_OK)
                        {
                            continue;
                        }
                    ...
                    if (ngx_http_upstream_process_headers(real_r, real_u)
                        != NGX_OK)
                    {
                        continue;
                    }
            ...
            else if (rc == NGX_HTTP_UPSTREAM_PARSE_ERROR)
            {
                ...
                if (!real_u->header_sent)
                {
                    ngx_http_upstream_finalize_request(real_r, real_u,
            ...
            else
            {
                ...
                if (!real_u->header_sent)
                {
                    ngx_http_upstream_finalize_request(
                        real_r, real_u, NGX_HTTP_INTERNAL_SERVER_ERROR);
                }
                else
                {
                    ngx_http_upstream_finalize_request(real_r, real_u,
...
void
ngx_http_multi_upstream_init_request(ngx_connection_t   *pc,
                                     ngx_http_request_t *r)
{
    ngx_http_upstream_t *u;
    ...
    {
        u->send_pool =
            ngx_create_pool(NGX_DEFAULT_POOL_SIZE, r->connection->log);
        if (u->send_pool == NULL)
        {
            return;
        }
...
ngx_http_multi_upstream_connect_init(ngx_connection_t *pc)
{
    ...
    for (q = ngx_queue_head(data); q != ngx_queue_sentinel(data);
         q = ngx_queue_next(q))
    {
