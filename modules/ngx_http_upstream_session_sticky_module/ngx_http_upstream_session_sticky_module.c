...
static void ngx_http_session_sticky_tmtoa(ngx_http_request_t *r, ngx_str_t *str,
                                          time_t t);
...
static char     *ngx_http_session_sticky_hide_cookie(ngx_conf_t    *cf,
                                                     ngx_command_t *cmd,
                                                     void          *conf);
static ngx_int_t ngx_http_upstream_session_sticky_init_upstream(
...
static ngx_command_t ngx_http_session_sticky_commands[] = {
    ...
    {ngx_string("session_sticky"),
     ...
     ngx_http_upstream_session_sticky, NGX_HTTP_SRV_CONF_OFFSET, 0, NULL},

    {ngx_string("session_sticky_hide_cookie"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_http_session_sticky_hide_cookie, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
                                           ...
                                           ngx_http_upstream_srv_conf_t *us)
{
    ...
    sscf = ngx_http_conf_upstream_srv_conf(
        us, ngx_http_upstream_session_sticky_module);
...
ngx_http_session_sticky_header_handler(ngx_http_request_t *r)
{
    ...
    slcf = ngx_http_get_module_loc_conf(
        r, ngx_http_upstream_session_sticky_module);
    ...
    uscf = slcf->uscf;
    sscf = ngx_http_conf_upstream_srv_conf(
        uscf, ngx_http_upstream_session_sticky_module);
    if (sscf != NULL && (sscf->flag & NGX_HTTP_SESSION_STICKY_REWRITE))
    {
        return NGX_DECLINED;
    }
        ...
        for (p = v; p < vv; p++)
        {
            if (*p == NGX_HTTP_SESSION_STICKY_DELIMITER)
            {
                ...
                else if (delimiter == 2)
                {
                    ...
                    ctx->s_lastseen.data =
                        ngx_pnalloc(r->pool, ctx->s_lastseen.len);
...
ngx_http_upstream_session_sticky_get_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (ctx->sid.len == server[i].sid.len
            && ngx_strncmp(ctx->sid.data, server[i].sid.data, ctx->sid.len)
                   == 0)
        {
#if (NGX_HTTP_UPSTREAM_CHECK)
...
failed:
    ...
    for (i = 0; i < n; i++)
    {
        if (server[i].name->len == pc->name->len
            && ngx_strncmp(server[i].name->data, pc->name->data, pc->name->len)
                   == 0)
        {
            ctx->sid.len  = server[i].sid.len;
            ctx->sid.data = server[i].sid.data;
            ...
            for (i = 0; i < part->nelts; i++)
            {
                if (table[i].key.len == (sizeof("set-cookie") - 1)
                    && ngx_strncasecmp(table[i].key.data,
                                       (u_char *)"set-cookie", table[i].key.len)
                           == 0)
                {
...
ngx_http_session_sticky_prefix(ngx_http_request_t *r, ngx_table_elt_t *table)
{
    ...
    p   = ngx_strlcasestrn(table->value.data,
                           ...
                           ctx->sscf->cookie.data, ctx->sscf->cookie.len - 1);
    if (p == NULL)
    {
        return NGX_AGAIN;
    }
...
success:
    ...
    en               = p;
    table->value.len = table->value.len - (en - st) + ctx->sscf->cookie.len
                       + 1 /* '=' */
                       + ctx->sid.len;
...
ngx_http_session_sticky_insert(ngx_http_request_t *r)
{
    ...
    set_cookie->value.len = ctx->sscf->cookie.len + sizeof("=") - 1
                            + ctx->sid.len + sizeof("; Domain=") - 1
                            + ctx->sscf->domain.len + sizeof("; Path=") - 1
                            + ctx->sscf->path.len;

    if (ctx->sscf->maxidle != NGX_CONF_UNSET)
    {
        set_cookie->value.len = set_cookie->value.len + ctx->s_lastseen.len
                                + ctx->s_firstseen.len + 2; /* '|' and '|' */
    }
    else
    {
        set_cookie->value.len = set_cookie->value.len + sizeof("; Max-Age=") - 1
                                + ctx->sscf->maxage.len + sizeof("; Expires=")
                                - 1 + sizeof("Xxx, 00-Xxx-00 00:00:00 GMT") - 1;
    }
    ...
    *p++ = '=';
    ...
    if (ctx->sscf->maxidle == NGX_CONF_UNSET && ctx->sscf->maxage.len)
    {
        ...
        ngx_uint_t maxage =
            ngx_atoi(ctx->sscf->maxage.data, ctx->sscf->maxage.len);
...
static char *
ngx_http_upstream_session_sticky(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_int_t                        rc;
    ngx_uint_t                       i;
    ngx_str_t                       *value;
                                    ...
                                    void *conf)
{
    ...
    {
        ngx_conf_deprecated(cf, &ngx_conf_deprecated_session_sticky_header,
                            NULL);
    }
    ...
    if (ngx_strncmp(value[1].data, "upstream=", 9) == 0)
    {
        ...
        if (slcf->uscf == NULL)
        {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid upstream name");
            return NGX_CONF_ERROR;
        }
        return NGX_CONF_OK;
    }
                                               ...
                                               ngx_http_upstream_srv_conf_t *us)
{
    ...
    for (peer = peers->peer, i = 0; peer; peer = peer->next, i++)
    {
        ...
        else if (ngx_http_upstream_session_sticky_set_sid(cf, &sscf->server[i])
                 != NGX_OK)
        {
            return NGX_ERROR;
        }
