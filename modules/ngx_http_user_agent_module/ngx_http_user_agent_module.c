...
static ngx_command_t ngx_http_user_agent_commands[] = {
    ...
    {ngx_string("user_agent"),
     ...
     ngx_http_user_agent_block, NGX_HTTP_MAIN_CONF_OFFSET, 0, NULL},
...
ngx_http_user_agent(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (nelts <= 1)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid first parameter");
        return NGX_CONF_ERROR;
    }
    ...
    if (nelts == 2)
    {
        ...
        if (ngx_strcmp(args[0].data, "default") == 0)
        {
            ...
            ctx->default_value =
                ngx_pcalloc(ctx->pool, sizeof(ngx_http_variable_t));
            if (ctx->default_value == NULL)
            {
                return NGX_CONF_ERROR;
            }
    ...
    if (nelts == 2)
    {
        ...
        interval =
            ngx_pcalloc(ctx->pool, sizeof(ngx_http_user_agent_interval_t));
        if (interval == NULL)
        {
            return NGX_CONF_ERROR;
        }
        ...
        interval->var =
            ngx_pcalloc(ctx->pool, sizeof(ngx_http_variable_value_t));
        if (interval->var == NULL)
        {
            return NGX_CONF_ERROR;
        }
                             ...
                             ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    {
        if (user_agent->data[pos] >= '0' && user_agent->data[pos] <= '9')
        {
            break;
        }
        else if (user_agent->data[pos] == ';' || user_agent->data[pos] == '('
                 || user_agent->data[pos] == ')')
        {
            break;
        }
    ...
    for (/* void */; pos < (ngx_int_t)user_agent->len; pos++)
    {
        ...
        else if (user_agent->data[pos] >= '0' && user_agent->data[pos] <= '9')
        {
