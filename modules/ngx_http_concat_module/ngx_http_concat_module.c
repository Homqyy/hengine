...
static ngx_int_t ngx_http_concat_add_path(ngx_http_request_t *r,
                                          ngx_array_t *uris, size_t max,
                                          ngx_str_t *path, u_char *p,
                                          u_char *v);
...
static ngx_str_t ngx_http_concat_default_types[] = {
    ngx_string("application/javascript"), ngx_string("text/css"),
    ngx_null_string};
...
static ngx_command_t ngx_http_concat_commands[] = {
    ...
    {ngx_string("concat"),
     ...
     offsetof(ngx_http_concat_loc_conf_t, enable), NULL},

    {ngx_string("concat_max_files"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_concat_loc_conf_t, max_files), NULL},

    {ngx_string("concat_unique"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_concat_loc_conf_t, unique), NULL},
    ...
    {ngx_string("concat_delimiter"),
     ...
     offsetof(ngx_http_concat_loc_conf_t, delimiter), NULL},

    {ngx_string("concat_ignore_file_error"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_concat_loc_conf_t, ignore_file_error), NULL},
...
ngx_http_concat_handler(ngx_http_request_t *r)
{
    ...
    for (p = r->args.data + 1, v = p, timestamp = 0; p != e; p++)
    {
        if (*p == ',')
        {
            ...
            rc = ngx_http_concat_add_path(r, &uris, clcf->max_files, &path, p,
                                          v);
            if (rc != NGX_OK)
            {
                return rc;
...
static ngx_int_t
ngx_http_concat_add_path(ngx_http_request_t *r, ngx_array_t *uris, size_t max,
                         ngx_str_t *path, u_char *p, u_char *v)
{
    u_char    *d;
    ngx_str_t *uri, args;
    ngx_uint_t flags;
