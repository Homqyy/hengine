...
static ngx_int_t
                 ngx_http_proxy_connect_set_local(ngx_http_request_t                *r,
                                                  ngx_http_proxy_connect_upstream_t *u,
                                                  ngx_http_proxy_connect_address_t  *local);
static ngx_int_t ngx_http_proxy_connect_variable_get_time(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static void ngx_http_proxy_connect_variable_set_time(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t
...
static ngx_command_t ngx_http_proxy_connect_commands[] = {
    ...
    {ngx_string("proxy_connect_read_timeout"),
     ...
     offsetof(ngx_http_proxy_connect_loc_conf_t, read_timeout), NULL},

    {ngx_string("proxy_connect_send_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_connect_loc_conf_t, send_timeout), NULL},

    {ngx_string("proxy_connect_connect_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_connect_loc_conf_t, connect_timeout), NULL},

    {ngx_string("proxy_connect_send_lowat"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_connect_loc_conf_t, send_lowat), NULL},

    {ngx_string("proxy_connect_address"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE1,
     ngx_http_set_complex_value_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_connect_loc_conf_t, address), NULL},

    {ngx_string("proxy_connect_bind"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_CONF_TAKE12,
     ngx_http_proxy_connect_bind, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_connect_loc_conf_t, local), NULL},
...
static ngx_http_variable_t ngx_http_proxy_connect_vars[] = {
    ...
    {ngx_string("connect_addr"), NULL,
     ngx_http_proxy_connect_connect_addr_variable, 0, NGX_HTTP_VAR_NOCACHEABLE,
     0},
...
ngx_http_proxy_connect_test_connect(ngx_connection_t *c)
{
    ...
    if (ngx_event_flags & NGX_USE_KQUEUE_EVENT)
    {
        if (c->write->pending_eof || c->read->pending_eof)
        {
            ...
            (void)ngx_connection_error(
                c, err, "proxy_connet: upstream connect failed (kevent)");
            return NGX_ERROR;
        }
    }
    else
#endif
    {
        ...
        if (err)
        {
            c->log->action = "connecting to upstream";
            (void)ngx_connection_error(
                c, err, "proxy_connect: upstream connect failed");
            return NGX_ERROR;
        }
    }
...
static void
ngx_http_proxy_connect_tunnel(ngx_http_request_t *r, ngx_uint_t from_upstream,
                              ngx_uint_t do_write)
{
    size_t                             size;
    ssize_t                            n;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "proxy_connect: tunnel fu:%ui write:%ui", from_upstream,
                   do_write);
...
ngx_http_proxy_connect_handler(ngx_http_request_t *r)
{
    ...
    if (ngx_parse_url(r->pool, &url) != NGX_OK)
    {
        ...
        {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                          "proxy_connect: %s in connect host \"%V\"", url.err,
                          &url.url);
    ...
    else if (plcf->allow_ports)
    {
        ...
        for (i = 0; i < plcf->allow_ports->nelts; i++)
        {
            ...
            if ((ports[i][1] == 0 && r->connect_port_n == ports[i][0])
                || (ports[i][0] <= r->connect_port_n
                    && r->connect_port_n <= ports[i][1]))
            {
                allow = 1;
                break;
            }
...
ngx_http_proxy_connect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    pclcf =
        ngx_http_conf_get_module_loc_conf(cf, ngx_http_proxy_connect_module);
...
char *
ngx_http_proxy_connect_bind(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    char *p = conf;
                                         ...
                                         uintptr_t                  data)
{
    ...
    {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "proxy_connect: invalid msec \"%V\" (ctx offset=%ui)", &s,
                      data);
        return;
    }
...
ngx_http_proxy_connect_post_read_handler(ngx_http_request_t *r)
{
    ...
    if (r->method == NGX_HTTP_CONNECT)
    {
        ...
        ctx->buf.last =
            ctx->buf.pos + sizeof(NGX_HTTP_PROXY_CONNECT_ESTABLISTHED) - 1;
        ctx->buf.memory = 1;
