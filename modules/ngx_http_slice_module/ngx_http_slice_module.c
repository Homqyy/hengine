...
static ngx_command_t ngx_http_slice_commands[] = {
    ...
    {ngx_string("slice"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS, ngx_http_slice,
     0, 0, NULL},
    ...
    {ngx_string("slice_arg_begin"),
     ...
     offsetof(ngx_http_slice_loc_conf_t, begin), NULL},

    {ngx_string("slice_arg_end"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_slice_loc_conf_t, end), NULL},

    {ngx_string("slice_header"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_slice_loc_conf_t, header), NULL},

    {ngx_string("slice_footer"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_slice_loc_conf_t, footer), NULL},

    {ngx_string("slice_header_first"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_slice_loc_conf_t, header_first), NULL},

    {ngx_string("slice_footer_last"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_slice_loc_conf_t, footer_last), NULL},
...
ngx_http_slice_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "http slice filename: \"%V\"",
                   &path);
    ...
    len = (end == begin)
              ...
              : ((end - begin)
                 ...
                 + ((end == of.size && slcf->footer_last) ? slcf->footer.len
                                                          : 0));
     ...
     */
    if (slcf->header.len && !(begin == 0 && !slcf->header_first))
    {
     ...
     */
    if (slcf->footer.len && !(end == of.size && !slcf->footer_last))
    {
        b = ngx_pcalloc(r->pool, sizeof(ngx_buf_t));
        if (b == NULL)
        {
