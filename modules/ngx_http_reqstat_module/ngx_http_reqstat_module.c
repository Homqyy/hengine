...
off_t ngx_http_reqstat_fields[29] = {NGX_HTTP_REQSTAT_BYTES_IN,
...
static char *ngx_http_reqstat_zone_recycle(ngx_conf_t *cf, ngx_command_t *cmd,
                                           void *conf);
static char *ngx_http_reqstat(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static void  ngx_http_reqstat_count(void *data, off_t offset, ngx_int_t incr);
...
static ngx_command_t ngx_http_reqstat_commands[] = {
    ...
    {ngx_string("req_status_zone"), NGX_HTTP_MAIN_CONF | NGX_CONF_2MORE,
     ngx_http_reqstat_zone, 0, 0, NULL},

    {ngx_string("req_status"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_1MORE,
     ngx_http_reqstat, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("req_status_lazy"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_reqstat_conf_t, lazy), NULL},

    {ngx_string("req_status_bypass"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF
         | NGX_CONF_1MORE,
     ngx_http_set_predicate_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_reqstat_conf_t, bypass), NULL},

    {ngx_string("req_status_show"), NGX_HTTP_LOC_CONF | NGX_CONF_ANY,
     ngx_http_reqstat_show, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("req_status_show_field"), NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
     ngx_http_reqstat_show_field, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("req_status_zone_add_indicator"),
     NGX_HTTP_MAIN_CONF | NGX_CONF_2MORE, ngx_http_reqstat_zone_add_indicator,
     0, 0, NULL},

    {ngx_string("req_status_zone_key_length"),
     NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE2, ngx_http_reqstat_zone_key_length, 0,
     0, NULL},

    {ngx_string("req_status_zone_recycle"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE3,
     ngx_http_reqstat_zone_recycle, 0, 0, NULL},
...
static char *
ngx_http_reqstat_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_ptr_value(conf->user_defined_str, prev->user_defined_str,
                             NULL);
...
ngx_http_reqstat_show_field(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    rlcf->user_select =
        ngx_array_create(cf->pool, cf->args->nelts - 1, sizeof(ngx_int_t));
    ...
    for (i = 1; i < cf->args->nelts; i++)
    {
        ...
        if (!valid)
        {
            ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                          "field \"%V\" does not exist", &value[i]);
...
ngx_http_reqstat_show(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        shm_zone =
            ngx_shared_memory_add(cf, &value[i], 0, &ngx_http_reqstat_module);
        if (shm_zone == NULL)
        {
            return NGX_CONF_ERROR;
        }
                                    ...
                                    void *conf)
{
    ...
    shm_zone =
        ngx_shared_memory_add(cf, &value[1], 0, &ngx_http_reqstat_module);
    if (shm_zone == NULL)
    {
        return NGX_CONF_ERROR;
    }
    ...
    if (shm_zone->data == NULL)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "zone \"%V\" should be defined first", &value[1]);
        return NGX_CONF_ERROR;
    }
    ...
    ctx->user_defined =
        ngx_array_create(cf->pool, cf->args->nelts - 2, sizeof(ngx_int_t));
    if (ctx->user_defined == NULL)
    {
        return NGX_CONF_ERROR;
    }

    rlcf->user_defined_str =
        ngx_array_create(cf->pool, cf->args->nelts - 2, sizeof(ngx_str_t));
    ...
    for (j = 2; j < cf->args->nelts; j++)
    {
        ...
        *i = ngx_http_get_variable_index(cf, &value[j]);
        ...
        {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "failed to obtain variable \"%V\"", &value[j]);
            return NGX_CONF_ERROR;
        }
    }
...
static char *
ngx_http_reqstat_zone_key_length(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t              *value;
    ngx_shm_zone_t         *shm_zone;
    ngx_http_reqstat_ctx_t *ctx;
    ...
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid key length");
        return NGX_CONF_ERROR;
    }
...
static char *
ngx_http_reqstat_zone_recycle(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (rate == NGX_ERROR)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid threshold");
        return NGX_CONF_ERROR;
    }
    ...
    if (scale == NGX_ERROR)
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid scale");
        return NGX_CONF_ERROR;
    }
...
ngx_http_reqstat_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "the value \"%V\" is a constant", &value[2]);
        return NGX_CONF_ERROR;
    }
    ...
    *ctx->val = value[2];
    ...
    shm_zone =
        ngx_shared_memory_add(cf, &value[1], size, &ngx_http_reqstat_module);
    if (shm_zone == NULL)
    {
        return NGX_CONF_ERROR;
    }
...
ngx_http_reqstat_log_handler(ngx_http_request_t *r)
{
    ...
    for (i = 0; i < store->monitor_index.nelts; i++)
    {
        ...
        if (r->upstream_states != NULL && r->upstream_states->nelts > 0)
        {
            ...
            ngx_http_reqstat_count(fnode, NGX_HTTP_REQSTAT_UPS_RT, total_ms);
            ngx_http_reqstat_count(fnode, NGX_HTTP_REQSTAT_UPS_TRIES, utries);
        }
...
ngx_http_reqstat_show_handler(ngx_http_request_t *r)
{
    ...
    for (i = 0; i < display->nelts; i++)
    {
        ...
        for (q = ngx_queue_head(&ctx->sh->queue);
             q != ngx_queue_sentinel(&ctx->sh->queue); q = ngx_queue_next(q))
        {
            ...
            b->last = ngx_slprintf(b->last, b->end, "%*s,", (size_t)node->len,
                                   node->data);
            if (rlcf->user_select)
            {
                ...
                for (j = 0; j < rlcf->user_select->nelts; j++)
                {
                    if (user[j] < NGX_HTTP_REQSTAT_RSRV)
                    {
                        ...
                        b->last = ngx_slprintf(
                            b->last, b->end, "%uA,",
                            *NGX_HTTP_REQSTAT_REQ_FIELD(
                                node, ngx_http_reqstat_fields[index]));
                    }
                    else
                    {
                        ...
                        b->last = ngx_slprintf(
                            b->last, b->end, "%uA,",
                            *NGX_HTTP_REQSTAT_REQ_FIELD(
                                node, NGX_HTTP_REQSTAT_EXTRA(index)));
                    }
                }
            }
            else
            {
                ...
                {
                    b->last =
                        ngx_slprintf(b->last, b->end, "%uA,",
                                     *NGX_HTTP_REQSTAT_REQ_FIELD(
                                         node, ngx_http_reqstat_fields[j]));
                }
...
ngx_http_reqstat_rbtree_lookup(ngx_shm_zone_t *shm_zone, ngx_str_t *val)
{
    ...
    while (node != sentinel)
    {
        ...
        if (rc == 0)
        {
            ...
            rs->excess =
                rs->excess - ngx_abs(ms) * ctx->recycle_rate / 1000 + 1000;
            rs->last_visit = now;
    ...
    size = offsetof(ngx_rbtree_node_t, color)
           + offsetof(ngx_http_reqstat_rbnode_t, data) + ctx->key_len;
    ...
    if (node == NULL)
    {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_CORE, shm_zone->shm.log, 0,
                       "reqstat lookup try recycle: %*s, %d", rs->len, rs->data,
                       excess);
        ...
        if (excess < 0)
        {
            ...
            node = (ngx_rbtree_node_t *)((char *)rs
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, shm_zone->shm.log, 0,
                   "reqstat lookup build: %*s", rs->len, rs->data);
...
ngx_http_reqstat_input_body_filter(ngx_http_request_t *r, ngx_buf_t *buf)
{
    ...
    case NGX_DECLINED:
    case NGX_AGAIN: return ngx_http_next_input_body_filter(r, buf);
                              ...
                              ngx_http_reqstat_conf_t *rlcf)
{
    ...
    {
    case NGX_ERROR: return NULL;

    case NGX_DECLINED: store->bypass = 1; return store;

    default: /* NGX_OK */ break;
    }

    if (ngx_array_init(&store->monitor_index, r->pool, rlcf->monitor->nelts,
                       sizeof(ngx_http_reqstat_rbnode_t *))
        == NGX_ERROR)
    {
        return NULL;
    }
