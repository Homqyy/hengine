...
static void ngx_http_upstream_init_virtual_peers(
    ...
    ngx_http_upstream_vnswrr_srv_conf_t *uvnscf, ngx_uint_t s, ngx_uint_t e);


static ngx_command_t ngx_http_upstream_vnswrr_commands[] = {

    {ngx_string("vnswrr"), NGX_HTTP_UPS_CONF | NGX_CONF_NOARGS,
     ngx_http_upstream_vnswrr, 0, 0, NULL},
...
ngx_http_upstream_vnswrr(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    uscf->flags = NGX_HTTP_UPSTREAM_CREATE | NGX_HTTP_UPSTREAM_WEIGHT
                  | NGX_HTTP_UPSTREAM_BACKUP | NGX_HTTP_UPSTREAM_MAX_FAILS
...
ngx_http_upstream_init_vnswrr(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
    ...
    uvnscf =
        ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_vnswrr_module);
    if (uvnscf == NULL)
    {
        return NGX_ERROR;
    }
    ...
    {
        uvnscf->vpeers =
            ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_rr_vpeers_t)
                                      * peers->total_weight);
    ...
    {
        ubvnscf =
            ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_vnswrr_srv_conf_t));
        if (ubvnscf == NULL)
        {
            return NGX_ERROR;
        }
...
ngx_http_upstream_get_vnswrr_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "get vnswrr peer, try: %ui",
                   pc->tries);
    ...
    else
    {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                       "get vnswrr peer, current: %p %i", peer,
                       peer->current_weight);
    }
...
ngx_http_upstream_get_vnswrr(ngx_http_upstream_vnswrr_peer_data_t *vnsp)
{
    ...
    for (i = begin_number, flag = 1; i != begin_number || flag;
         i   = peers->weighted ? ((i + 1) % uvnscf->vnumber)
                               : ((i + 1) % peers->number),
        peer = peers->weighted ? vpeers[i].vpeer
                               : (peer->next ? peer->next : peers->peer))
    {
        flag = 0;
        if (peers->weighted)
        {
