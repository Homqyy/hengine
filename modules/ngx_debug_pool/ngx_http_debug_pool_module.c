...
static char     *ngx_http_debug_pool(ngx_conf_t *cf, ngx_command_t *cmd,
                                     void *conf);
static ngx_int_t ngx_http_debug_pool_buf(ngx_pool_t *pool, ngx_buf_t *b);

static ngx_command_t ngx_http_debug_pool_commands[] = {

    {ngx_string("debug_pool"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS,
     ngx_http_debug_pool, 0, 0, NULL},
...
#define NGX_POOL_PID_SIZE \
    (NGX_TIME_T_LEN + sizeof("pid:\n") - 1) /* sizeof pid_t equals time_t */
#define NGX_POOL_PID_FORMAT "pid:%P\n"
#define NGX_POOL_ENTRY_SIZE \
    (48 /* func */ + 12 * 4 + sizeof("size: num: cnum: lnum: \n") - 1)
#define NGX_POOL_ENTRY_FORMAT "size:%12z num:%12z cnum:%12z lnum:%12z %s\n"
#define NGX_POOL_SUMMARY_SIZE \
    (12 * 4 + sizeof("size: num: cnum: lnum: [SUMMARY]\n") - 1)
#define NGX_POOL_SUMMARY_FORMAT \
    "size:%10z%2s num:%12z cnum:%12z lnum:%12z [SUMMARY]\n"
    ...
    p = ngx_snprintf(p, NGX_POOL_SUMMARY_SIZE, NGX_POOL_SUMMARY_FORMAT, s, unit,
                     n, cn, ln);
