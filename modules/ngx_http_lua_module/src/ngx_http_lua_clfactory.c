...
#define MAX_BEGIN_CODE_SIZE                                         \
    (POS_BYTECODE + LUA_LITTLE_ENDIAN_8BYTES_CODE_LEN + sizeof(int) \
     + sizeof(int))
...
ngx_http_lua_clfactory_bytecode_prepare(lua_State                         *L,
                                        ...
                                        int fname_index)
{
    int x = 1, size_of_int, size_of_size_t, little_endian, size_of_inst,
        version, stripped;
    ...
    *lf->begin_code.str = LUA_SIGNATURE[0];
    ...
    else
    {
        ...
        if (ngx_memcmp(lf->begin_code.str, LUA_SIGNATURE,
            ...
            || version != LUAC_VERSION || little_endian != (int)(*(char *)&x)
            || size_of_int != sizeof(int) || size_of_size_t != sizeof(size_t)
...
ngx_http_lua_clfactory_loadfile(lua_State *L, const char *filename)
{
    ...
    ungetc(c, lf.f);
#endif
    status = lua_load(L, ngx_http_lua_clfactory_getF, &lf, lua_tostring(L, -1));
...
ngx_int_t
ngx_http_lua_clfactory_loadbuffer(lua_State *L, const char *buff, size_t size,
                                  const char *name)
{
