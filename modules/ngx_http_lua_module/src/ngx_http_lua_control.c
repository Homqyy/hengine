...
ngx_http_lua_ngx_exec(lua_State *L)
{
    ...
    if (n == 2)
    {
        ...
        case LUA_TNIL: ngx_str_null(&user_args); break;

        default:
            msg = lua_pushfstring(L,
                                  "string, number, or table expected, "
                                  "but got %s",
                                  luaL_typename(L, 2));
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "lua exec \"%V?%V\"", &ctx->exec_uri, &ctx->exec_args);
...
ngx_http_lua_ngx_redirect(lua_State *L)
{
    ...
    if (n == 2)
    {
        ...
        if (rc != NGX_HTTP_MOVED_TEMPORARILY && rc != NGX_HTTP_MOVED_PERMANENTLY
            && rc != NGX_HTTP_SEE_OTHER && rc != NGX_HTTP_PERMANENT_REDIRECT
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "lua redirect to \"%V\" with code %i", &h->value,
                   ctx->exit_code);
...
ngx_http_lua_ngx_exit(lua_State *L)
{
    ...
    ngx_http_lua_check_context(
        ...
        NGX_HTTP_LUA_CONTEXT_REWRITE | NGX_HTTP_LUA_CONTEXT_ACCESS
            ...
            | NGX_HTTP_LUA_CONTEXT_HEADER_FILTER | NGX_HTTP_LUA_CONTEXT_BALANCER
    ...
    if (ctx->context
        & (NGX_HTTP_LUA_CONTEXT_SSL_CERT | NGX_HTTP_LUA_CONTEXT_SSL_SESS_STORE
           | NGX_HTTP_LUA_CONTEXT_SSL_SESS_FETCH))
    {
#if (NGX_HTTP_SSL)
    ...
    if (ctx->no_abort && rc != NGX_ERROR && rc != NGX_HTTP_CLOSE
    ...
    if ((r->header_sent || ctx->header_sent) && rc >= NGX_HTTP_SPECIAL_RESPONSE
        && rc != NGX_HTTP_REQUEST_TIME_OUT
        && rc != NGX_HTTP_CLIENT_CLOSED_REQUEST && rc != NGX_HTTP_CLOSE)
    {
                      ...
                      size_t *errlen)
{
    ...
    if (ngx_http_lua_ffi_check_context(
            ctx,
            NGX_HTTP_LUA_CONTEXT_REWRITE | NGX_HTTP_LUA_CONTEXT_ACCESS
                | NGX_HTTP_LUA_CONTEXT_CONTENT | NGX_HTTP_LUA_CONTEXT_TIMER
                | NGX_HTTP_LUA_CONTEXT_HEADER_FILTER
                | NGX_HTTP_LUA_CONTEXT_BALANCER | NGX_HTTP_LUA_CONTEXT_SSL_CERT
                | NGX_HTTP_LUA_CONTEXT_SSL_SESS_STORE
                | NGX_HTTP_LUA_CONTEXT_SSL_SESS_FETCH,
            err, errlen)
        != NGX_OK)
    ...
    if (ctx->no_abort && status != NGX_ERROR && status != NGX_HTTP_CLOSE
    ...
    if ((r->header_sent || ctx->header_sent)
        ...
        && status != NGX_HTTP_REQUEST_TIME_OUT
        && status != NGX_HTTP_CLIENT_CLOSED_REQUEST && status != NGX_HTTP_CLOSE)
    {
