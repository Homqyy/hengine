...
ngx_http_lua_body_filter_inline(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    rc = ngx_http_lua_cache_loadbuffer(
        r->connection->log, L, llcf->body_filter_src.value.data,
...
ngx_http_lua_body_filter_file(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ngx_http_complex_value(r, &llcf->body_filter_src, &eval_src) != NGX_OK)
    {
        return NGX_ERROR;
    }

    script_path =
        ngx_http_lua_rebase_path(r->pool, eval_src.data, eval_src.len);
                                   ...
                                   ngx_http_lua_ctx_t *ctx)
{
    ...
    {
    case LUA_TSTRING:
    case LUA_TNUMBER: data = (u_char *)lua_tolstring(L, 3, &size); break;
    ...
    default:
        return luaL_error(L, "bad chunk data type: %s", lua_typename(L, type));
    }
