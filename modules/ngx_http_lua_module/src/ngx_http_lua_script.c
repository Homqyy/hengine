...
ngx_http_lua_compile_complex_value(ngx_http_lua_compile_complex_value_t *ccv)
{
    ...
    n = nv
            * (2 * sizeof(ngx_http_lua_script_copy_code_t)
                                  ...
                                  ngx_str_t *value, ngx_uint_t last)
{
    ...
    code = ngx_http_lua_script_add_code(
        *sc->lengths, sizeof(ngx_http_lua_script_copy_code_t));
    if (code == NULL)
    {
        return NGX_ERROR;
    }
    ...
    code->code =
        (ngx_http_lua_script_code_pt)(void *)ngx_http_lua_script_copy_len_code;
    code->len = len;

    size =
        (sizeof(ngx_http_lua_script_copy_code_t) + len + sizeof(uintptr_t) - 1)
        & ~(sizeof(uintptr_t) - 1);
...
ngx_http_lua_script_copy_code(ngx_http_lua_script_engine_t *e)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, e->log, 0, "lua script copy: \"%*s\"",
                   e->pos - p, p);
}
                                     ...
                                     ngx_uint_t                     n)
{
    ...
    code = ngx_http_lua_script_add_code(
        *sc->lengths, sizeof(ngx_http_lua_script_capture_code_t));
    if (code == NULL)
    {
        return NGX_ERROR;
    }
    ...
    code = ngx_http_lua_script_add_code(
        *sc->values, sizeof(ngx_http_lua_script_capture_code_t));
    if (code == NULL)
    {
        return NGX_ERROR;
    }
...
ngx_http_lua_script_copy_capture_code(ngx_http_lua_script_engine_t *e)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, e->log, 0, "lua script capture: \"%*s\"",
                   e->pos - pos, pos);
}
