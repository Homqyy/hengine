...
ngx_http_lua_ngx_echo(lua_State *L, unsigned newline)
{
    ...
    for (i = 1; i <= nargs; i++)
    {
        ...
        case LUA_TTABLE:
            ...
            size += ngx_http_lua_calc_strlen_in_table(L, i, i, 0 /* strict */);
            break;
...
ngx_http_lua_copy_str_in_table(lua_State *L, int index, u_char *dst)
{
    ...
    for (i = 1; i <= max; i++)
    {
        ...
        default: luaL_error(L, "impossible to reach here"); return NULL;
        }
...
ngx_http_lua_ngx_flush(lua_State *L)
{
    ...
    {
        return luaL_error(L,
                          "attempt to pass %d arguments, but accepted 0 "
                          "or 1",
                          n);
    }
    ...
    dd("wait:%d, rc:%d, buffered:0x%x", wait, (int)rc, r->connection->buffered);
            ...
            || wev->delayed))
    {
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       ...
                       "delayed:%d",
                       (unsigned)r->connection->buffered, wev->delayed);
...
ngx_http_lua_ngx_eof(lua_State *L)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "lua send eof");
