 ...
 * */
static ngx_int_t
ngx_http_lua_cache_load_code(ngx_log_t *log, lua_State *L, const char *key)
{
    ...
    if (lua_isfunction(L, -1))
    {
        ...
        ngx_log_error(NGX_LOG_ERR, log, 0,
                      "lua: failed to run factory at key \"%s\": %s", key, err);
        lua_pop(L, 2);
        return NGX_ERROR;
...
ngx_int_t
ngx_http_lua_cache_loadbuffer(ngx_log_t *log, lua_State *L, const u_char *src,
                              size_t src_len, const u_char *cache_key,
                              const char *name)
{
    int         n;
    ngx_int_t   rc;
...
error:
    ...
    ngx_log_error(NGX_LOG_ERR, log, 0, "failed to load inlined Lua code: %s",
                  err);
    lua_settop(L, n);
    return NGX_ERROR;
}
...
ngx_int_t
ngx_http_lua_cache_loadfile(ngx_log_t *log, lua_State *L, const u_char *script,
