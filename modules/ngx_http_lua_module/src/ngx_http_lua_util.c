...
static void ngx_http_lua_set_path(ngx_cycle_t *cycle, lua_State *L, int tab_idx,
                                  const char *fieldname, const char *path,
...
static ngx_int_t ngx_http_lua_post_zombie_thread(ngx_http_request_t    *r,
                                                 ...
                                                 ngx_http_lua_co_ctx_t *thread);
static void
ngx_http_lua_cleanup_zombie_child_uthreads(ngx_http_request_t *r, lua_State *L,
                                           ngx_http_lua_ctx_t    *ctx,
                                           ngx_http_lua_co_ctx_t *coctx);
...
ngx_http_lua_new_thread(ngx_http_request_t *r, lua_State *L, int *ref)
{
    ...
    lua_pushlightuserdata(L, ngx_http_lua_lightudata_mask(coroutines_key));
                                   ...
                                   ngx_http_lua_ctx_t *ctx)
{
    ...
    if (!r->header_sent && !ctx->header_sent)
    {
        ...
        if (!ctx->mime_set && ngx_http_lua_set_content_type(r, ctx) != NGX_OK)
        {
            return NGX_ERROR;
        }
                             ...
                             ngx_chain_t *in)
{
    ...
    if (llcf->http10_buffering && !ctx->buffering && !r->header_sent
        && !ctx->header_sent && r->http_version < NGX_HTTP_VERSION_11
...
static ngx_int_t
ngx_http_lua_send_http10_headers(ngx_http_request_t *r, ngx_http_lua_ctx_t *ctx)
{
    off_t        size;
    ngx_chain_t *cl;
    ngx_int_t    rc;
...
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, log, 0, "lua initializing lua registry");
    ...
    lua_pushlightuserdata(L, ngx_http_lua_lightudata_mask(regex_cache_key));
...
ngx_http_lua_init_globals(lua_State *L, ngx_cycle_t *cycle,
                          ngx_http_lua_main_conf_t *lmcf, ngx_log_t *log)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, log, 0, "lua initializing lua globals");
                            ...
                            ngx_log_t *log)
{
...
#ifdef OPENRESTY_LUAJIT
    {
        ...
        const char buf[] = "local ngx_log = ngx.log\n"
        ...
        if (rc != 0)
        {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                          "failed to load Lua code (%i): %s", rc,
                          lua_tostring(L, -1));
                            ...
                            ngx_int_t *eof)
{
    ...
    *eof = 0;
    ...
    while (in)
    {
        ...
        {
            b->last =
                ngx_copy(b->last, in->buf->pos, in->buf->last - in->buf->pos);
        }
...
ngx_http_lua_reset_ctx(ngx_http_request_t *r, lua_State *L,
                       ngx_http_lua_ctx_t *ctx)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "lua reset ctx");
                        ...
                        ngx_http_lua_ctx_t *ctx, volatile int nrets)
{
    ...
    NGX_LUA_EXCEPTION_TRY
    {
        ...
        for (;;)
        {
            ...
            dd("%p: saved co top: %d, nrets: %d, true top: %d", orig_coctx->co,
            ...
            case LUA_YIELD:
                ...
                dd("%p: saving curr top after yield: %d (co-op: %d)",
                   orig_coctx->co, (int)lua_gettop(orig_coctx->co),
                   (int)ctx->co_op);
            ...
            case LUA_ERRERR: err = "error handler error"; break;
    ...
    NGX_LUA_EXCEPTION_CATCH { dd("nginx execution restored"); }
...
no_parent:
    ...
    if (ctx->entered_content_phase && r->connection->fd != (ngx_socket_t)-1)
    {
        rc = ngx_http_lua_send_chain_link(r, ctx, NULL /* last_buf */);
...
ngx_http_lua_wev_handler(ngx_http_request_t *r)
{
    ...
    if (c->buffered & NGX_HTTP_LOWLEVEL_BUFFERED)
    {
        ...
        dd("flush pending output returned %d, c->error: %d", (int)rc, c->error);
                                  ...
                                  ngx_http_lua_ctx_t *ctx)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "lua flushing output: buffered 0x%uxd", c->buffered);
                         ...
                         ngx_http_lua_ctx_t *ctx)
{
    ...
    dd("internal redirect to %.*s", (int)ctx->exec_uri.len, ctx->exec_uri.data);
    ...
    dd("internal redirect returned %d when in content phase? "
       "%d",
       (int)rc, ctx->entered_content_phase);
                         ...
                         ngx_http_lua_ctx_t *ctx)
{
...
#if 1
    if (!r->header_sent && !ctx->header_sent && r->headers_out.status == 0
    ...
    if (ctx->buffering && r->headers_out.status && ctx->exit_code != NGX_ERROR
    ...
    if ((ctx->exit_code == NGX_OK && ctx->entered_content_phase)
        || (ctx->exit_code >= NGX_HTTP_OK
...
#if 1
    if ((r->header_sent || ctx->header_sent) && ctx->exit_code > NGX_OK
        && ctx->exit_code != NGX_HTTP_REQUEST_TIME_OUT
        && ctx->exit_code != NGX_HTTP_CLIENT_CLOSED_REQUEST
        && ctx->exit_code != NGX_HTTP_CLOSE)
    {
...
void
ngx_http_lua_process_args_option(ngx_http_request_t *r, lua_State *L, int table,
                                 ngx_str_t *args)
{
    u_char   *key;
    size_t    key_len;
    ...
    while (lua_next(L, table) != 0)
    {
        ...
        key_escape = 2
                     * ngx_http_lua_escape_uri(NULL, key, key_len,
        ...
        case LUA_TSTRING:
            ...
            total_escape += 2
                            * ngx_http_lua_escape_uri(NULL, value, value_len,
                                                      NGX_ESCAPE_URI_COMPONENT);
    ...
    while (lua_next(L, table) != 0)
    {
        ...
        case LUA_TBOOLEAN:
            if (lua_toboolean(L, -1))
            {
                if (total_escape)
                {
                    p = (u_char *)ngx_http_lua_escape_uri(
                        p, key, key_len, NGX_ESCAPE_URI_COMPONENT);
                }
                else
                {
                    dd("shortcut: no escape required");
            ...
            while (lua_next(L, -2) != 0)
            {
                ...
                else
                {
                    ...
                    *p++ = '=';
                    ...
                    {
                        p = (u_char *)ngx_http_lua_escape_uri(
                            p, value, value_len, NGX_ESCAPE_URI_COMPONENT);
                    }
                    else
                    {
                        p = ngx_copy(p, value, value_len);
        ...
        default: luaL_error(L, "should not reach here"); return;
        }
    ...
    {
        luaL_error(L, "buffer error: %d != %d", (int)(p - args->data),
                   (int)len);
        return;
    }
}
    ...
    if (*free)
    {
        ...
        *free    = cl->next;
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, log, 0,
                       ...
                       "because %uz >= %O, cl:%p, p:%p",
                       len, (off_t)(b->end - b->start), cl, b->start);
...
ngx_http_lua_param_get(lua_State *L)
{
    ...
    ngx_http_lua_check_context(
        L, ctx, NGX_HTTP_LUA_CONTEXT_SET | NGX_HTTP_LUA_CONTEXT_BODY_FILTER);
...
ngx_http_lua_create_co_ctx(ngx_http_request_t *r, ngx_http_lua_ctx_t *ctx)
{
    ...
    {
        ctx->user_co_ctx =
            ngx_list_create(r->pool, 4, sizeof(ngx_http_lua_co_ctx_t));
        if (ctx->user_co_ctx == NULL)
        {
            return NULL;
        }
...
static void
ngx_http_lua_finalize_threads(ngx_http_request_t *r, ngx_http_lua_ctx_t *ctx,
                              lua_State *L)
{
#ifdef NGX_LUA_USE_ASSERT
    int top;
            ...
            if (ref != LUA_NOREF)
            {
                ...
                if (!inited)
                {
                    lua_pushlightuserdata(
                        L, ngx_http_lua_lightudata_mask(coroutines_key));
                    lua_rawget(L, LUA_REGISTRYINDEX);
                    inited = 1;
                }
    ...
    if (ref != LUA_NOREF)
    {
        ...
        if (!inited)
        {
            lua_pushlightuserdata(L,
                                  ngx_http_lua_lightudata_mask(coroutines_key));
            lua_rawget(L, LUA_REGISTRYINDEX);
            inited = 1;
        }
...
ngx_http_lua_check_broken_connection(ngx_http_request_t *r, ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                   "http lua check client, write event:%d, \"%V\"", ev->write,
                   &r->uri);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, err, "http lua recv(): %d", n);
...
ngx_http_lua_finalize_fake_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "http lua finalize fake request: %d, a:%d, c:%d", rc,
                   r == c->data, r->main->count);
...
lua_State *
ngx_http_lua_init_vm(lua_State *parent_vm, ngx_cycle_t *cycle, ngx_pool_t *pool,
                     ngx_http_lua_main_conf_t *lmcf, ngx_log_t *log,
...
ngx_http_lua_create_fake_request(ngx_connection_t *c)
{
    ...
    dd("r pool allocated: %d",
       (int)(sizeof(ngx_http_lua_ctx_t) + sizeof(void *) * ngx_http_max_module
...
ngx_http_lua_cleanup_free(ngx_http_request_t *r, ngx_http_cleanup_pt *cleanup)
{
    ...
    cln = (ngx_http_cleanup_t *)((u_char *)cleanup
                                 - offsetof(ngx_http_cleanup_t, handler));

    dd("cln: %p, cln->handler: %p, &cln->handler: %p", cln, cln->handler,
       &cln->handler);
