...
static u_char *ngx_http_lua_log_init_worker_error(ngx_log_t *log, u_char *buf,
                                                  size_t len);
...
ngx_http_lua_init_worker(ngx_cycle_t *cycle)
{
    ...
    if (ngx_array_init(&fake_cycle->listening, cycle->pool,
                       cycle->listening.nelts || 1, sizeof(ngx_listening_t))
        != NGX_OK)
    {
        goto failed;
    }
    ...
    http_ctx.loc_conf =
        ngx_pcalloc(conf.pool, sizeof(void *) * ngx_http_max_module);
