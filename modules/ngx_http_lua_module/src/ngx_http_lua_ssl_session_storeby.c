                                           ...
                                           lua_State               *L)
{
    ...
    rc = ngx_http_lua_cache_loadbuffer(
        r->connection->log, L, lscf->srv.ssl_sess_store_src.data,
        lscf->srv.ssl_sess_store_src.len, lscf->srv.ssl_sess_store_src_key,
                                   ...
                                   void *conf)
{
    ...
    else
    {
        ...
        p = ngx_palloc(cf->pool, sizeof("ssl_session_store_by_lua")
...
int
ngx_http_lua_ffi_ssl_get_serialized_session(ngx_http_request_t *r, u_char *buf,
                                            char **err)
{
 ...
 * the memory allocation of buf should be handled externally. */
int
ngx_http_lua_ffi_ssl_get_session_id(ngx_http_request_t *r, u_char *buf,
                                    char **err)
{
    int                     id_len;
    u_char                 *id;
...
int
ngx_http_lua_ffi_ssl_get_session_id_size(ngx_http_request_t *r, char **err)
{
    ngx_ssl_conn_t         *ssl_conn;
    ngx_connection_t       *c;
    ngx_http_lua_ssl_ctx_t *cctx;
