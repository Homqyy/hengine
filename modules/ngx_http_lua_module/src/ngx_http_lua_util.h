                               ...
                               u_char *err, size_t *errlen)
{
    ...
    {
        *errlen =
            ngx_snprintf(err, *errlen, "API disabled in the context of %s",
                         ngx_http_lua_context_name((ctx)->context))
            - err;
...
u_char *ngx_http_lua_digest_hex(u_char *dest, const u_char *buf, int buf_len);
...
void ngx_http_lua_process_args_option(ngx_http_request_t *r, lua_State *L,
...
void ngx_http_lua_finalize_fake_request(ngx_http_request_t *r, ngx_int_t rc);
