...
static ndk_set_var_value_pt
           ngx_http_lookup_ndk_set_var_directive(u_char *name, size_t name_len);
...
ngx_http_lua_ndk_set_var_get(lua_State *L)
{
    ...
    {
        return luaL_error(L,
                          "ndk.set_var: directive \"%s\" not found "
...
ngx_http_lua_run_set_var_directive(lua_State *L)
{
    ...
    if (rc != NGX_OK)
    {
        return luaL_error(L, "calling directive %s failed with code %d", p,
                          (int)rc);
    }
...
int
ngx_http_lua_ffi_ndk_lookup_directive(const u_char *var_data, size_t var_len,
                                      ndk_set_var_value_pt *func)
{
...
ngx_http_lua_ffi_ndk_set_var_get(ngx_http_request_t  *r,
                                 ndk_set_var_value_pt func,
                                 const u_char *arg_data, size_t arg_len,
