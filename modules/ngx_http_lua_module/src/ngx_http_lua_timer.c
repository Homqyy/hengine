...
ngx_http_lua_ngx_timer_helper(lua_State *L, int every)
{
    ...
    {
        tctx->client_addr_text.data =
            ngx_palloc(tctx->pool, r->connection->addr_text.len);
...
ngx_http_lua_timer_copy(ngx_http_lua_timer_ctx_t *old_tctx)
{
    ...
    {
        tctx->client_addr_text.data =
            ngx_palloc(tctx->pool, tctx->client_addr_text.len);
        if (tctx->client_addr_text.data == NULL)
        {
            goto nomem;
        }
...
ngx_http_lua_abort_pending_timers(ngx_event_t *ev)
{
    ...
    while (n < lmcf->pending_timers)
    {
        ...
        if (prev == cur->parent)
        {
            ...
            {
                ev =
                    (ngx_event_t *)((char *)cur - offsetof(ngx_event_t, timer));
        ...
        else if (prev == cur->left)
        {
            ...
            ev = (ngx_event_t *)((char *)cur - offsetof(ngx_event_t, timer));
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "lua found %i pending timers to be aborted prematurely", n);
