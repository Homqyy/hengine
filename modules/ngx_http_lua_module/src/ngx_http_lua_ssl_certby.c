                                     ...
                                     lua_State               *L)
{
    ...
    rc = ngx_http_lua_cache_loadbuffer(
        r->connection->log, L, lscf->srv.ssl_cert_src.data,
        lscf->srv.ssl_cert_src.len, lscf->srv.ssl_cert_src_key,
...
char *
ngx_http_lua_ssl_cert_by_lua(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
#if OPENSSL_VERSION_NUMBER < 0x1000205fL

    ngx_log_error(
        NGX_LOG_EMERG, cf->log, 0,
        "at least OpenSSL 1.0.2e required but found " OPENSSL_VERSION_TEXT);
    ...
    else
    {
        ...
        p = ngx_palloc(cf->pool, sizeof("ssl_certificate_by_lua")
...
void *
ngx_http_lua_ffi_parse_pem_cert(const u_char *pem, size_t pem_len, char **err)
{
    BIO   *bio;
    X509  *x509;
    u_long n;
...
int
ngx_http_lua_ffi_set_cert(ngx_http_request_t *r, void *cdata, char **err)
{
#ifdef LIBRESSL_VERSION_NUMBER
...
int
ngx_http_lua_ffi_set_priv_key(ngx_http_request_t *r, void *cdata, char **err)
{
    EVP_PKEY       *pkey = NULL;
    ngx_ssl_conn_t *ssl_conn;
