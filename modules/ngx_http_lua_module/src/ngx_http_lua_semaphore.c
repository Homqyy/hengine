...
int ngx_http_lua_ffi_sema_new(ngx_http_lua_sema_t **psem, int n, char **errmsg);
int ngx_http_lua_ffi_sema_post(ngx_http_lua_sema_t *sem, int n);
int ngx_http_lua_ffi_sema_wait(ngx_http_request_t *r, ngx_http_lua_sema_t *sem,
                               int wait_ms, u_char *err, size_t *errlen);
...
ngx_http_lua_free_sema(ngx_http_lua_sema_t *sem)
{
    ...
    if (block->epoch < mid_epoch)
    {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       ...
                       "mid_epoch: %d cur_epoch: %d",
                       sem, (int)block->epoch, (int)mid_epoch,
                       (int)mm->cur_epoch);
    }
    else
    {
...
int
ngx_http_lua_ffi_sema_new(ngx_http_lua_sema_t **psem, int n, char **errmsg)
{
    ...
    *psem               = sem;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "http lua semaphore new: %p, resources: %d", sem,
                   sem->resource_count);
...
ngx_http_lua_ffi_sema_post(ngx_http_lua_sema_t *sem, int n)
{
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "http lua semaphore post: %p, n: %d, resources: %d", sem, n,
                   sem->resource_count);
...
int
ngx_http_lua_ffi_sema_wait(ngx_http_request_t *r, ngx_http_lua_sema_t *sem,
                           int wait_ms, u_char *err, size_t *errlen)
{
    ngx_http_lua_ctx_t    *ctx;
...
ngx_http_lua_ffi_sema_gc(ngx_http_lua_sema_t *sem)
{
    ...
    if (!ngx_terminate && !ngx_quit && !ngx_queue_empty(&sem->wait_queue))
    {
