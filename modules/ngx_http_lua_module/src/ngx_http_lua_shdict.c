    ...
    while (n < 3)
    {
        ...
        if (sd->value_type == SHDICT_TLIST)
        {
            ...
            for (lq = ngx_queue_head(list_queue);
                 lq != ngx_queue_sentinel(list_queue); lq = ngx_queue_next(lq))
            {
                lnode =
                    ngx_queue_data(lq, ngx_http_lua_shdict_list_node_t, queue);
        ...
        node = (ngx_rbtree_node_t *)((u_char *)sd
    ...
    case SHDICT_TNUMBER:
        ...
        if (value.len != sizeof(double))
        {
            ...
            return luaL_error(L,
                              ...
                              "in shared_dict %s: %lu",
                              key.data, name.data, (unsigned long)value.len);
        }
...
ngx_http_lua_shdict_flush_all(lua_State *L)
{
    ...
    for (q = ngx_queue_head(&ctx->sh->lru_queue);
         q != ngx_queue_sentinel(&ctx->sh->lru_queue); q = ngx_queue_next(q))
    {
        sd          = ngx_queue_data(q, ngx_http_lua_shdict_node_t, queue);
        sd->expires = 1;
    }
...
{
    return ngx_http_lua_shdict_set_helper(
        L, NGX_HTTP_LUA_SHDICT_ADD | NGX_HTTP_LUA_SHDICT_SAFE_STORE);
}
...
ngx_http_lua_shdict_set_helper(lua_State *L, int flags)
{
    ...
    remove:
        ...
        if (sd->value_type == SHDICT_TLIST)
        {
            ...
            for (q = ngx_queue_head(queue); q != ngx_queue_sentinel(queue);
                 q = ngx_queue_next(q))
            {
...
insert:
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_lua_shdict_node_t, data) + key.len + value.len;
    ...
    if (node == NULL)
    {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0,
                       ...
                       "due to memory shortage for entry \"%V\"",
                       &key);
...
allocated:
    ...
    if (exptime > 0)
    {
        tp = ngx_timeofday();
        sd->expires =
            (uint64_t)tp->sec * 1000 + tp->msec + (uint64_t)(exptime * 1000);
    }
    else
    {
        sd->expires = 0;
...
insert:
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_lua_shdict_node_t, data) + key.len + sizeof(double);
                             ...
                             size_t key_len, ngx_http_lua_value_t *value)
{
    ...
    case SHDICT_TNUMBER:
        ...
        if (len != sizeof(double))
        {
            ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                          "bad lua number "
                          "value size found for key %*s: %lu",
                          key_len, key_data, (unsigned long)len);
    ...
    default:
        ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0,
                      "bad lua value type "
                      "found for key %*s: %d",
                      key_len, key_data, (int)value->type);
...
init_list:
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_lua_shdict_node_t, data) + key.len
        + sizeof(ngx_queue_t);
...
push_node:
    ...
    n = offsetof(ngx_http_lua_shdict_list_node_t, data) + value.len;
...
ngx_http_lua_shdict_pop_helper(lua_State *L, int flags)
{
    ...
    if (sd->value_len <= 0)
    {
        ...
        return luaL_error(L,
                          ...
                          "in shared_dict %s: %lu",
                          key.data, name.data, (unsigned long)sd->value_len);
    }
...
ngx_http_lua_ffi_shdict_store(ngx_shm_zone_t *zone, int op, u_char *key,
                              ...
                              u_char *str_value_buf, size_t str_value_len,
                              ...
                              char **errmsg, int *forcible)
{
    ...
    *forcible = 0;
    ...
    default: *errmsg = "unsupported value type"; return NGX_ERROR;
    }
    ...
    replace:
            ...
            && sd->value_type != SHDICT_TLIST)
        {
            ...
            if (exptime > 0)
            {
                tp = ngx_timeofday();
                sd->expires =
                    (uint64_t)tp->sec * 1000 + tp->msec + (uint64_t)exptime;
            }
            else
            {
                sd->expires = 0;
...
insert:
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_lua_shdict_node_t, data) + key_len + str_value_len;
...
allocated:
    ...
    {
        tp          = ngx_timeofday();
        sd->expires = (uint64_t)tp->sec * 1000 + tp->msec + (uint64_t)exptime;
    }
    else
    {
        sd->expires = 0;
...
int
ngx_http_lua_ffi_shdict_get(ngx_shm_zone_t *zone, u_char *key, size_t key_len,
                            int *value_type, u_char **str_value_buf,
                            size_t *str_value_len, double *num_value,
                            int *user_flags, int get_stale, int *is_stale,
                            char **err)
{
    ngx_str_t                   name;
    uint32_t                    hash;
    ...
    *err = NULL;
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ctx->log, 0,
                   "fetching key \"%*s\" in shared dict \"%V\"", key_len, key,
                   &name);
#endif /* NGX_DEBUG */
...
int
ngx_http_lua_ffi_shdict_incr(ngx_shm_zone_t *zone, u_char *key, size_t key_len,
                             double *value, char **err, int has_init,
...
insert:
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_lua_shdict_node_t, data) + key_len + sizeof(double);
...
setvalue:
    ...
    if (init_ttl > 0)
    {
        sd->expires = (uint64_t)tp->sec * 1000 + tp->msec + (uint64_t)init_ttl;
    }
    else
    {
        sd->expires = 0;
