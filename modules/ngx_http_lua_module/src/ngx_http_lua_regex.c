...
static void      ngx_http_lua_re_collect_named_captures(
         lua_State *L, int res_tb_idx, u_char *name_table, int name_count,
         int name_entry_size, unsigned flags, ngx_str_t *subj);


#define ngx_http_lua_regex_exec(re, e, s, start, captures, size, opts)         \
    pcre_exec(re, e, (const char *)(s)->data, (s)->len, start, opts, captures, \
              size)
...
ngx_http_lua_ngx_re_match_helper(lua_State *L, int wantcaps)
{
    ...
    {
        return luaL_error(L,
                          "expecting 2, 3, 4 or 5 arguments, "
                          "but got %d",
                          nargs);
    }
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   ...
                   pat.data, opts.data, (flags & NGX_LUA_RE_COMPILE_ONCE) != 0,
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE)
    {
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       ...
                       "(entries %i)",
                       re_comp.captures, lmcf->regex_cache_entries);
...
exec:
    ...
    if (pcre_fullinfo(re_comp.regex, NULL, PCRE_INFO_NAMECOUNT, &name_count)
        != 0)
    {
    ...
    if (name_count > 0)
    {
        ...
        if (pcre_fullinfo(re_comp.regex, NULL, PCRE_INFO_NAMETABLE, &name_table)
            != 0)
        {
    ...
    if (flags & NGX_LUA_RE_MODE_DFA)
    {
        ...
        rc = ngx_http_lua_regex_dfa_exec(re_comp.regex, sd, &subj, (int)pos,
                                         cap, ovecsize, ws,
    ...
    for (i = 0, n = 0; i <= re_comp.captures; i++, n += 2)
    {
        ...
        {
            lua_pushlstring(L, (char *)&subj.data[cap[n]], cap[n + 1] - cap[n]);
...
ngx_http_lua_ngx_re_gmatch_iterator(lua_State *L)
{
    ...
    if (pcre_fullinfo(ctx->regex, NULL, PCRE_INFO_NAMECOUNT, &name_count) != 0)
    {
        msg = "cannot acquire named subpattern count";
        goto error;
    }
    ...
    if (name_count > 0)
    {
        ...
        if (pcre_fullinfo(ctx->regex, NULL, PCRE_INFO_NAMETABLE, &name_table)
            != 0)
        {
            msg = "cannot acquire named subpattern table";
            goto error;
    ...
    else
    {
        rc = ngx_http_lua_regex_exec(ctx->regex, ctx->regex_sd, &subj, offset,
                                     cap, ctx->captures_len, exec_opts);
    }
                               ...
                               ngx_str_t *opts, int narg)
{
    ...
    while (*p != '\0')
    {
        switch (*p)
        {
        case 'i': re->options |= NGX_REGEX_CASELESS; break;

        case 's': re->options |= PCRE_DOTALL; break;

        case 'm': re->options |= PCRE_MULTILINE; break;

        case 'u': re->options |= PCRE_UTF8; break;
        ...
        case 'x': re->options |= PCRE_EXTENDED; break;

        case 'o': flags |= NGX_LUA_RE_COMPILE_ONCE; break;

        case 'j': flags |= NGX_LUA_RE_MODE_JIT; break;

        case 'd': flags |= NGX_LUA_RE_MODE_DFA; break;

        case 'a': re->options |= PCRE_ANCHORED; break;
        ...
        case 'J': re->options |= PCRE_JAVASCRIPT_COMPAT; break;
#endif

        default:
            msg = lua_pushfstring(L, "unknown flag \"%c\" (flags \"%s\")", *p,
                                  opts->data);
...
ngx_http_lua_ngx_re_sub_helper(lua_State *L, unsigned global)
{
    ...
    case LUA_TNUMBER:
    case LUA_TSTRING: tpl.data = (u_char *)lua_tolstring(L, 3, &tpl.len); break;

    default:
        msg = lua_pushfstring(L,
                              "string, number, or function expected, "
                              "got %s",
                              lua_typename(L, type));
...
exec:
    ...
    for (;;)
    {
        if (flags & NGX_LUA_RE_MODE_DFA)
        {
            ...
            int ws[NGX_LUA_RE_DFA_MODE_WORKSPACE_COUNT];
            rc = ngx_http_lua_regex_dfa_exec(
                re_comp.regex, sd, &subj, offset, cap, ovecsize, ws,
                sizeof(ws) / sizeof(ws[0]), exec_opts);
        ...
        if (func)
        {
            ...
            if (name_count > 0)
            {
                ngx_http_lua_re_collect_named_captures(
                    L, lua_gettop(L), name_table, name_count, name_entry_size,
                    flags, &subj);
            }
            ...
            default:
                msg = lua_pushfstring(L,
                                      "string or number expected to be "
                                    ...
                                    size_t *errstr_size)
{
    ...
    lmcf->jit_stack = pcre_jit_stack_alloc(NGX_LUA_RE_MIN_JIT_STACK_SIZE, size);
    ...
    *errstr_size =
        ngx_snprintf(errstr, *errstr_size, "no pcre jit support found")
        - errstr;
    return NGX_ERROR;
...
ngx_http_lua_re_collect_named_captures(lua_State *L, int res_tb_idx,
                                       u_char *name_table, int name_count,
                                       int name_entry_size, unsigned flags,
                                       ngx_str_t *subj)
{
    int     i, n;
    size_t  len;
                               ...
                               size_t errstr_size)
{
    ...
    if (pcre_fullinfo(re_comp.regex, NULL, PCRE_INFO_NAMECOUNT, &re->name_count)
        != 0)
    {
        msg = "cannot acquire named subpattern count";
        goto error;
                            ...
                            const u_char *s, size_t len, int pos)
{
    ...
    if (flags & NGX_LUA_RE_MODE_DFA)
    {
        ...
        int ws[NGX_LUA_RE_DFA_MODE_WORKSPACE_COUNT];
        rc = ngx_http_lua_regex_dfa_exec(re->regex, sd, &subj, (int)pos, cap,
                                         ovecsize, ws,
                                         sizeof(ws) / sizeof(ws[0]), exec_opts);
...
ngx_http_lua_ffi_init_script_engine(ngx_http_lua_script_engine_t *e,
                                    ...
                                    ngx_http_lua_regex_t *compiled, int count)
{
    e->log           = ngx_cycle->log;
    e->ncaptures     = count * 2;
