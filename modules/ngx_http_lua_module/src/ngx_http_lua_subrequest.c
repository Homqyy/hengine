...
#define ngx_http_lua_method_name(m)    \
    {                                  \
        sizeof(m) - 1, (u_char *)m " " \
    }
...
ngx_http_lua_adjust_subrequest(ngx_http_request_t *sr, ngx_uint_t method,
                               int forward_body, ngx_http_request_body_t *body,
...
static void ngx_http_lua_process_vars_option(ngx_http_request_t *r,
                                             ...
                                             ngx_array_t **varsp);
static ngx_int_t
                 ngx_http_lua_subrequest_add_extra_vars(ngx_http_request_t *r,
                                                        ngx_array_t        *extra_vars);
static ngx_int_t ngx_http_lua_subrequest(ngx_http_request_t *r, ngx_str_t *uri,
                                         ngx_str_t                  *args,
...
ngx_http_lua_ngx_location_capture_multi(lua_State *L)
{
    ...
    p = ngx_pcalloc(r->pool, sr_statuses_len + sr_headers_len + sr_bodies_len
                                 + sr_flags_len);
    ...
    for (index = 0; index < nsubreqs; index++)
    {
        ...
        {
            return luaL_error(L,
                              "the query argument %d is not a table, "
                              "but a %s",
        ...
        {
            return luaL_error(L,
                              "query argument %d expecting one or "
                              "two arguments",
                              index);
        }
        ...
        if (nargs == 2)
        {
            ...
            {
                return luaL_error(L,
                                  "expecting table as the 2nd argument for "
                                  "subrequest %d, but got %s",
                                  index, luaL_typename(L, 4));
            }
            ...
            default: return luaL_error(L, "Bad args option value");
            }
            ...
            default: return luaL_error(L, "Bad vars option value");
            }
            ...
            default: return luaL_error(L, "Bad share_all_vars option value");
            }
            ...
            default: return luaL_error(L, "Bad copy_all_vars option value");
            }
        ...
        p = ngx_palloc(
            r->pool, ofs1 + ofs2 + sizeof(ngx_http_lua_post_subrequest_data_t));
        if (p == NULL)
        {
            return luaL_error(L, "no memory");
        }
        ...
        ngx_http_lua_assert((void *)sr_ctx
                            == ngx_align_ptr(sr_ctx, sizeof(void *)));

        p += ofs2;

        psr_data = (ngx_http_lua_post_subrequest_data_t *)p;

        ngx_http_lua_assert((void *)psr_data
                            == ngx_align_ptr(psr_data, sizeof(void *)));
                               ...
                               unsigned vars_action, ngx_array_t *extra_vars)
{
    ...
    if (body)
    {
        ...
        rc = ngx_http_lua_set_content_length_header(
            sr, body->buf ? ngx_buf_size(body->buf) : 0);
    ...
    else if (!always_forward_body && method != NGX_HTTP_PUT
             && method != NGX_HTTP_POST && r->headers_in.content_length_n > 0)
    {
    ...
    switch (method)
    {
    case NGX_HTTP_GET: sr->method_name = ngx_http_lua_get_method; break;

    case NGX_HTTP_POST: sr->method_name = ngx_http_lua_post_method; break;

    case NGX_HTTP_PUT: sr->method_name = ngx_http_lua_put_method; break;

    case NGX_HTTP_HEAD: sr->method_name = ngx_http_lua_head_method; break;

    case NGX_HTTP_DELETE: sr->method_name = ngx_http_lua_delete_method; break;

    case NGX_HTTP_OPTIONS: sr->method_name = ngx_http_lua_options_method; break;

    case NGX_HTTP_MKCOL: sr->method_name = ngx_http_lua_mkcol_method; break;

    case NGX_HTTP_COPY: sr->method_name = ngx_http_lua_copy_method; break;

    case NGX_HTTP_MOVE: sr->method_name = ngx_http_lua_move_method; break;
    ...
    case NGX_HTTP_LOCK: sr->method_name = ngx_http_lua_lock_method; break;

    case NGX_HTTP_UNLOCK: sr->method_name = ngx_http_lua_unlock_method; break;

    case NGX_HTTP_PATCH: sr->method_name = ngx_http_lua_patch_method; break;

    case NGX_HTTP_TRACE: sr->method_name = ngx_http_lua_trace_method; break;
...
static void
ngx_http_lua_process_vars_option(ngx_http_request_t *r, lua_State *L, int table,
                                 ngx_array_t **varsp)
{
    ngx_array_t  *vars;
    ngx_keyval_t *var;
...
ngx_http_lua_post_subrequest(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
    ...
    if (rc == NGX_ERROR || rc == NGX_HTTP_CREATED || rc == NGX_HTTP_NO_CONTENT
        || (rc >= NGX_HTTP_SPECIAL_RESPONSE && rc != NGX_HTTP_CLOSE
...
ngx_http_lua_set_content_length_header(ngx_http_request_t *r, off_t len)
{
    ...
    dd("r content length: %.*s", (int)r->headers_in.content_length->value.len,
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        if (ngx_http_lua_set_input_header(r, header[i].key, header[i].value, 0)
            == NGX_ERROR)
        {
            return NGX_ERROR;
        }
                                     ...
                                     ngx_http_lua_ctx_t *ctx)
{
    ...
    for (index = 0; index < coctx->nsubreqs; index++)
    {
        dd("summary: reqs %d, subquery %d, pending %d, req %.*s",
           (int)coctx->nsubreqs, (int)index, (int)coctx->pending_subreqs,
...
static ngx_int_t
ngx_http_lua_subrequest(ngx_http_request_t *r, ngx_str_t *uri, ngx_str_t *args,
                        ngx_http_request_t        **psr,
...
ngx_http_lua_copy_request_headers(ngx_http_request_t *sr, ngx_http_request_t *r)
{
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        dd("setting request header %.*s: %.*s", (int)header[i].key.len,
           header[i].key.data, (int)header[i].value.len, header[i].value.data);

        if (ngx_http_lua_set_input_header(sr, header[i].key, header[i].value, 0)
            == NGX_ERROR)
        {
            return NGX_ERROR;
        }
