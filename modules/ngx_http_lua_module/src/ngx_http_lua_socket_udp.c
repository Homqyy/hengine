...
static int ngx_http_lua_socket_resolve_retval_handler(
    ngx_http_request_t *r, ngx_http_lua_socket_udp_upstream_t *u, lua_State *L);
...
ngx_http_lua_inject_socket_udp_api(ngx_log_t *log, lua_State *L)
{
    ...
    lua_pushlightuserdata(
        L, ngx_http_lua_lightudata_mask(socket_udp_metatable_key));
    ...
    /* udp socket object metatable */
    lua_pushlightuserdata(
        L, ngx_http_lua_lightudata_mask(udp_udata_metatable_key));
...
ngx_http_lua_socket_udp_setpeername(lua_State *L)
{
    ...
    {
        return luaL_error(L,
                          "ngx.socket.udp setpeername: expecting 2 or 3 "
...
nomem:
    ...
    u->prepare_retvals = ngx_http_lua_socket_error_retval_handler;
    ngx_http_lua_socket_udp_handle_error(r, u, NGX_HTTP_LUA_SOCKET_FT_NOMEM);
...
ngx_http_lua_socket_udp_settimeout(lua_State *L)
{
    ...
    {
        return luaL_error(L,
                          "ngx.socket settimout: expecting at least 2 "
                          "arguments (including the object) but seen %d",
                          lua_gettop(L));
    }
                             ...
                             ngx_http_lua_socket_udp_upstream_t *u)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "lua udp recv returned %z",
                   n);
...
ngx_http_lua_udp_connect(ngx_http_lua_udp_connection_t *uc)
{
    ...
    if (uc->sockaddr->sa_family == AF_UNIX)
    {
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, &uc->log, 0,
                       "datagram unix "
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, &uc->log, 0, "connect to %V, fd:%d #%d",
                   &uc->server, s, c->number);
