...
static ngx_command_t ngx_http_upstream_chash_commands[] = {
    ...
    {ngx_string("consistent_hash"), NGX_HTTP_UPS_CONF | NGX_CONF_TAKE1,
     ngx_http_upstream_chash, 0, 0, NULL},
...
ngx_module_t ngx_http_upstream_consistent_hash_module = {
    NGX_MODULE_V1, &ngx_http_upstream_consistent_hash_module_ctx,
    /* module context */
...
ngx_http_upstream_init_chash(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
    ...
    ucscf = ngx_http_conf_upstream_srv_conf(
        us, ngx_http_upstream_consistent_hash_module);
    if (ucscf == NULL)
    {
        return NGX_ERROR;
    }
    ...
    ucscf->number = 0;
    ucscf->real_node =
        ngx_pcalloc(cf->pool, n * sizeof(ngx_http_upstream_chash_server_t **));
    ...
    for (i = 0; i < n; i++)
    {
        ...
        ucscf->real_node[i] = ngx_pcalloc(
            cf->pool,
            (peers->peer[i].weight * NGX_CHASH_VIRTUAL_NODE_NUMBER + 1)
    ...
    ucscf->servers =
        ngx_pcalloc(cf->pool, (ucscf->number + 1)
                                  * sizeof(ngx_http_upstream_chash_server_t));
    ...
    ucscf->d_servers = ngx_pcalloc(
        cf->pool,
        (ucscf->number + 1) * sizeof(ngx_http_upstream_chash_down_server_t));
                                  ...
                                  ngx_http_upstream_srv_conf_t *us)
{
    ...
    uchpd->ucscf = ucscf;
    if (ngx_http_script_run(r, &hash_value, ucscf->lengths->elts, 0,
                            ucscf->values->elts)
        == NULL)
    {
        return NGX_ERROR;
...
ngx_http_upstream_get_chash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (!ngx_queue_empty(&ucscf->down_servers))
    {
        ...
        while (q != ngx_queue_sentinel(&ucscf->down_servers))
        {
            ...
            down_server =
                ngx_queue_data(q, ngx_http_upstream_chash_down_server_t, queue);
            now = ngx_time();
    ...
    index = ngx_http_upstream_chash_get_server_index(
        ucscf->servers, ucscf->number, uchpd->hash);
    server = &ucscf->servers[index];

    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                   "consistent hash [peer name]:%V %ud", &server->peer->name,
                   server->hash);
...
#endif
        server->peer->fails > server->peer->max_fails || server->peer->down)
    {
        ngx_http_upstream_chash_delete_node(ucscf, server);

        while (1)
        {
            p      = ucscf->tree->query(ucscf->tree, 1, 1, ucscf->number, 1,
                                        index - 1);
            index1 = p->key;

            p = ucscf->tree->query(ucscf->tree, 1, 1, ucscf->number, index + 1,
                                   ucscf->number);
            index2 = p->key;
                                    ...
                                    ngx_http_upstream_chash_server_t   *server)
{
    ...
    for (; *servers; servers++)
    {
        ...
        if (!p->down)
        {
            ...
            p->down = 1;
            ucscf->d_servers[p->index].timeout =
                ngx_time() + p->peer->fail_timeout;
...
ngx_http_upstream_chash(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ucscf = ngx_http_conf_upstream_srv_conf(
        uscf, ngx_http_upstream_consistent_hash_module);
    if (ucscf == NULL)
    {
        return NGX_CONF_ERROR;
    }
...
#endif
                  | NGX_HTTP_UPSTREAM_WEIGHT | NGX_HTTP_UPSTREAM_MAX_FAILS
